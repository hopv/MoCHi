...
let is_binop = function
  | And|Or|Lt|Gt|Leq|Geq|EqUnit|EqBool|EqInt|Add|Sub|Mul|Div|MemSet|AddSet|Subset|LinSet -> true
  | _ -> false
...
let make_linset t1 t2 t3 = make_app (Const Subset) [t1; t2; t3]
...
let rec get_typ_arity = function
  | TFun(_,typ2) -> 1 + get_typ_arity (typ2 (Const Unit))
  | _ -> 0
