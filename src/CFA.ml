...
let add_flow flow t1 t2 =
  ...
  try
    ...
    if IntSet.cardinal fl = IntSet.cardinal fl' then false
    else (
      Hashtbl.replace flow id1 fl';
      true)
  with Not_found -> assert false

let get_flow map flow t =
  get_id t |> Hashtbl.find flow |> IntSet.elements |> List.map (Hashtbl.find map)
  ...
  Hashtbl.iter
    (fun n m ->
      if n <> 0 then
        ...
        match t.desc with
        | Var _ ->
            Debug.printf "%d:%a ===> %a@.@." n Print.term t (List.print Print.term)
              (List.map (Trans.remove_id -| Hashtbl.find map) @@ IntSet.elements m)
        | _ -> ())
    flow;
  ((flow, map), t'')
  ...
  let replace_const_term (flow, map) t =
    ...
    | Var (LId x) ->
        ...
        if List.for_all check ts then
          let cs =
            List.filter_map (fun t -> match t.desc with Const c -> Some c | _ -> None) ts
          in
          match cs with c :: cs' when List.for_all (( = ) c) cs' -> make (Const c) t.typ | _ -> t
        else t
    | Var _ -> unsupported "CFA"
    | _ -> tr.term_rec (flow, map) t
  in
  tr.term <- replace_const_term;
  Problem.map (cfa |- Fun.uncurry tr.term |- Trans.remove_id |- Trans.reconstruct)
