...
let string_of_command () =
  !Flag.Log.args
  |> List.mapi (fun i -> (i = 0) ^> Filename.basename)
...
let versions () =
  let mochi =
    ...
    Option.map Version.to_string !!version
  in
  let z3_lib = Some Z3.Version.full_version in
  let z3_bin =
    Option.map
      (fun z3 -> String.trim @@ Unix.CPS.open_process_in (z3 ^ " -version") IO.input_all)
      !Flag.Refine.z3
  in
  ...
  let hoice =
    Option.map
      (fun hoice ->
        List.nth (String.split_blanc @@ Unix.CPS.open_process_in (hoice ^ " -V") IO.input_all) 1)
      !Flag.Refine.hoice
  in
  let ocaml = Some Sys.ocaml_version in
  [
    (`Other "Build", mochi);
    (`Other "Z3 library", z3_lib);
    (`Other "Z3 binary", z3_bin);
    (`Version "HorSat", horsat);
    (`Version "TRecS", trecs);
    (`Version "HorSat", horsat);
    (`Version "HorSat2", horsat2);
    (`Version "HorSatP", horsatp);
    (`Version "HoIce", hoice);
    (`Version "OCaml", ocaml);
  ]

let string_of_label = function `Other s -> s | `Version s -> s ^ " version"

let print_env ~cmd ~json ppf =
  let pr fm = Format.fprintf ppf fm in
  if json then
    !!versions
    |> List.flatten_map (fun ((`Other s | `Version s), ver) ->
           match ver with None -> [] | Some ver -> [ (s, `String ver) ])
    |> (fun a -> `Assoc a)
    |> JSON.to_string
    |> pr "%s"
  else (
    Color.fprintf ppf Green "MoCHi: Model Checker for Higher-Order Programs@.";
    !!versions
    |> List.iter (fun (label, ver) ->
           match ver with None -> () | Some ver -> pr "  %s: %s@." (string_of_label label) ver);
    if cmd then pr "  Command: %s@." !!string_of_command)

let just_run_other_command cmd =
  if !Flag.IO.filenames = [] then (
    Format.eprintf {|Option "-just-run" must follow input file@.|};
    exit 1);
  let filename = List.hd !Flag.IO.filenames in
  let total, r =
    Time.measure (fun () -> Sys.command @@ snd @@ String.replace ~str:cmd ~sub:"%i" ~by:filename)
  in
...
let align_spec specs =
  specs |> List.flatten_map (fun (desc, spec) -> ("", Arg.Unit ignore, desc) :: spec) |> Arg.align
...
let rec spec_general =
  ( "General options",
    [
      ( "-I",
        Arg.String Parser_wrapper.add_load_path,
      ...
      ( "-color-always",
        Arg.Set Flag.PrettyPrinter.color_always,
        " Turn on syntax highlighting even if stdout does not refer to a terminal" );
      ( "-ignore-conf",
        Arg.Unit Flag.IO.(fun () -> if !conf_file = conf_file_default then conf_file := ""),
        " Ignore option.conf" );
      ...
      ( "-v",
        Arg.Unit
          (fun () ->
            Format.printf "%t" (print_env ~cmd:false ~json:false);
            exit 0),
        " Print the versions" );
      ( "-env",
        Arg.Unit
          (fun () ->
            Format.printf "%t" (print_env ~cmd:false ~json:true);
            exit 0),
        " Print the version and the environment as JSON" );
      ( "-version",
        Arg.Unit
          (fun () ->
            Format.printf "%t" (print_env ~cmd:false ~json:false);
      ...
      ( "-limit-sub",
        Arg.Set_int Flag.Limit.time_subproblem,
        " Set time limit for each sub-problem (seconds) (unsupported)" );
      ( "-limit-par",
        Arg.Set_int Flag.Parallel.time,
        " Set time limit for parallel execution (seconds) (unsupported)" );
      ...
      ( "-trans",
        Arg.Symbol
          ( List.map fst Flag.Trans.destinations,
            Flag.(
              fun s ->
                Trans.set_trans s;
                set_only_result ()) ),
        Format.asprintf "Translate the input to\n%s" !!Flag.Trans.string_of_destinations );
      ("-chc", Arg.Set Flag.Method.chc, " CHC solving mode");
      ("-p", Arg.Set_int Flag.Parallel.num, "<n>  Numbers of jobs to run simultaneously");
      ("-child", Arg.Set_string Flag.Parallel.child, "<id>.out  (Only used internally)");
      ( "-s",
        Arg.Unit set_silent,
        " Do not print information to stdout except in some printing options " );
      ( "-json",
        Arg.Unit
          (fun () ->
            set_silent ();
            Flag.Print.json := true),
        " Do not print information to stdout except the result in JSON format" );
      ("-json-file", Arg.Set_string Flag.Log.json_file, "<FILE>.json  Output result to <FILE>.json");
      ("-save-files", Arg.Set Flag.IO.save_files, " Save intermediate files to *.out/");
    ] )
...
and spec_debug =
  ( "Debug options",
    [
      ( "-debug",
        Arg.String Flag.Debug.set_debug_modules,
...
and spec_experiment =
  ...
  ( "Options for experiments",
    [
      ( "-just-run",
        Arg.String just_run_other_command,
        " (just for experiments, %i is replaced with the filename)" );
      ( "-hors-quickcheck-short",
        Arg.Unit Q.(fun () -> use := Some Shortest),
        " Use shortest counterexample generated by hors_quickcheck" );
      ( "-hors-quickcheck-long",
        Arg.Unit Q.(fun () -> use := Some Longest),
        " Use longest counterexample generated by hors_quickcheck" );
      ( "-hors-quickcheck-low",
        Arg.Unit Q.(fun () -> use := Some LowestCoverage),
        " Use lowest coverage counterexample generated by hors_quickcheck" );
      ( "-hors-quickcheck-high",
        Arg.Unit Q.(fun () -> use := Some HighestCoverage),
        " Use highest coverage counterexample generated by hors_quickcheck" );
    ] )
  ...
  ( "Options for abstraction",
    [
      ...
      ( "-ignore-non-termination",
        Arg.Set Flag.Method.ignore_non_termination,
        " Ignore non-termination" );
  ...
  ( "Options for completion",
    [
      ...
      ( "-debug-list",
        Arg.Unit
          (fun () ->
            List.iter (Format.fprintf !Flag.Print.target "%s@.") !Flag.Debug.debuggable_modules;
            exit 0),
        " Print list of debug options" );
      ( "-prep-list",
        Arg.Unit
          (fun () ->
      ...
      ( "-trans-list",
        Arg.Unit
          (fun () ->
            ...
            @@ List.map fst Flag.Trans.destinations;
            exit 0),
        " Print list of -trans destinations" );
    ] )
...
and spec_printing =
  ...
  ( "Options for printing",
    [
      ...
      ( "-print-cert",
        Arg.Set P.certificate,
        " Print certificates even if the model checker does not support certificates (need TRecS)"
      );
      ( "-print-depth",
        Arg.Int Print.set_depth,
        "<n>  Limit the printing of terms/types to a maximal depth of n" );
      ( "-print-length",
        Arg.Int Print.set_length,
        "<n>  Limit the printing of sequence of term/types to a maximal length of n" );
      ( "-print-assert-location",
        Arg.Unit
          (fun () ->
            P.assert_loc := true;
            P.exit_after_parsing := true),
        " Print the locations of assertions (used for -target)" );
      ( "-print-target-ext-location",
        Arg.Unit
          (fun () ->
            P.target_ext_loc := true;
            P.exit_after_parsing := true),
        " Print the locations of target external functions (used for -target)" );
      ( "-print-id-location",
        Arg.String
          (fun s ->
            P.id_loc := s;
            P.exit_after_parsing := true),
        {|<var>  Print the locations of <var> (used for "external" in spec)|} );
...
and spec_preprocessing =
  ( "Options for preprocessing",
    [
      ...
      ( "-replace-const",
        Arg.Set Flag.Method.replace_const,
        " Replace unchanging variables with constants" );
      ...
      ( "-disable-preprocess",
        Arg.Clear Flag.Method.init_trans,
        " Disable encoding of recursive data structures, CPS transformation, etc." );
      ...
      ( "-ins-param-funarg",
        Arg.Set Flag.Method.insert_param_funarg,
        " Insert an extra param for functions with function arguments" );
      ...
      ( "-data-to-int-but-exn",
        Arg.Set Flag.Abstract.data_to_int_but_exn,
        " Replace data types with int except exceptions" );
      ...
      ( "-encode-before-make-ext-fun",
        Arg.Set Flag.Method.encode_before_make_ext_fun,
        " Encode before make external functions" );
      ( "-make-ext-fun-before-encode",
        Arg.Clear Flag.Method.encode_before_make_ext_fun,
        " Make external functions before encode (BUGGY)" );
      ...
      ( "-reduce-memory",
        Arg.Set Flag.Preprocess.reduce_memory,
        " Reduce memory usage in preprocess. This disables some features." );
      ( "-recdata",
        Arg.Int
          (fun n ->
            ...
            additional := a),
        Format.asprintf
          "<n>  Encoding of recursive data types. Examples for int-labeled binary trees: \n%s"
        @@ String.join "\n"
        @@ List.map
             (fun (n, s) -> Format.sprintf "%d: %s" n s)
             [
      ...
      ( "-use-elim-may-div",
        Arg.Set Flag.Method.use_elim_may_div,
        " Eliminate unused let-bindings that may diverge" );
    ] )
...
and spec_verification =
  ( "Options for verification",
    [
        ...
        Arg.Unit
          Flag.(
            fun () ->
              ...
              Flag.Method.expand_nonrec_full := true),
      ...
      ( "-verify-ref-typ",
        Arg.Unit (fun () -> Flag.(mode := Verify_ref_typ)),
        " Verify whether functions have given refinement types" );
      ("-spec", Arg.Set_string Flag.IO.spec, "<filename>  use <filename> as a specification");
      ( "-use-spec",
        Arg.Set Flag.Method.use_spec,
        " use XYZ.spec for verifying XYZ.ml if exists\n(This option is ignored if -spec is used)" );
      ...
      ( "-module-verification",
        Arg.Unit (fun () -> Flag.(mode := Verify_module)),
        " Check input as library" );
      ( "-quickcheck",
        Arg.Unit (fun () -> Flag.(mode := Quick_check)),
        " Disprove safety via QuickCheck (other method options will be ignored)" );
      ("-only-specified", Arg.Set Flag.Method.only_specified, " Verify only specified targets");
      ( "-target",
        Arg.String
          (fun s ->
            Flag.Method.(targets := s :: !targets);
            Flag.Method.only_specified := true),
        "<location>  Verify only assertions matched with <location> (Use -print-assert-location)" );
      ( "-target-exn",
        Arg.String
          (fun s ->
            Flag.Method.(target_exn := s :: !target_exn);
            Flag.Method.only_specified := true),
        "<exception>  Verify only uncaght-exception <exception>" );
      ("-sub", Arg.Set Flag.Method.sub, " Verify sub-programs");
      ("-sub-hops", Arg.Set Flag.Method.sub_hops, " Verify sub-programs (sliced by hops)");
      ( "-verify-ref-interface",
        Arg.Set Flag.Method.verify_ref_interface,
        " Verify refinement type interface" );
      ( "-set-theory",
        Arg.Unit
          (fun () ->
            Flag.Method.use_set_theory := true;
...
and spec_cegar =
  ( "Options for CEGAR",
    [
      ( "-split-assert",
        Arg.Set Flag.Method.split_assert,
        " Reduce to verification of multiple programs\n(each program has only one assertion)" );
      ( "-disable-predicate-accumulation",
        Arg.Set Flag.Refine.disable_predicate_accumulation,
        " Disable predicate accumulation" );
    ] )
...
and spec_relative_complete =
  ( "Options for relatively complete verification",
    [
      ( "-relative-complete",
        Arg.Set Flag.Method.relative_complete,
        " Enable relatively complete verification from the begining" );
    ] )
...
and spec_predicate_abstraction =
  ...
  ( "Options for predicate abstraction",
    [
      ( "-abs-remove-false",
        Arg.Set P.remove_false,
        " Do not use unsatisfiable predicates in abstraction" );
      ( "-no-enr",
        Arg.Unit
          P.(
            fun () ->
              expand_non_rec := false;
              expand_non_rec_init := false),
        " Do not expand non-recursive functions" );
      ( "-enr",
        Arg.Unit
          P.(
            fun () ->
      ...
      ( "-neg-pred-off",
        Arg.Set P.never_use_neg_pred,
        " Never use negative predicates for abstraction" );
      ( "-decomp-pred",
        Arg.Set P.decomp_pred,
        " Decompose abstraction predicates (e.g., [P1 && P2] ==> [P1, P2])" );
      ( "-decomp-eq-pred",
        Arg.Set P.decomp_eq_pred,
        " Decompose abstraction predicates on equalities (e.g., [t1 = t2] ==> [t1 <= t2, t1 >= t2])"
      );
...
and spec_predicate_discovery =
  ...
  ( "Options for predicate discovery",
    [
      ...
      ( "-bool-init-empty",
        Arg.Set Flag.PredAbst.bool_init_empty,
        " Use an empty set as the initial sets of predicates for booleans" );
      ( "-bool-init-self",
        Arg.Clear Flag.PredAbst.bool_init_empty,
        " Use predicates of themselves as the initial sets of predicates for booleans" );
      ( "-mp",
        Arg.Set R.use_multiple_paths,
        " Use multiple infeasible error paths for predicate discovery" );
      ( "-no-simplification",
        Arg.Set Flag.PredAbst.no_simplification,
        " Do not simplify abstracted programs" );
      ...
      ( "-rec-chc-limit",
        Arg.Set_int R.solver_timelimit,
        " Set time limit for recursive CHC solver (seconds)" );
      ( "-rec-chc-app-id",
        Arg.Set Flag.Method.occurence_param,
        " Add extra parameter for application ID" );
      ( "-hoice",
        Arg.Unit R.(fun () -> solver := Hoice),
        " Use HoICE as the recursive horn-clause solver" );
      set_bin "hoice" R.hoice;
      ("-z3", Arg.Unit (fun () -> R.(solver := Z3)), " Use Z3 as the recursive horn-clause solver");
      set_bin "z3" R.z3;
      ( "-z3-spacer",
        Arg.Unit R.(fun () -> solver := Z3_spacer),
        " Use Z3 (Spacer) as the recursive horn-clause solver" );
      set_bin "z3-spacer" R.z3_spacer;
    ] )

and spec_smt = ("Options for SMT solver", [ set_bin "cvc3" Flag.External.cvc3 ])

and spec_fair_termination =
  ( "Options for fair termination mode",
    [
      ( "-fair-termination",
        Arg.Unit Flag.(fun () -> mode := FairTermination),
        " Check fair termination" );
...
and spec_termination =
  ...
  ( "Options for termination mode",
    [
      ...
      ("-termination", Arg.Unit Flag.(fun _ -> mode := Termination), " Check termination");
      ( "-termination-sep",
        Arg.Unit
          Flag.(
            fun _ ->
      ...
      ( "-non-termination",
        Arg.Unit
          (fun _ ->
            Flag.NonTermination.use_omega := true;
...
and spec_non_termination =
  ...
  ( "Options for non-termination mode",
    [
      ...
      ("-use-omega", Arg.Set N.use_omega, " Use omega solver for under-approximation");
      ("-no-use-omega", Arg.Clear N.use_omega, " Do not use omega solver for under-approximation");
      ( "-use-omega-first",
...
let set_file name =
  ...
  | ".bin" -> Flag.IO.(filenames := [ name ])
  | ".cegar" | ".debug" | ".smt2" ->
      if !Flag.IO.filenames <> [] then
        unsupported "Multiple files for *%s (only the last file is used)" ext;
...
let add_includes () =
  ...
  if Sys.file_exists inc && not (Sys.is_directory inc) then (
    let s = IO.input_file inc in
    if String.exists_stdlib (List.mem -$- [ '\\'; '\"'; '\'' ]) s then unsupported "%s" __FUNCTION__;
    let args =
      s
