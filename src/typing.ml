...
let assoc_env x env = try List.assoc x env with Not_found -> CEGAR_util.type_not_found x
...
let rec unify typ1 typ2 =
  ...
  | TAbst "string", TAbst "Pervasives.format" | TAbst "Pervasives.format", TAbst "string" -> ()
  | TFun (typ11, typ12), TFun (typ21, typ22) ->
      unify typ11 typ21;
      unify typ12 typ22
  | TTuple [], typ | typ, TTuple [] -> unify TUnit typ
  | TTuple typs1, TTuple typs2 -> List.iter2 unify typs1 typs2
...
let rec trans_typ = function
  ...
  | TTuple typs -> TConstr (TTuple, List.map trans_typ typs, nil)
...
let rec infer_term fun_arg_env env = function
  ...
  | Var x -> (
      try assoc_env x env
      with CEGAR_util.Type_not_found _ when Fpat.RefTypInfer.is_parameter x -> TInt)
  | App (t1, t2) ->
...
let infer ?(fun_annot = false) ?(rename = false) prog =
  ...
  if fun_annot then CEGAR_util.map_body_prog (annotate_fun_arg_term !fun_arg_env) prog else prog
