...
let get_top_ty ty =
  let open Flag.Encode.RecData in
  match !additional with Nothing -> ty | Top -> fst_typ ty | Unit_top -> proj_typ 1 ty

let get_element pos t_f =
  let open Flag.Encode.RecData in
  match !additional with
  | Nothing -> Term.(t_f @ [ pos ])
  | Top -> if pos.desc = Nil then Term.(fst t_f) else Term.(snd t_f @ [ pos ])
  | Unit_top -> if pos.desc = Nil then Term.(proj 1 t_f) else Term.(proj 2 t_f @ [ pos ])
          ...
          |> List.map (fun row ->
                 let row_args =
                   ...
                   |> List.map (function
                        ...
                        | _ ->
                            unsupported
                              "encode_rec_variant: non-simple recursion (use -data-to-int, \
                               -ignore-data-arg, etc.)")
                 in
                 { row with row_args })
        in
...
let term_of_path xs = make_list ~typ:Ty.int @@ List.map Term.int xs
...
let abst_recdata_term_rec env x_top _path t =
  ...
  let paths =
    let rec aux ix t1 =
      if ty_orig <> t1.typ then []
        ...
        match t1.desc with
        | Constr (true, _, _) -> unsupported "%s (poly)" __MODULE__
        | Constr (false, _, ts) -> (ix, None) :: List.flatten_mapi (fun i t -> aux (i :: ix) t) ts
        | _ ->
  ...
  let rec proj ix tail t1 =
    if ty_orig <> t1.typ then abst_recdata.term env t1
    else
      match (ix, t1.desc) with
      | [], Constr (false, s1, ts1) ->
          let ts1' =
            let aux t2 = if ty_orig = t2.typ then Term.unit else abst_recdata.term env t2 in
            List.map aux ts1
          in
      ...
      | i :: ix', Constr (false, _s1, ts1) -> proj ix' tail (List.nth ts1 i)
      | _ -> assert false
  in
  let proj' (ix, tail) t1 = if ix = [] then Term.var x_top else proj ix tail t1 in
  let pat_of_path (ix, tail) =
    let tl = match tail with None -> Pat.nil ~elem_typ:Ty.int | Some tl -> Pat.var tl in
...
let abst_recdata_term (env : env) t =
  ...
  | Match (t1, pats) ->
      ...
      let aux (p, t) t_acc =
        let t_acc' =
          let p', bind = abst_recdata_pat env p in
          let t' = abst_recdata.term env t in
          let pat_acc = match t_acc with None -> [] | Some t -> [ (Pat.(__ t'.typ), t) ] in
          if bind = [] then Term.(match_ (var x) ((p', t') :: pat_acc))
          else
            let ts, ps = List.split bind in
            let tp = Term.tuple ts in
            let pp = Pat.tuple ps in
            match p'.pat_desc with
            | PVar y -> subst_var y x @@ Term.(match_ tp ((pp, t') :: pat_acc))
            | _ ->
                let cond =
                  Term.(match_ tp [ (pp, Term.true_); (Pat.(__ pp.pat_typ), Term.false_) ])
                in
  ...
  | Local (Decl_type [ (s, (params, ty)) ], t) ->
      ...
      let env' =
        (s, (ty, List.exists (fst |- ( = ) (Type.LId s)) (get_tconstr_typ ty), ty')) :: env
      in
...
let trans_pred =
  ...
  let term (x, sym_env) t =
    ...
    | Match ({ desc = Var (LId y) }, ps) when Id.(x = y) ->
        ...
        let preds =
          snd
          @@ List.Labels.fold_right ps ~init:(labels, []) ~f:(fun (p, t) (remain, arms) ->
                 match p.pat_desc with
  ...
  List.Labels.mapi lts ~f:(fun i (l, (tys, tys')) ->
      ...
      let map : (int, term) Map.t =
        ...
        let rec go m i j tys tys' =
          match (tys, tys') with
          | _, [] -> m
          | TConstr (s', _) :: tys, _ when s = s' -> go m (i + 1) j tys tys'
          | _ :: tys, _ :: tys' ->
...
let rec trans_rty env =
  ...
  | Constr (s, tys, x, t) ->
      ...
      if is_rec_type env (TConstr (s, [])) then trans_rty_rec_data (s, x, t) ty_before ty_after
  ...
  | List (x, p_len, y, p_i, ty2) ->
      List (tr.var x, tr.term p_len, tr.var y, tr.term p_i, trans_rty env ty2)
...
let trans_env env (x, ty) = (trans_rid env x, trans_rty env ty)
