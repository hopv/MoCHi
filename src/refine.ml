...
let add_preds_env map env =
  let aux (f, typ1) =
    let typ' = List.assoc_opt f map |> Option.map @@ merge_typ typ1 |> Option.default typ1 in
    (f, typ')
  in
  List.map aux env
...
let rec negate_typ ty =
  ...
  | TConstr _ ->
      Format.eprintf "negate_typ: %a." CEGAR_print.typ ty;
      assert false
...
let add_preds map prog = { prog with env = add_preds_env map prog.env }

let rec col_fix_pred path env_rev acc ty =
  match ty with
  | TBase _ -> acc
  | TConstr (TFixPred p, [ (TBase _ as ty') ], _) ->
      let x = new_id "x" in
...
let fix_pred_of f ty =
  ...
  |> col_fix_pred [] [] []
  |@> Debug.printf "[fix_pred_of] OUTPUT %s : %a@." f
        Print.(list (triple (list int) __ CEGAR_print.term))
...
let rec proj_paths acc_xs_rev acc_path_rev paths ty =
  if paths = [] then []
    ...
    | TBase (b, ps) when List.exists (( = ) []) paths ->
        let x = new_id "x" in
        let ps' = ps (Var x) |> List.filter_out (List.mem dummy -| get_fv) in
    ...
    | TFun (ty1, ty2) ->
        ...
        @ proj_paths (x :: acc_xs_rev) (1 :: acc_path_rev) paths2'
        @@ ty2 (Var x)
...
let instansiate_pred_by_env env c =
  ...
  let rec has_path x (y, path) =
    match x with
    | Fpat.Idnt.T (x', _, arg) -> (
        match path with [] -> false | i :: path' -> i = arg && has_path x' (y, path'))
    | _ -> path = [] && x = y
  in
  let map =
    let aux (x, _) =
...
let refine labeled is_cp _prefix ces ext_ces prog =
  let tmp = Time.get () in
  try
    Verbose.printf "%a(%d-4)[%.3f] Discovering predicates (infeasible case) ...%t @." Color.set
      Color.Green !Flag.Log.cegar_loop !!Time.get Color.reset;
    set_status @@ Flag.Log.Other (Format.sprintf "(%d-4) Predicate discovery" !Flag.Log.cegar_loop);
    if Flag.Refine.use_prefix_trace then failwith "Not implemented: Flag.use_prefix_trace";
    let map =
      let ces, ext_ces =
        if !Flag.Refine.use_multiple_paths then (ces, ext_ces)
        else ([ List.hd ces ], [ List.hd ext_ces ])
      in
      let solver orig c =
        ...
        let r =
          if !Flag.Refine.use_rec_chc_solver then
            try Rec_CHC_solver.solve c'
            with Rec_CHC_solver.TimeOut | Rec_CHC_solver.SolverAborted -> orig c'
          else orig c'
        in
        Verbose.printf "@]";
        r
    ...
    let env =
      if !Flag.Refine.disable_predicate_accumulation then map' else add_preds_env map' prog.env
    in
    Verbose.printf "DONE!@.@.";
    refine_post tmp;
    (map, { prog with env })
...
let refine_with_ext labeled is_cp _prefix ces ext_ces prog =
  let tmp = Time.get () in
  try
    Verbose.printf ~color:Green "(%d-4)[%.3f] Discovering predicates (feasible case) ... @."
...
let print_list fm = function
  | [] -> Format.fprintf fm "[]@."
  | x :: xs ->
      let rec iter = function [] -> "" | y :: ys -> ", " ^ string_of_int y ^ iter ys in
...
let refine_rank_fun ce ex_ce prog =
  ...
  try
    ...
    Verbose.printf "(%d-4)[%.3f] Discovering ranking function ... @." !Flag.Log.cegar_loop
      !!Time.get;
    set_status
    @@ Flag.Log.Other (Format.sprintf "(%d-4) Ranking function discovery" !Flag.Log.cegar_loop);
    let env, spc =
      Format.fprintf !Flag.Print.target "@[<v>";
