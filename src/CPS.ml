...
and t_cps =
  ...
  | MatchCPS of term * ((path * typed_ident) * term * term) * term
    (* MatchCPS(t1,(("Constr",x),cond,t2),t3) means "match t1 with Constr x when cond -> t2 | _ -> t3" *)
...
and effect_constr = CGeq of effect_var * effect | CGeqVar of effect_var * effect_var
[@@deriving show]

type env = { mutable constraints : effect_constr list; mutable counter : int }

let effect_max x y =
  match (x, y) with
  | EUnknown, _ | _, EUnknown -> assert false
  | ENone, _ -> y
  | _, ENone -> x
  | _, EExcep -> EExcep
  | EExcep, _ -> EExcep
...
module Pr = struct
  let rec typ_cps sol fm typ =
    ...
    | TTupleCPS typs -> Format.fprintf fm "{%a}" (print_list (typ_cps sol) " *@ ") typs
    | TVariantCPS styss ->
        let pr fm (s, tys) =
          if tys = [] then Format.fprintf fm "%a" Print_typ.constr s
          else Format.fprintf fm "%a of %a" Print_typ.constr s (print_list (typ_cps sol) " *@ ") tys
        in
        Format.fprintf fm "(%a)" (print_list pr " |@ ") styss
    | TSetCPS ty -> Format.fprintf fm "(%a set)" (typ_cps sol) ty
  ...
  and term sol fm { t_cps = t; typ_cps = typ; effect = e } =
    ...
    | true, e ->
        Format.fprintf fm "(%a :%a: %a)" (t_cps sol) t (Color.green effect) e
          (Color.cyan @@ typ_cps sol)
          typ
  ...
  and t_cps sol fm = function
    ...
    | FunCPS (x, t) ->
        Format.fprintf fm "@[<hov 2>fun %a : %a ->@ %a@]" Print.id x.id_cps (typ_cps sol) x.id_typ
          (term sol) t
    | AppCPS (t1, t2) -> Format.fprintf fm "%a%a" (term sol) t1 (term sol) t2
    | IfCPS (t1, t2, t3) ->
        Format.fprintf fm "@[@[if %a@]@;then @[%a@]@;else @[%a@]@]" (term sol) t1 (term sol) t2
          (term sol) t3
    | BranchCPS (t1, t2) -> Format.fprintf fm "@[br@; @[%a@]@; @[%a@]@]" (term sol) t1 (term sol) t2
    | LetCPS (bindings, t) ->
        let head = ref "let rec" in
        let pr fm (f, t) =
          Format.fprintf fm "@[<hov 2>%s %a : %a =@ @[%a@]@]@;" !head Print.id f.id_cps
            (typ_cps sol) f.id_typ (term sol) t;
          head := "and"
        in
    ...
    | NotCPS t -> Format.fprintf fm "not %a" (term sol) t
    | EventCPS s -> Format.fprintf fm "{%s}" s
    | ProjCPS (i, t) -> Format.fprintf fm "#%d %a" i (term sol) t
    | TupleCPS ts -> Format.fprintf fm "(%a)" (print_list (term sol) ",@ ") ts
    | RaiseCPS t -> Format.fprintf fm "@[raise %a@]" (term sol) t
    | TryWithCPS (t1, t2) ->
        Format.fprintf fm "@[<hov 2>@[<hov 2>try@ %a@]@ with@ %a@]" (term sol) t1 (term sol) t2
    | ConstrCPS (s, t) -> Format.fprintf fm "%a %a" Print_typ.path s (term sol) t
    | MatchCPS (t1, ((s, x), cond, t2), t3) ->
        let pr_cond fm =
          if cond.t_orig.desc <> Const True then Format.fprintf fm " when %a" (term sol) cond
        in
        Format.fprintf fm "@[<hov 2>@[<hov 2>match@ %a@]@ with@ %a %a%t@ ->@ %a@ | _ ->@ %a]"
          (term sol) t1 Print_typ.path s Print.id x.id_cps pr_cond (term sol) t2 (term sol) t3
...
let decomp_set_typ ty = match ty with TSetCPS ty -> ty | _ -> invalid_arg "decomp_set_typ"
...
let rec unify env typ1 typ2 =
  ...
  | TSetCPS ty1, TSetCPS ty2 -> unify env ty1 ty2
  | _ ->
      Format.eprintf "Bug?@.typ1: %a@.typ2: %a@."
        (Pr.typ_cps (fun _ -> ENone))
        typ1
        (Pr.typ_cps (fun _ -> ENone))
        typ2;
      Format.eprintf "typ1: %a@.typ2: %a@." pp_typ_cps typ1 pp_typ_cps typ2;
      assert false
...
let rec lift_letrec_typ env typed =
  ...
  | FunCPS (_, ({ t_cps = FunCPS _ } as typed1)), _ -> lift_letrec_typ env typed1
  | FunCPS _, TFunCPS (e, _, _) -> env.constraints <- CGeq (e, ECont) :: env.constraints
  | FunCPS _, _ -> assert false
  | _ -> ()

let rec etyp_of_typ ty =
  match ty with
  | TBase _ | TConstr (_, []) -> TBaseCPS ty
...
let rec infer_effect_typ env typ =
  match typ with
  | TBase _ | TConstr (_, []) -> TBaseCPS typ
  | TFun (x, typ2) ->
...
let rec infer_effect env tenv t =
  ...
  let t_cps, typ_cps, effect =
    ...
    | Const c -> (ConstCPS c, TBaseCPS t.typ, new_evar ())
    | Bottom ->
        let e = new_evar () in
        env.constraints <- CGeq (e, ECont) :: env.constraints;
    ...
    | Var (LId x) ->
        let typ =
          try List.assoc (Id.to_string x) tenv with
    ...
    | Fun (x, t1) ->
        ...
        let e, a_typ, r_typ =
          match typ' with TFunCPS (e, typ1, typ2) -> (e, typ1, typ2) | _ -> assert false
        in
        if !Flag.Method.cps_simpl then env.constraints <- CGeq (e, ECont) :: env.constraints;
    ...
    | Proj (i, t1) ->
        ...
        let typ1 =
          match typ with
          | TTupleCPS typs -> List.nth typs i
          | _ ->
    ...
    | Tuple ts ->
        ...
        let e = new_evar () in
        List.iter
          (fun typed -> env.constraints <- CGeqVar (e, typed.effect) :: env.constraints)
          typeds;
        env.constraints <- CGeq (e, ECont) :: env.constraints;
        (* for remove_pair *)
    ...
    | TryWith (t1, t2) ->
        ...
        let typ = infer_effect_typ env t.typ in
        let _e2, typ2 =
          match typed2.typ_cps with TFunCPS (e, _typ1, typ2) -> (e, typ2) | _ -> assert false
        in
    ...
    | Match (t1, (p, t2) :: pats) ->
        let p', cond =
          match p.pat_desc with PWhen (p', cond) -> (p', cond) | _ -> (p, Term.true_)
        in
        let s, x =
          match p'.pat_desc with
...
let solve_constraints constrs =
  ...
  let cgeqvars =
    List.filter_map (function CGeqVar (x, y) when x <> y -> Some (x, y) | _ -> None) constrs
  in
  ...
  fun e -> match sol.(e) with None -> ENone | Some e -> e
...
let check_solution sol env =
  ...
  let aux = function
    | CGeqVar (x, y) ->
        ...
        let e2 = sol y in
        if dbg then
          Format.fprintf !Flag.Print.target "%a(%a) :> %a(%a)@." Pr.evar x Pr.effect e1 Pr.evar y
            Pr.effect e2;
        check e1 e2
    | CGeq (x, e) ->
        let e1 = sol x in
        if dbg then
          Format.fprintf !Flag.Print.target "%a(%a) :> %a@." Pr.evar x Pr.effect e1 Pr.effect e;
        check e1 e
  in
...
let rec add_preds_cont_aux k t =
  let desc =
    ...
    | App (t1, ts) ->
        let aux t (typ, ts) =
          let x, typ' =
            match typ with TFun (x, typ) -> (x, subst_type x t typ) | _ -> assert false
          in
          let t' =
            if t.desc = Var (LId k) then make_var (Id.set_typ k @@ Id.typ x)
    ...
    | If (t1, t2, t3) ->
        If (add_preds_cont_aux k t1, add_preds_cont_aux k t2, add_preds_cont_aux k t3)
...
let add_preds_cont k t =
  ...
  if List.length ks = 0 then (
    assert (t.desc = Bottom);
    (k, t'))
  else (
...
let rec trans_typ sol typ_excep typ_orig typ =
  ...
  | TAttr (attr, typ_orig'), _ ->
      let aux a = match a with TAPureFun -> false | TAEffect _ -> false | _ -> true in
...
let trans_var' sol typ_excep x typ =
  (* for predicates *)
  let x' = trans_var sol typ_excep x in
  if same_shape typ @@ Id.typ x' then x' else Id.set_typ x' typ

let get_tfun_effect = function TFunCPS (e, _, _) -> e | _ -> assert false

let make_app' t1 ts =
  match (t1.desc, ts) with
  | Fun (x, t1'), [ t2 ] -> subst x t2 t1'
  | Fun (x1, { desc = Fun (x2, t1') }), [ t2; t3 ] ->
      if count_occurrence x2 t1' >= 2 then
        (* t3 must be an exception hanadler *)
        Term.(let_ [ (x2, t3) ] ((x1 |-> t2) t1'))
      else subst_map [ (x1, t2); (x2, t3) ] t1'
  | _ -> make_app t1 ts
  ...
  let t' =
    ...
    | AppCPS (t1, t2), EExcep ->
        ...
        funs [ k; h ]
          (let_
             [ (h', var h) ]
             (* to prevent the increase of code size in eta-reduction *)
             (app (sol t1.effect) t1' ~h:(var h')
                ~k:
                  (fun_ x1
                     (app (sol t2.effect) t2' ~h:(var h')
    ...
    | BranchCPS (t1, t2), ECont ->
        ...
        fun_ k
          (let_
             [ (k', var k) ]
             (merge_attrs t_orig.attr
                (br (app (sol t1.effect) t1' ~k:(var k')) (app (sol t2.effect) t2' ~k:(var k')))))
    ...
    | LetCPS (bindings, t1), ECont ->
        ...
        let aux (f, t) =
          ...
          let f' = trans_var sol typ_excep f in
          let f'' = if sol t.effect = ENone then f' else Id.set_typ f' t'.typ in
          (f'', t')
        in
        let bindings' = List.map aux bindings in
        let t1' = tr t1 in
        ...
        in
        let t1'' =
          List.fold_right2 aux bindings bindings' @@ app (sol t1.effect) t1' ~k:(make_var k)
        in
    ...
    | BinOpCPS (op, t1, t2), ECont ->
        ...
        fun_ k
          (app (sol t1.effect) t1'
             ~k:
               (fun_ x1 (app (sol t2.effect) t2' ~k:(fun_ x2 (var k @ [ var x1 <| op |> var x2 ])))))
    ...
    | NotCPS t1, EExcep ->
        ...
        let t1' = tr t1 in
        let open Term in
        funs [ k; h ] (app (sol t1.effect) t1' ~h:(var h) ~k:[%term fun b -> k (not b)])
    | EventCPS s, ENone -> make_event_cps s
    | ProjCPS (i, t1), ENone -> make_proj i @@ tr t1
    | ProjCPS (i, t1), ECont ->
        let r = Id.new_var ~name:"r" @@ tr_typ typ_orig typ in
        let k = Id.new_var ~name:("k" ^ k_post) (TFun (r, typ_result)) in
    ...
    | ProjCPS (i, t1), EExcep ->
        ...
        let t1' = tr t1 in
        let open Term in
        funs [ k; h ] (app (sol t1.effect) t1' ~h:(var h) ~k:[%term fun p -> k p##i])
    | TupleCPS ts, ENone -> Term.tuple @@ List.map tr ts
    | TupleCPS ts, ECont ->
        let r = Id.new_var ~name:"r" (tr_typ typ_orig typ) in
        let k = Id.new_var ~name:("k" ^ k_post) (TFun (r, typ_result)) in
    ...
    | TryWithCPS (t1, _), ENone -> tr t1
    | TryWithCPS (t1, _), ECont -> tr t1
    | TryWithCPS (t1, t2), EExcep ->
        ...
        (* bind h' to h when eliminating this assertion *)
        let open Term in
        funs [ k; h ]
               ...
               (fun_ e
                  (app (sol t2.effect) t2' ~h:(var h)
                     ~k:
                       (fun_ f
                          (app
                             (sol (get_tfun_effect t2.typ_cps))
                             (var f @ [ var e ])
                             ~k:(var k) ~h:(var h))))))
    | ConstrCPS (s, t1), ECont ->
        ...
        let t1' = tr t1 in
        let open Term in
        fun_ k (app (sol t1.effect) t1' ~k:(fun_ x (var k @ [ funs ks (var ki @ [ var x ]) ])))
    | ConstrCPS _, EExcep -> unsupported "CPS.transform Constr"
    | MatchCPS (t1, ((s, x), cond, t2), t3), ECont ->
        ...
        fun_ k
          (let_
             [ (k', var k) ]
             (merge_attrs t_orig.attr
                (app (sol t1.effect) t1' ~k:(fun_ c (let_ [ (x3, t3') ] (var c @ ts))))))
    | MatchCPS (_t1, ((_s, _xs), _cond, _t2), _t3), EExcep -> unsupported "CPS.transform Match"
    | MemSetCPS (t1, t2), ENone ->
        (* TODO: merge with AddSet, Subset, etc. *)
        let t1' = tr t1 in
        let t2' = tr t2 in
        Term.(mem t1' t2')
    | MemSetCPS (t1, t2), ECont ->
        ...
        fun_ k
          (app (sol t1.effect) t1'
             ~k:(fun_ x1 (app (sol t2.effect) t2' ~k:(fun_ x2 (var k @ [ mem (var x1) (var x2) ])))))
    ...
    | AddSetCPS (t1, t2), ECont ->
        ...
        fun_ k
          (app (sol t1.effect) t1'
             ~k:
               (fun_ x1
                  (app (sol t2.effect) t2' ~k:(fun_ x2 (var k @ [ addset (var x1) (var x2) ])))))
    ...
    | SubsetCPS (t1, t2), ECont ->
        ...
        fun_ k
          (app (sol t1.effect) t1'
             ~k:
               (fun_ x1
                  (app (sol t2.effect) t2' ~k:(fun_ x2 (var k @ [ subset (var x1) (var x2) ])))))
...
let rec col_exn_typ { t_cps = t } =
  ...
  | LetCPS (bindings, t2) ->
      List.fold_left (fun acc (_, t) -> col_exn_typ t @ acc) (col_exn_typ t2) bindings
  ...
  | MatchCPS (t1, (_, cond, t2), t3) ->
      col_exn_typ t1 @ col_exn_typ cond @ col_exn_typ t2 @ col_exn_typ t3
...
let rec assoc_typ_cps f { t_cps } =
  ...
  | RandCPS _ -> []
  | VarCPS _ -> []
  | FunCPS (_, t1) -> assoc_typ_cps f t1
  | AppCPS (t1, t2) -> assoc_typ_cps f t1 @@@ assoc_typ_cps f t2
  | IfCPS (t1, t2, t3) -> assoc_typ_cps f t1 @@@ assoc_typ_cps f t2 @@@ assoc_typ_cps f t3
  | BranchCPS (t1, t2) -> assoc_typ_cps f t1 @@@ assoc_typ_cps f t2
  | LetCPS (bindings, t1) ->
      let aux (g, t) =
  ...
  | BinOpCPS (_, t1, t2) -> assoc_typ_cps f t1 @@@ assoc_typ_cps f t2
  | NotCPS t1 -> assoc_typ_cps f t1
  | EventCPS _ -> []
  | ProjCPS (_, t1) -> assoc_typ_cps f t1
  | TupleCPS ts -> List.rev_flatten @@ List.map (assoc_typ_cps f) ts
  | RaiseCPS t1 -> assoc_typ_cps f t1
  | TryWithCPS (t1, t2) -> assoc_typ_cps f t1 @@@ assoc_typ_cps f t2
  | ConstrCPS (_, t1) -> assoc_typ_cps f t1
  | MatchCPS (t1, (_, cond, t2), t3) ->
...
let rec uncps_ref_type sol typ_exn rtyp e etyp =
  ...
  | RT.Fun (x, rtyp1, rtyp2), ENone, TFunCPS (e, etyp1, etyp2) ->
      ...
      RT.Fun (x', rtyp1', rtyp2')
  | RT.Fun (_, RT.Fun (_, rtyp, RT.Base (TUnit, _, _)), RT.Base (TUnit, _, _)), ECont, _ ->
      if dbg then Debug.printf "%s@.@." __LOC__;
      uncps_ref_type sol typ_exn rtyp ENone etyp
  | ( RT.Fun
        ( _,
          RT.Fun (_, rtyp1, RT.Base (TUnit, _, _)),
          RT.Fun (_, RT.Fun (_, rtyp2, RT.Base (TUnit, _, _)), RT.Base (TUnit, _, _)) ),
      EExcep,
      _ ) ->
      if dbg then Debug.printf "%s@.@." __LOC__;
  ...
  | ( RT.Fun
        ( _,
          RT.Fun (_, rtyp1, RT.Base (TUnit, _, _)),
          RT.Fun (_, RT.Inter (_, rtyps), RT.Base (TUnit, _, _)) ),
      EExcep,
      _ ) ->
      if dbg then Debug.printf "%s@.@." __LOC__;
      let rtyp1' = uncps_ref_type sol typ_exn rtyp1 ENone etyp in
      ...
      in
      let styp' =
        match List.map aux rtyps with [] -> typ_of_etyp etyp | rtyp' :: _ -> RT.to_simple rtyp'
      in
  ...
  | RT.Tuple xrtyps, _, TTupleCPS etyps ->
      if dbg then Debug.printf "%s@.@." __LOC__;
      RT.Tuple
        (List.map2 (fun (x, rtyp) etyp -> (x, uncps_ref_type sol typ_exn rtyp e etyp)) xrtyps etyps)
  | RT.ExtArg (x, rtyp1, rtyp2), _, _ ->
      if dbg then Debug.printf "%s@.@." __LOC__;
      RT.ExtArg (x, rtyp1, uncps_ref_type sol typ_exn rtyp2 e etyp)
  | _, _, TBaseCPS styp when RT.is_top' rtyp -> RT.top styp
  | _, _, TBaseCPS styp when RT.is_bottom' rtyp -> RT.bottom styp
  (*
...
*)
  | RT.Fun (_x, RT.Inter (_, []), _rtyp'), _, _ -> assert false
  | RT.Fun (x, RT.Inter (_, rtyps), rtyp'), _, _ ->
      let rtyps =
        List.map (fun rtyp -> uncps_ref_type sol typ_exn (RT.Fun (x, rtyp, rtyp')) e etyp) rtyps
      in
...
let infer_effect env t =
  ...
  if List.length ext_funs <> List.length (List.unique ~eq:Id.eq ext_funs) then (
    ...
    ext_funs |> List.sort compare |> List.iter (Format.eprintf "  %a@." Print.id_typ);
    unsupported "CPS: polymorphic use of external functions");
  let tenv =
    List.map (Pair.make Id.to_string (Id.typ |- infer_effect_typ env |- force_cont)) ext_funs
  in
...
let make_get_rtyp sol typ_exn typed get_rtyp f =
  ...
  if !!Flag.Debug.print_ref_typ then
    Format.eprintf "CPS ref_typ: %a: @[@[%a@]@ ==>@ @[%a@]@]@." Id.print f RT.print rtyp RT.print
      rtyp';
  rtyp'
...
let exists_let_desc desc =
  match desc with Local (Decl_let _, _) -> true | _ -> exists_let.desc_rec desc
...
let reduce_fail =
  ...
  tr.typ <- Fun.id;
  fun t -> if Flag.(!mode <> Reachability) then t else tr.term t

let has_typ_result =
  let col = Col.make false ( || ) in
  let col_typ typ = if typ = typ_result then true else col.typ_rec typ in
  col.typ <- col_typ;
  col.typ
...
let trans_force_typ typ_excep typ =
  infer_effect_typ !!initial_env typ |> force_cont |> trans_typ initial_sol typ_excep typ
...
let rec trans_ref_typ is_CPS typ =
  ...
  | Inter (_, typs) ->
      ...
      let styp' = match typs' with [] -> typ_unknown | typ' :: _ -> RT.to_simple typ' in
      Inter (styp', typs')
  | Union (_, typs) ->
      let typs' = List.map (trans_ref_typ is_CPS) typs in
      let styp' = match typs' with [] -> typ_unknown | typ' :: _ -> RT.to_simple typ' in
...
let trans ({ Problem.term = t; spec; attr } as problem) =
  ...
  let term =
    ...
    let h =
      let e = Id.new_var ~name:"e" typ_excep' in
      if !Flag.Method.only_specified then Term.fun_ e cps_result
      else Term.(fun_ e (fail_term_cps @ [ unit; k ]))
    in
    let reduce_trivial_branch =
      if !Flag.Method.modular then Fun.id else Trans.reduce_trivial_branch
    in
    (* WORKAROUND *)
...
let trans_as_direct t =
  t |> Problem.safety |> trans |> Pair.map_fst Problem.term |> Pair.map_fst Trans.direct_from_CPS
