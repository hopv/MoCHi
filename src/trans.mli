...
val copy_poly_values : term -> term * ((id -> Ref_type.t) -> id -> Ref_type.t)
...
val set_main_for : ?force:bool -> lid list -> term -> term
...
val lift_type_decl : ?decl_ext:(Type.constr * (typ list * typ)) list -> term -> term (** also remove type extension *)
val mark_free_as_external : term -> term
val map_var : (id -> id) -> term -> term
val split_mutual_rec : ?only_top:bool -> term -> term
val get_set_main : term -> lid option
...
val abst_format_const : term -> term
...
val elim_unused_let : ?leave_last:bool -> ?cbv:bool -> ?elim_may_div:bool -> term -> term (** does not support modules *)
...
val set_assert_ref_typ : ?catch_all:(term -> term) -> id -> Ref_type.t -> term -> (id * Ref_type.t) list * term
...
val trans_match_with_recover_pat_bv : tr_desc_rec:(desc -> desc) -> tr_typ:(typ -> typ) -> term -> term
...
(** {1 Utilities} *)
val recover_pat_bv : tr_typ:(typ -> typ) -> before:pattern * 'a -> after:pattern * term -> pattern * term
