...
let get_rtyp_lift t f rtyp =
  let rec aux rtyp typ =
    ...
    | RT.Fun (x, rtyp1, rtyp2) ->
        if RT.arg_num rtyp = arg_num typ then rtyp
        else
          let rtyp' = aux rtyp2 typ in
          if RT.occur x rtyp' then RT.ExtArg (x, rtyp1, rtyp') else rtyp'
    | _ -> assert false
  in
...
let get_rtyp_lift t f rtyp =
  ...
  if !!Flag.Debug.print_ref_typ then
    Format.fprintf !Flag.Print.target "LIFT: %a: @[@[%a@]@ ==>@ @[%a@]@]@." Id.print f RT.print rtyp
      RT.print rtyp';
  rtyp'
...
module Id' = struct
  ...
  let compare x y =
    let dbg = 0 = 1 in
    let count_pred x =
      match get_tapred @@ Id.typ x with None -> 0 | Some (_, ps) -> List.length ps
    in
    let aux z = (not @@ is_base_typ @@ Id.typ z, count_pred z, Id.to_string z) in
...
let rec lift_aux post xs t =
  let defs, desc =
        ...
        let fs =
          let aux (f, _) =
            let tfun x ty =
              let x' =
                let has_no_pred =
                  List.for_all (function TARefPred (_, _) -> false | _ -> true) -| assoc_tattr
                in
                if !Flag.PredAbst.shift_pred && is_base_var x && has_no_pred (Id.typ x) then
            ...
            (f, (f', Term.(var f' @@ vars fv)))
          in
          List.map aux bindings
        in
        let subst_f t =
          List.fold_left2 (fun t (_, (_, f'')) (f, _) -> subst f f'' t) t fs bindings
        in
        let aux (f, t1) =
    ...
    | _ ->
        Format.eprintf "lift: %a@." Print.term t;
        assert false
  in
  (defs, make desc t.typ)

let lift ?(args = []) t = (lift_aux "" (set_of_list args) t, get_rtyp_lift t)
