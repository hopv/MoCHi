...
let rec constraints_term env_orig env t typ =
  ...
  | App
      ( App
          ( Const (And | Or | Lt | Gt | Leq | Geq | EqUnit | EqBool | EqInt | Add | Sub | Mul | Div),
            t1 ),
        t2 ) ->
...
let print_constraint (typ1, typ2) =
  Format.fprintf !Flag.Print.target "%a <: %a@." print_typ typ1 print_typ typ2
    ...
    | TVar x -> (
        ...
        with Not_found -> (
          try
            ...
            let constrs' =
              List.filter (function TVar y, _ -> List.mem y (x :: xs) | _ -> false) constrs
            in
            let constrs'' =
              List.filter (function _, TVar y -> not (List.mem y (x :: xs)) | _ -> true) constrs'
            in
...
let infer { env; defs; main } =
  ...
  let constrs =
    (List.assoc main env', TBase) :: List.rev_flatten_map (constraints_def env env') defs
  in
...
let elim_def env { fn = f; args = xs; cond = t1; body = t2 } =
  let f_typ = List.assoc f env in
  if snd (decomp_tfun f_typ) = TTop then None
  else
    let use = use_of_typ f_typ in
    let xs' = List.map (List.nth xs) use in
    let rec get_arg_env typ xs =
    ...
    in
    let env' = get_arg_env f_typ xs @@@ env in
    Some { fn = f; args = xs'; cond = elim_term env' t1; body = elim_term env' t2 }

(** call-by-name *)
let elim { env; defs; main; info } =
  let env' = infer { env; defs; main; info } in
  let defs' = List.filter_map (elim_def env') defs in
  Format.fprintf !Flag.Print.target "BEFORE:@.%a@.@.%a@.AFTER:@.%a@." CEGAR_print.prog
    { env; defs; main; info } print_env env' CEGAR_print.prog { env; defs = defs'; main; info };
