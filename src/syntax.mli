...
and id = typ Id.t
and lid = LId of id | LDot of lid * string | LApp of lid * lid
...
and pat_desc =
  ...
  | PEval of id * term * pattern
      (** (match v with PEval(x,t,p) -> t2) is reduced t2' if
          (match [v/x]t with p -> [v/x]t2) is reduced t2' *)
...
val pp_attr_info_ref : (Format.formatter -> attr_info -> unit) ref
val pp_attr_info : Format.formatter -> attr_info -> unit
val update_pp_attr_info : (Format.formatter -> attr_info -> bool) -> unit
val _LId : id -> lid
...
module Tr : sig
  type t = {
    ...
    mutable var : id -> id;
    mutable lid : lid -> lid;
    mutable path : Type.path -> Type.path;
...
module Tr2 : sig
  type 'a t = {
    ...
    mutable path : 'a -> Type.path -> Type.path;
...
module Tr_col2 : sig
  type ('a, 'b) t = {
    ...
    mutable path : 'b -> Type.path -> 'a * Type.path;
...
module Fold_tr : sig
  type 'a t = {
    ...
    mutable path : 'a -> Type.path -> 'a * Type.path;
...
val get_fv_gen : (typ -> id list) -> term -> id list
...
val set_desc : term -> desc -> term [@@alert unsafe "This function must be used carefully"]
val set_attr : term -> attr list -> term [@@alert unsafe "This function must be used carefully"]

val set_pat_desc_raw : pattern -> pat_desc -> pattern
[@@alert unsafe "This function must be used carefully"]
