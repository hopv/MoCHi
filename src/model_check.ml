...
type mc = TRecS | HorSat | HorSat2 | HorSatP [@@deriving show]
...
let available mc =
  let path =
    ...
    match mc with TRecS -> !trecs | HorSat -> !horsat | HorSat2 -> !horsat2 | HorSatP -> !horsatp
  in
  path <> None
...
let use mc =
  ...
  current_mc := Some mc;
  match mc with
  | TRecS -> mc_safety := (module TrecsInterface : MC_safety_interface)
  | HorSat ->
...
let init () =
  if available HorSat2 then use HorSat2 else if available HorSat then use HorSat else use TRecS

let capitalize_var = String.capitalize_ascii
let uncapitalize_var = String.uncapitalize_ascii
...
let capitalize { env; defs; main; info } =
  ...
  let aux { fn = f; args = xs; cond; body } =
    ...
    { fn = capitalize_var f; args = xs'; cond = sbst1 @@ sbst2 cond; body = sbst1 @@ sbst2 body }
  in
...
let make_bottom { env; defs; main; info } =
  ...
  let aux_def { fn = f; args = xs; cond; body = t2 } =
    ...
    let rec aux_term t typ =
      ...
      | App (App (App (Const If, t1), t2), t3), typ ->
          ...
          let aux_term' t typ = try aux_term t typ with TypeBottom -> make_bottom 0 in
      ...
      | App (t1, t2), _ ->
          ...
          let typ' = match typ with TFun (typ, _) -> typ | _ -> assert false in
          App (aux_term t1 typ, aux_term t2 typ')
      | Let _, _ -> assert false
      | Fun _, _ -> assert false
    in
    let app_typ x = function TFun (_, typ2) -> typ2 (Var x) | _ -> assert false in
  ...
  let bottom_defs =
    let make (x, n) =
      { fn = x; args = List.init n @@ Fun.const "x"; cond = Const True; body = bot0 }
    in
...
let eta_expand prog =
  { prog with defs = List.map (eta_expand_def prog.env) prog.defs } |> CEGAR_lift.lift2
...
let should_reduce { fn = f; cond } env defs =
  ...
  && List.length (List.rev_flatten_map (fun { body } -> List.filter (( = ) f) (get_fv body)) defs)
     = 1
...
let rec beta_reduce_term flag ({ fn = f; args = xs; body = t2 } as def) = function
  ...
  | App _ as t ->
      ...
      if t1 = Var f then
        if
          List.for_all (function Const _ | Var _ -> true | App _ -> false | _ -> assert false) ts'
...
let beta_reduce_term flag ({ fn = f } as def) t =
  ...
  if n = 1 then beta_reduce_term flag def t
  else (
...
let beta_reduce_aux { env; defs; main; info } =
  let rec aux defs1 = function
    ...
    | def :: defs2 when should_reduce def env (defs1 @@@ (def :: defs2)) ->
        ...
        if !flag then aux (defs1' @ [ def ]) defs2' else aux defs1' defs2'
...
let rec beta_reduce prog =
  ...
  if prog.defs = prog'.defs then prog else beta_reduce prog'
...
let check abst prog spec =
  ...
  let module MC_fair_nontermination =
    (val !mc_fair_nontermination : MC_fair_nontermination_interface)
  in
  ...
  let abst' = if List.mem ACPS prog.info.attr then preprocess_cps abst else preprocess abst in
  let result =
    match spec with
    | Safety ->
    ...
    | NonTermination ->
        let labels =
          List.map make_randint_label @@ List.filter_map (decomp_randint_name -| fst) prog.env
        in
...
let check abst prog spec = Time.measure_and_add Flag.Log.Time.mc (check abst prog) spec
