...
let init_env = !!Tenv.init

let rec is_gadt env ty =
  match ty with
  | TVariant (_, rows) -> List.exists (row_ret |- Option.is_some) rows
  | TConstr (c, _) -> (
      match Tenv.assoc_type_opt c env with
      | Some (_, (_, ty')) when ty <> ty' (* ??? *) -> is_gadt env ty'
      | _ -> false)
  | _ -> false

let use_gadt env = Type.make_find (fun _ -> is_gadt env) |- Option.is_some
let set_abst_flag () = Flag.Abstract.over "GADT"

let abst_term, abst_typ =
  let tr = Tr2.make () in
  tr.term <-
    (fun ((env, recs) as arg) t ->
      match t.desc with
      | Fun _ when use_gadt env t.typ ->
          set_abst_flag ();
          let fv = get_fv t |> List.filter_out (Id.List.mem -$- recs) |> List.map (tr.var arg) in
          tr.typ arg t.typ |> Term.rand |> List.fold_right make_use_var fv |> set_afv_shallowly
      | Local (Decl_type decls, _) ->
          t |> tr.term_rec (Tenv.add_typ_decl decls env, recs) |> set_afv_shallowly
      | Match _ ->
          Trans.trans_match_with_recover_pat_bv ~tr_desc_rec:(tr.desc_rec arg) ~tr_typ:(tr.typ arg)
            t
...
let abstract problem =
  ...
  let env =
    List.fold_right
      (fun (x, ty) env -> Tenv.add_module (Id.set_typ x ty) ty ~env)
      problem.ext_mod_typ init_env
  in
  let term = abst_term ~env problem.term in
  let ext_mod_typ = List.map (Pair.map_snd (abst_typ ~env)) problem.ext_mod_typ in
