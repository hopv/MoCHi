...
let print ppf n =
  if !Flag.PrettyPrinter.color || !Flag.PrettyPrinter.color_always then
    Format.pp_print_as ppf 0 @@ "\027[" ^ string_of_int n ^ "m"
...
let init () = if Unix.isatty Unix.stdout then () else Flag.PrettyPrinter.color := false
let () = init ()
