...
let rec inv_abst_type aty =
  match aty with
  | F.AbsType.Base(F.TypConst.Ext("X"), _, _) ->
      TBase(TAbst("X"), fun _ -> [])
...
let insert_extra_param t =
  ...
  let rec aux rfs bvs exs t =
    let desc =
      ...
      | S.Local(S.Open m, t2) -> S.Local(S.Open m, aux rfs bvs exs t2)
      ...
      | S.Module _decls -> unsupported "insert_extra_param Module"
      | S.MemSet _ -> unsupported __LOC__
      | S.AddSet _ -> unsupported __LOC__
      | S.Subset _ -> unsupported __LOC__
    in
    {t with S.desc}
  in
  let res = aux [] [] [] t in
