...
let rec conv_typ ty =
  ...
  | TBase (TAbst s, _) -> F.Type.mk_const (F.TypConst.Ext s)
  | TFun (ty1, tmp) ->
...
let conv_const c : F.Const.t =
  ...
  | CPS_result -> F.(UFun (Type.mk_const (F.TypConst.Ext "X"), Idnt.make "end"))
  | Proj _ -> assert false
...
let conv_var x =
  if F.RefTypInfer.is_parameter x || is_extra_coeff_name x then F.Idnt.mk_coeff x else F.Idnt.make x
...
let conv_event e t =
  ...
  let t' =
    if e <> "fail" && Flag.(List.mem !mode [ FairTermination; FairNonTermination ]) then t
    else F.Term.mk_const Unit
  in
  F.Term.(mk_app (mk_const (F.Const.Event e)) [ t' ])

let rec conv_term env t =
  match t with
  | App (App (App (Const If, t1), Const True), Const False) -> conv_term env t1
  ...
  | Const c -> F.Term.mk_const (conv_const c)
  | Var x -> F.Term.mk_var @@ conv_var x
...
let rec of_term t : F.Term.t =
  ...
  | App ({ desc = Var (LId p) }, ts) when Term_util.PredVar.is_pvar p ->
      (* for predicate variables *)
      let ts' =
        ts |> List.map @@ Pair.add_right @@ (of_typ -| S.typ) |> List.map @@ Pair.map_fst of_term
      in
...
let inv_const (c : F.Const.t) =
  ...
  | Eq ty when F.Type.is_ext ty -> F.Type.let_ext ty (fun typ -> CmpPoly (typ, "="))
  | Neq ty when F.Type.is_ext ty -> F.Type.let_ext ty (fun typ -> CmpPoly (typ, "<>"))
  | Lt ty when F.Type.is_ext ty -> F.Type.let_ext ty (fun typ -> CmpPoly (typ, "<"))
  | Gt ty when F.Type.is_ext ty -> F.Type.let_ext ty (fun typ -> CmpPoly (typ, ">"))
  | Leq ty when F.Type.is_ext ty -> F.Type.let_ext ty (fun typ -> CmpPoly (typ, "<="))
  | Geq ty when F.Type.is_ext ty -> F.Type.let_ext ty (fun typ -> CmpPoly (typ, ">="))
  | String s -> String s
  | Real r -> Float r
  | UFun (ty, x)
    when F.Idnt.string_of x = "end" && F.Type.is_ext ty && F.Type.let_ext ty (( = ) "X") ->
      CPS_result
  | Iff -> EqBool
  | Mod -> Mod
  ...
  | _ ->
      Format.eprintf "%s@." (F.Const.string_of c);
      assert false
...
let inv_fdef (fdef : F.Fdef.t) =
  (fdef.name, fdef.args, inv_formula fdef.guard, [], inv_term fdef.body)
...
let conv_prog (prog : prog) : F.Prog.t =
  ...
  {
    fdefs = List.map (conv_fdef typs) fdefs;
    types = List.map (fun (x, ty) -> (F.Idnt.make x, conv_typ ty)) typs;
    main;
  }

let rec inv_abst_type (aty : F.AbsType.t) =
  match aty with
  | Base (Ext "X", _, _) -> TBase (TAbst "X", fun _ -> [])
  | Base (b, x, ts) ->
  ...
  | Fun (aty1, aty2) ->
      let x =
        if F.AbsType.is_base aty1 then F.Idnt.string_of (F.AbsType.bv_of aty1) else "_dummy"
      in
      TFun (inv_abst_type aty1, subst_typ x -$- inv_abst_type aty2)

let is_cp prog = prog |> conv_prog |> F.RefTypInfer.is_cut_point

let make_def_randint prog f =
  if CEGAR_syntax.is_randint_var f then []
  else
    let argss =
      List.filter_map (fun { fn = g; args } -> if f = g then Some args else None) prog.defs
    in
...
let trans fair_cond cexs prog =
  ...
  let defs =
    if fair_cond then
      (* TODO: ad-hoc fix, remove after Fpat is fixed *)
...
let infer_with_ext (labeled : string list) (is_cp : F.Idnt.t -> bool) (cexs : int list list)
    (ext_cexs : (F.Idnt.t * F.Pred.t list) list list) (prog : CEGAR_syntax.prog) =
  ...
  let pr ppf (tenv, phi) = Verbose.fprintf ppf "(%a).%a" F.TypEnv.pr tenv F.Formula.pr phi in
  Verbose.printf "ext_cexs %a@."
    (Util.List.print
    @@ Util.List.print (fun fm (x, p) ->
           Format.fprintf fm "%a, %a" F.Idnt.pr x (Util.List.print pr) p))
    ext_cexs;
  ...
  F.HCCSSolver.link_dyn
    (fst -| fst -| F.AEHCCSSolver.solve (F.EAHCCSSolver.solve [] [] [] F.BwIPHCCSSolver.solve));
    ...
    List.flatten
      (List.map
         (fun x ->
           let x' = trans_id x in
           (match x'.Id.typ with
           | Type.TFun (_, _) | Type.TTuple _ (* ToDo: fix it *) ->
               F.Util.List.unfold
                 (fun i ->
    ...
    let desc : Syntax.desc =
      ...
      | Fun (y, t1) ->
          ...
          let rfs =
            match rfs with
            | [] -> assert false
            | (f, xxs, recursive) :: rfs' -> (f, xxs @ [ (y', ys) ], recursive) :: rfs'
          in
          let f, _ =
            List.fold_left
              (fun (f, ty) y ->
                ( (fun t -> f (S.make (S.Fun (y, t)) ty)),
                  match ty with Type.TFun (_, ty') -> ty' | _ -> assert false ))
              ((fun t -> t), trans_type t.S.typ)
              ys'
          in
          let bvs, exs = ((if true then bvs @ ys' else bvs @ [ y' ]), exs @ ys) in
      ...
      | App (t1, ts) ->
          ...
          let recursive, xss =
            ...
            | Var (LId f) -> (
                try
                  let _, xxss, _ =
                    List.find (fun (f', _, recursive) -> recursive && Id.(f' = f)) rfs
                  in
          ...
          App (t1', ts'')
      | If (t1, t2, t3) -> If (aux rfs bvs exs t1, aux rfs bvs exs t2, aux rfs bvs exs t3)
      | Local (Decl_type decls, t2) -> Local (Decl_type decls, aux rfs bvs exs t2)
      | Local (Type_ext (Ext_type (s, labels)), t2) ->
          Local (Type_ext (Ext_type (s, labels)), aux rfs bvs exs t2)
      | Local (Type_ext (Ext_rebind (s1, s2)), t2) ->
          Local (Type_ext (Ext_rebind (s1, s2)), aux rfs bvs exs t2)
      | Local (Decl_let bindings, t2) ->
          let bvs' = bvs @ List.map fst bindings in
          let aux' (f, t) =
            let f' = trans_id f in
          ...
          Local (Decl_let bindings', aux rfs (bvs @ List.map fst bindings') exs t2)
      ...
      | Nil -> Nil
      | Cons (t1, t2) -> Cons (aux rfs bvs exs t1, aux rfs bvs exs t2)
      | Constr (b, s, ts) -> Constr (b, s, List.map (aux rfs bvs exs) ts)
      | Match (t1, pats) ->
          let aux' (pat, t) =
            (* ToDo: need to update pat!? *)
            (pat, aux rfs (bvs @ S.get_bv_pat pat) exs t)
          in
      ...
      | SetRef (t1, t2) -> SetRef (aux rfs bvs exs t1, aux rfs bvs exs t2)
      | TNone -> TNone
...
let instantiate_param prog =
  ...
  if !F.RefTypInfer.prev_sol = [] then F.RefTypInfer.init_sol (conv_prog prog);
  let map = List.map (fun (x, t) -> (F.Idnt.string_of x, inv_term t)) !F.RefTypInfer.prev_sol in
  let res =
    ( typs,
      List.map
...
let simplify_term p = S.make (simplify_term p.S.desc) p.typ
...
let conv_pred (env : CEGAR_syntax.env) (p : CEGAR_syntax.t) =
  let env = env |> List.filter (is_base -| snd) |> List.map (F.Pair.map F.Idnt.make conv_typ) in
  let phi = conv_formula p in
  ((env, phi) : F.Pred.t)

let trans_ext (renv : (int * CEGAR_syntax.env) list)
    (map : (int * (CEGAR_syntax.t -> CEGAR_syntax.t list)) list) (n, bs) =
...
let parse_arg arg =
  ...
  try Arg.parse_argv ~current:(ref 0) args (Arg.align F.FPATConfig.arg_spec) ignore usage
