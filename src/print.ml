...
module Debug_attr = Debug.Make (struct
  let check = Flag.Debug.make_check (__MODULE__ ^ ".attr")
end)
...
let set_unused () = config_default := { !config_default with unused = true }
let set_depth depth = config_default := { !config_default with depth }
...
let is_definitions desc = (snd @@ decomp_locals (make_raw desc Ty.unit)).desc = End_of_definitions
let print_location fm { Location.loc_start } = Lexing.print_position fm loc_start
let print_dots fm = Color.s_yellow fm "..."

let rec print_typ fm t =
  Print_typ.typ ~occur:occur_typ (print_term { !config_default with ty = false } 0) fm t
  ...
  if xs <> [] then
    let xs' =
      ...
      | Some fv ->
          xs
          |> List.map (fun x ->
                 if Id.List.mem x fv then x
                 else if Id.typ x = TBase TUnit then Id.make "()" (TBase TUnit)
                 else Id.make "_" @@ Id.typ x)
    in
...
and print_id_typ cfg fm x =
  fprintf fm "(@[%a:%a@])" (print_id cfg) x (Color.cyan print_typ) (Id.typ x)

and print_lid cfg fm (lid : lid) = Lid.print_gen (print_id cfg) fm lid

and print_lid_typ cfg fm x =
  fprintf fm "(@[%a:%a@])" (print_lid cfg) x (Color.cyan print_typ) (Lid.typ x)
...
and paren_if b = if b then ("(", ")") else ("", "")
and paren pri p = paren_if (pri >= p)
...
and print_termlist cfg pri fm ts = print_list ~limit:cfg.length (print_term cfg pri) "@ " fm ts
...
and print_const cfg fm c =
  ...
  | Dummy _ when false -> fprintf fm "dummy"
  | Dummy ty -> fprintf fm "dummy[%a]" print_typ ty

and print_attr _ fm = function
  ...
  | AInfo info -> fprintf fm "@%a" pp_attr_info info
  | AFV fv -> fprintf fm "AFV [@[%a@]]" (print_list Id.print ",@ ") fv
  | ATarget -> fprintf fm "ATarget"

and print_attr_list cfg fm attrs = List.print (print_attr cfg) fm attrs

and is_ignore_attr a =
  match a with ADoNotInline -> true | AFV _ -> true | ALoc _ -> true | _ -> List.mem a const_attr

and decomp_comment a = match a with AComment s -> Some s | _ -> None

and filter_attr_list attr =
  if !!Debug_attr.check then attr
  else
    attr
...
and print_term cfg pri fm t =
  if cfg.depth = 0 then print_dots fm
  else
    let cfg' = { cfg with depth = cfg.depth - 1 } in
    let pr attr fm desc =
      let comments = List.filter_map decomp_comment t.attr in
      if comments = [] || !!Debug_attr.check then
        fprintf fm "@[%a@]" (print_desc cfg' pri attr) desc
      else
        fprintf fm "(@[(* @[%a@] *)@ %a@])" (print_list pp_print_string ", ") comments
          (print_desc cfg' pri attr) desc
    in
    ...
    if attr = [] || cfg.as_ocaml then pr t.attr fm t.desc
    else
      fprintf fm "(@[%a@ %a#@ %a%t@])" (pr t.attr) t.desc Color.set Color.Yellow
        (print_attr_list cfg') attr Color.reset;
    if marked then
...
and print_let_decls cfg bang fm bindings =
  ...
  let is_module =
    List.exists (fun (_, t) -> is_module t || is_functor ~default:false [] t) bindings
  in
  let print_binding fm (f, t1) =
    let pre =
      let s_rec = if is_module then " module" else if non_rec then "" else " rec" in
      if !first then "let" ^ (if bang then "!" else "") ^ s_rec else "and"
    in
    ...
    fprintf fm "@[<hov 2>%s @[<hov 2>%a%a%t@] =@ %a@]" pre (print_id cfg) f (print_ids ~fv cfg) xs
      pr_ty (print_term cfg 0) t1';
    first := false
  in
...
and print_type_decls cfg fm defs =
  ...
  let pr_decl fm (c, (params, ty)) =
    ...
    b := false
  in
  let open_box = Format.pp_open_vbox -$- 0 in
  print_list ~open_box ~limit:cfg.length pr_decl "@ " fm defs
...
and print_decl cfg bang fm decl =
  ...
  | Type_ext (Ext_type (s, (params, row))) ->
      ...
      if C.is_exn s then Format.fprintf fm "@[<hov 2>exception %a@]" print_typ ty
      else
        Format.fprintf fm "@[<hov 2>type %a%a +=@ %a@]" print_params params Print_typ.path s
          print_typ ty
  | Type_ext (Ext_rebind (s1, s2)) ->
      Format.fprintf fm "@[<hov 2>exception %a =@ %a@]" Print_typ.constr s1 Print_typ.path s2
  | Include t -> Format.fprintf fm "@[<hov 2>include@ %a@]" (print_term cfg 0) t
...
and print_desc cfg pri attr fm desc =
  ...
  let pr_t' ?(cfg = cfg) pri fm t =
    (* for printing subterms of [Local] *)
    match t.desc with
  ...
  | Fun (x, t1) ->
      let xs, t = if !!Debug.check then ([ x ], t1) else decomp_funs (make_raw desc typ_unknown) in
      let fv = get_fv t in
      let cfg = update_omit_id ~bv:xs ~fv:Id.List.Set.(fv - xs) cfg in
      let p = 15 in
      let s1, s2 = paren pri (p + 1) in
      fprintf fm "%s@[<hov 2>fun@[%a@] ->@ %a%s@]" s1
        (print_ids ~fv { cfg with top = false })
        xs (pr_t ~cfg 0) t s2
  | App (_, []) -> assert false
  ...
  | App (t, ts) ->
      let p = 80 in
      let s1, s2 = paren pri p in
      fprintf fm "@[<hov 2>%s%a@ %a%s@]" s1 (pr_t p) t
        (print_termlist { cfg with top = false } p)
        ts s2
  ...
  | If (t1, t2, t3) ->
      ...
      pp_print_string fm "  ";
      let s21, s22 = paren_if (is_seq t2) in
      fprintf fm "%s@[%a@]%s" s21 (pr_t p) t2 s22;
      if t3.desc <> Const Unit then (
        let s31, s32 = paren_if (is_seq t3) in
        fprintf fm "@ else@ ";
        pp_print_if_newline fm ();
        pp_print_string fm "  ";
        fprintf fm "%s@[%a@]%s" s31 (pr_t p) t3 s32);
      fprintf fm "@]%s" s2
  | Local (Decl_let [], t) when !Flag.Debug.minimize -> pr_t pri fm t
  ...
  | Local
      ( Decl_let [ (_, { desc = App ({ desc = Event ("fail", _) }, [ { desc = Const Unit } ]) }) ],
        { desc = Bottom } ) ->
      let p = 80 in
      let s1, s2 = paren pri p in
      fprintf fm "@[%sassert@ false%s@]" s1 s2
  | Local (Decl_let [ (u, t1) ], t2)
    when (not !!Debug.check)
         && Id.typ u = TBase TUnit
         && (not @@ Id.List.mem u @@ get_fv t2)
         && t2.desc <> End_of_definitions ->
      let p = 9 in
      let s1, s2 = paren pri p in
  ...
  | Local (decl, t2) ->
      ...
      fprintf fm "%s@[<hov>@[<hv>%a@]@ in@ @[<hov>%a@]@]%s" s1
        (print_decl { cfg with top = false } bang)
        decl (pr_t' ~cfg p) t2 s2
  ...
  | BinOp
      ( (Eq | Leq | Geq | Lt | Gt),
        { desc = App ({ desc = Const (Rand (TBase TInt, false)) }, [ { desc = Const Unit } ]) },
        { desc = Const _ } )
  | BinOp
      ( (Eq | Leq | Geq | Lt | Gt),
        { desc = Const _ },
    ...
    when not !!Debug.check ->
      let p = 80 in
      let s1, s2 = paren pri p in
      if cfg.as_ocaml then fprintf fm "%sRandom.bool@ ()%s" s1 s2
      else fprintf fm "%srand_bool@ ()%s" s1 s2
  ...
  | Record fields ->
      let pr fm (s, t) =
        match t with
        | { desc = Var (LId x) } when Id.to_string x = Id.to_string s ->
            fprintf fm "%a" Print_typ.label s
        | _ -> fprintf fm "%a=%a" Print_typ.label s (pr_t 0) t
      in
      fprintf fm "{%a}" (print_list ~limit:cfg.length pr ";@ ") fields
  | Field (t, s) ->
      let p = 90 in
      let s1, s2 = paren pri p in
  ...
  | Cons (t1, t2) -> (
      ...
      | None ->
          ...
          let s1, s2 = paren pri p in
          fprintf fm "%s@[%a::@,%a@]%s" s1 (pr_t p) t1 (pr_t p) t2 s2
      | Some ts' -> Format.fprintf fm "%a" (List.print @@ pr_t 0) ts')
  | Constr (b, s, ts) ->
      let p = 80 in
      let s1, s2 = paren pri p in
      let quot = if b then "`" else "" in
      if ts = [] then fprintf fm "%s%a" quot Print_typ.path s
      else
        fprintf fm "%s@[%s%a(%a)@]%s" s1 quot Print_typ.path s
          (print_list ~limit:cfg.length (pr_t 20) ",")
          ts s2
  | Match (_t1, [ ({ pat_desc = PTuple _ps }, _t2) ]) when false -> assert false
  | Match (t, pats) ->
      let pats' =
        if cfg.as_ocaml then
          match (fst (List.last pats)).pat_desc with
          | PAny | PVar _ -> pats
          | _ -> pats @ [ (make_pattern PAny typ_unknown, make_raw Bottom typ_unknown) ]
        else pats
      in
      let p = 10 in
      let s1, s2 = paren pri p in
  ...
  | TryWith (t1, { desc = Fun (e, { desc = Match ({ desc = Var (LId e') }, pats) }) })
    when Id.(e = e')
         && (function
              | { pat_desc = PAny }, { desc = Raise { desc = Var (LId e'') } } -> Id.same e e''
              | _ -> false)
            @@ List.last pats ->
      let p = 10 in
      let s1, s2 = paren pri (p + 1) in
  ...
  | Proj (i, t) when cfg.as_ocaml ->
      ...
      let s =
        "fun ("
        ^ String.join ","
            (List.init (Option.get @@ tuple_num t.typ) (fun j -> if i = j then "x" else "_"))
        ^ ") -> x"
      in
  ...
  | Module decls ->
      ...
      fprintf fm "%s@[<v>struct@\n@[%a@]@\nend@]%s" s1
        (print_decls ~indent:"  " { cfg with top = true })
        decls s2
  | Pack t -> fprintf fm "(module %a : %a)" (pr_t 80) t print_typ t.typ
  | Unpack t -> fprintf fm "(val %a)" (pr_t 80) t
  | MemSet (t1, t2) ->
      let p = 50 in
      let s1, s2 = paren pri (p + 1) in
      fprintf fm "%s@[%a ∈ %a@]%s" s1 (pr_t p) t1 (pr_t p) t2 s2
  | AddSet (t1, { desc = Const Empty }) -> fprintf fm "@[{%a}@]" (pr_t 0) t1
  | AddSet (t1, t2) ->
      let p = 60 in
      let s1, s2 = paren pri (p + 1) in
      fprintf fm "%s@[{%a} ∪ %a@]%s" s1 (pr_t p) t1 (pr_t p) t2 s2
  ...
  | Forall (x, t1) ->
      let xs, t =
        if !!Debug.check then ([ x ], t1) else decomp_forall (make_raw desc typ_unknown)
      in
      let fv = get_fv t in
      let cfg = update_omit_id ~bv:xs ~fv:Id.List.Set.(fv - xs) cfg in
      let p = 15 in
      let s1, s2 = paren pri (p + 1) in
      fprintf fm "%s@[<hov 2>forall @[%a@].@ %a%s@]" s1
        (print_ids ~fv { cfg with top = false })
        xs (pr_t ~cfg 0) t s2
  | Exists (x, t1) ->
      let xs, t =
        if !!Debug.check then ([ x ], t1) else decomp_exists (make_raw desc typ_unknown)
      in
      let fv = get_fv t in
      let cfg = update_omit_id ~bv:xs ~fv:Id.List.Set.(fv - xs) cfg in
      let p = 15 in
      let s1, s2 = paren pri (p + 1) in
      fprintf fm "%s@[<hov 2>exists @[%a@].@ %a%s@]" s1
        (print_ids ~fv { cfg with top = false })
        xs (pr_t ~cfg 0) t s2
...
and print_pattern cfg fm pat =
  ...
  | PConstr (b, c, pats) ->
      ...
      if pats <> [] then fprintf fm "(%a)" (print_list pr_p ",") pats
  | PNil -> fprintf fm "[]"
  ...
  | PWhen (p, cond) ->
      fprintf fm "@[<hov 4>@[<hov 2>%a@] when@ @[<hov 2>%a@]@]" pr_p p (print_term cfg 1) cond
  | PLazy p -> fprintf fm "(lazy %a)" pr_p p
  | PEval (x, t, p) ->
      fprintf fm "@[(%a@ eval %a@ with@ %a)@]" (print_id cfg) x (print_term cfg 1) t pr_p p

let print_term cfg fm = print_term cfg 0 fm

let update_config_for_local cfg t =
  match t.desc with Local _ -> { cfg with length = cfg.length - 1 } | _ -> cfg

(* TODO: merge with print_term *)
let rec print_term' cfg pri fm t =
  if cfg.depth = 0 || cfg.length = 0 then print_dots fm
  else
    let cfg' = { cfg with depth = cfg.depth - 1 } in
    let marked = List.exists (function AMark b -> !b | _ -> false) t.attr in
    if marked then
    ...
    | If (t1, t2, t3) ->
        let p = 1 in
        let s1, s2 = paren pri (p + 1) in
        fprintf fm "%s@[@[if %a@]@ then @[%a@]@ else @[%a@]@]%s" s1 (print_term' cfg' p) t1
          (print_term' cfg' p) t2 (print_term' cfg' p) t3 s2
    | Local (Decl_let bindings, t2) -> (
        let s_rec =
          if List.exists (is_module -| snd) bindings then " module"
        ...
        let b = ref true in
        let print_binding fm (f, t1) =
          let xs, t1' = if is_functor ~default:false [] t1 then ([], t1) else decomp_funs t1 in
          let pre = if !b then "let" ^ s_rec else "and" in
          fprintf fm "@[<hov 2>%s%a =@ %a@]" pre
            (print_ids { cfg with ty = true })
            (f :: xs) (print_term' cfg' p) t1';
          b := false
        in
        ...
        match t2.desc with
        | Local _ ->
            fprintf fm "%s@[<v>@[<hov 2>%a@]@ in@ %a@]%s" s1 print_bindings bindings
              (print_term' cfg' p) t2 s2
        | _ ->
            fprintf fm "%s@[<v>@[<hov 2>%a@]@ @[<v 2>in@ @]@[<hov>%a@]@]%s" s1 print_bindings
              bindings (print_term' cfg' p) t2 s2)
    | Local (Decl_type decls, t) ->
        let p = 10 in
        let s1, s2 = paren pri (p + 1) in
        let cfg' = update_config_for_local cfg t in
        fprintf fm "%s@[<v>@[<hv>%a@]@ in@ @[<hov>%a@]@]%s" s1 (print_type_decls cfg) decls
          (print_term' cfg' p) t s2
    | Local (Type_ext (Ext_type (s, (params, row))), t) ->
        ...
        let cfg' = update_config_for_local cfg t in
        if C.is_exn s then
          fprintf fm "%s@[<v>@[exception %a@]@ in@ @[<hov>%a@]@]%s" s1 print_typ ty
            (print_term' cfg' p) t s2
        else
          fprintf fm "%s@[<v>@[type %a%a +=@ %a@]@ in@ @[<hov>%a@]@]%s" s1 print_params params
            Print_typ.path s print_typ ty (print_term' cfg' p) t s2
    | Local (Type_ext (Ext_rebind (c1, c2)), t) ->
        let p = 10 in
        let s1, s2 = paren pri (p + 1) in
        let cfg' = update_config_for_local cfg t in
        fprintf fm "%s@[<v>@[exception %a =@ %a@]@ in@ @[<hov>%a@]@]%s" s1 Print_typ.constr c1
          Print_typ.path c2 (print_term' cfg' p) t s2
    | Local (Include t1, t2) ->
        let p = 10 in
        let s1, s2 = paren pri (p + 1) in
        fprintf fm "%s@[<hov 2>include@ %a@ in@ @[<hov>%a@]@]%s" s1 (print_term' cfg 0) t1
          (print_term' cfg p) t2 s2
    ...
    | BinOp (op, t1, t2) ->
        ...
        let s1, s2 = paren pri p in
        fprintf fm "%s%a %a %a%s" s1 (print_term' cfg' p) t1 print_binop op (print_term' cfg' p) t2
          s2
    | Not t ->
        let p = 6 in
        let s1, s2 = paren pri p in
    ...
    | SetField (t1, s, t2) ->
        fprintf fm "%a.%a <- %a" (print_term' cfg' 9) t1 Print_typ.label s (print_term' cfg' 3) t2
    | Nil -> fprintf fm "[]"
    | Cons (t1, t2) ->
        let p = 7 in
        let s1, s2 = paren pri (p + 1) in
    ...
    | Match (t, pats) ->
        ...
        let s1, s2 = paren pri (p + 1) in
        let aux (pat, t) =
          fprintf fm "@[<hov 4>%a -> %a@]@ " (print_pattern' cfg') pat (print_term' cfg' p) t
        in
    ...
    | Tuple [ t ] -> fprintf fm "@[(%a,)@]" (print_term' cfg' 0) t
    | Tuple ts -> fprintf fm "@[(%a)@]" (print_list ~limit:cfg.length (print_term' cfg' 0) ",@ ") ts
    | Proj (0, t) when tuple_num t.typ = Some 2 ->
        let p = 4 in
        let s1, s2 = paren pri (p + 1) in
    ...
    | Pack t -> fprintf fm "(module %a : %a)" (print_term' cfg' 1) t print_typ t.typ
    | Unpack t -> fprintf fm "(val %a)" (print_term' cfg' 1) t
    | MemSet (t1, t2) ->
        let p = 10 in
        let s1, s2 = paren pri (p + 1) in
        fprintf fm "@[%s%a ∈ %a%s@]" s1 (print_term' cfg' 1) t1 (print_term' cfg' 1) t2 s2
...
and print_declaration' cfg fm decls =
  (* TODO: fix *)
  let t =
...
and print_pattern' cfg fm pat =
  let rec pr fm pat =
    ...
    | PEval (x, t, p) ->
        fprintf fm "@[(%a@ eval %a@ with@ %a)@]" (print_id cfg) x (print_term' cfg 1) t pr p
  in
...
let print_defs fm (defs : (id * (id list * term)) list) =
  let print_fundef (f, (xs, t)) =
    fprintf fm "%a %a-> %a.\n" (print_id !config_default) f
      (print_ids { !config_default with ty = false })
      xs
      (print_term { !config_default with ty = false })
      t
  in
  List.iter print_fundef defs

let print_env fm { Tenv.menv; tenv; rebind } =
  let menv = List.map (Pair.map_snd snd) menv in
  let tenv = List.map (Pair.map_snd snd) tenv in
  let rebind = List.map (Pair.map_snd snd) rebind in
  let pr_decl fm (c, (ps, ty)) =
    Format.fprintf fm "%a%a = %a" print_params ps Print_typ.tconstr c print_typ ty
  in
  let pr_rebind fm (s1, s2) =
    Format.fprintf fm "exception %a = %a" Print_typ.tconstr s1 Print_typ.path s2
  in
  Format.fprintf fm "@[@[menv: %a;@]@ @[tenv: %a;@]@ @[rebind: %a;@]@]"
    (List.print (Pair.print Id.print print_typ))
    menv (List.print pr_decl) tenv (List.print pr_rebind) rebind
