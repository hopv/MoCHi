...
and print_attr fm = function
  ...
  | AInfo info -> fprintf fm "AInfo %a" print_info info
...
and print_term cfg pri fm t =
  if cfg.depth = 0 then
    ...
    if attr = [] || cfg.as_ocaml then
      ...
      fprintf fm "(@[%a@ #@ %a@])" (pr t.attr) t.desc print_attr_list attr;
    if marked then fprintf fm "@\n>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>%t@\n" Color.reset
...
and print_decl cfg bang fm decl =
  ...
  | Open m ->
      Format.fprintf fm "open %a" print_id m
...
and print_desc cfg pri attr fm desc =
  ...
  | Local(Decl_let [u,t1], t2) when not !!Debug.check && Id.typ u = TBase TUnit && not @@ Id.mem u @@ get_fv t2 && t2.desc <> End_of_definitions ->
      let p = 9 in
      let s1,s2 = paren pri p in
...
let rec print_term' pri fm t =
  ...
  fprintf fm "(@[";(
    ...
    | Local(Open m, t) ->
        let p = 10 in
        let s1,s2 = paren pri (p+1) in
        fprintf fm "%s@[open %a@\n@[<hov>%a@]@]%s" s1 print_id m (print_term' p) t s2
    | BinOp(op, t1, t2) ->
