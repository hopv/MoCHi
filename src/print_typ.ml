...
module Dbg_share = Debug.Make (struct
  let check = Flag.Debug.make_check (__MODULE__ ^ ".share")
end)
...
let remove_some_attr ty =
  ...
  | TAttr (attrs, ty) ->
      let attrs' =
        if !!Dbg_share.check then attrs
        else List.filter (function TAPredShare _ | TAId _ -> false | _ -> true) attrs
      in
      TAttr (attrs', ty)
  | _ -> ty
...
let rec print_attr cfg fm a =
  ...
  | TAExistential s -> Format.fprintf fm "TAExistential %s" s
  | TARec r -> Format.fprintf fm "TARec %a" (print cfg) (TVar r)

and print_tvar ?(weak = false) fm n =
  if n < 0 then Format.fprintf fm "'_for_debug"
  else
...
and print_decl_item cfg ~first ~last fm (c, (params, ty)) =
  ...
  Format.fprintf fm "@[<hov 2>%s %a%a = %a@]" header (print_params cfg) params print_constr c
    (print cfg) ty
[@@ocaml.warning "-27"]

and print_declaration ?(need_new_line = false) cfg fm decls =
...
and print_ext cfg fm (tc, (params, row)) =
  if C.is_exn tc then Format.fprintf fm "@[<hov 2>exception"
  else Format.fprintf fm "@[<hov 2>type %a%a +=" (print_params cfg) params print_path tc;
  let row =
    {
...
and print_signature cfg fm sgn =
  if sgn = empty_signature then Format.fprintf fm "sig end"
  else if !Flag.Print.signature then
    let pr = print cfg in
    let pr_elem fm = function
      | Sig_type decl -> print_declaration cfg fm [ decl ]
      | Sig_value x -> Format.fprintf fm "@[<hov 2>val %a : %a@]" Id.print x pr (Id.typ x)
      | Sig_ext ext -> print_ext cfg fm ext
    in
    Format.fprintf fm "(@[sig@[<hv -1>@ %a@]@ end@])" (print_list pr_elem "@\n") sgn
  else Format.fprintf fm "sig...end"

and print_row cfg poly fm { row_constr; row_args; row_ret } =
  match row_ret with
  | None ->
      let s = if poly then "`" else "" in
      if row_args = [] then Format.fprintf fm "%s%a" s print_constr row_constr
      else
        Format.fprintf fm "@[%s%a of %a@]" s print_constr row_constr
          (print_list (print cfg) "@ *@ ")
          row_args
  | Some ty ->
      if row_args = [] then Format.fprintf fm "%a : %a" print_constr row_constr (print cfg) ty
      else
        Format.fprintf fm "@[%a : %a@]" print_constr row_constr
          (print_list (print cfg) "@ ->@ ")
          (row_args @ [ ty ])
...
and print cfg fm typ =
  ...
  | TFun _ ->
      let func = is_functor_typ typ in
      let rec aux fm (xs, typ) =
        match xs with
        | [] -> print' fm typ
        | x :: xs' ->
            let s1, s2 = if func then ("functor (", ")") else ("", "") in
            if List.exists (cfg.occur x) (typ :: List.map Id.typ xs) || func then
              Format.fprintf fm "@[<hov 2>%s%a:%a%s ->@ %a@]" s1 Id.print x print' (Id.typ x) s2 aux
                (xs', typ)
  ...
  | TFuns (xs, typ) ->
      let rec aux fm (xs, typ) =
        ...
        | x :: xs' ->
            if List.exists (cfg.occur x) (typ :: List.map Id.typ xs) then
              Format.fprintf fm "@[<hov 2>%a:%a ->@ %a@]" Id.print x print' (Id.typ x) aux (xs', typ)
            else Format.fprintf fm "@[<hov 2>%a ->@ %a@]" print' (Id.typ x) aux (xs', typ)
      in
      Format.fprintf fm "(%a)" aux (xs, typ)
  | TTuple [] -> Format.fprintf fm "()"
  | TTuple [ x ] -> Format.fprintf fm "(@[<hov 2>%a * @])" print' (Id.typ x)
  | TTuple xs ->
      let pr fm x =
  ...
  | TAttr (TAPred (x, ps) :: preds, typ) when not !!Dbg.check ->
      Format.fprintf fm "@[%a@[<hov 3>[\\%a. %a]@]@]" print'
        (TAttr (preds, typ))
        Id.print x print_preds ps
  ...
  | TAttr (TAId (_, x) :: attrs, ty) when not !!Dbg.check ->
      let ty' = TAttr (attrs, ty) in
      let s1, s2 = match ty with TFun _ -> ("(", ")") | _ -> ("", "") in
      Format.fprintf fm "@[%s%a%s^%d@]" s1 print' ty' s2 x
  | TAttr (TAExistential s :: _, TVar (_, { contents = None }, _)) -> Format.pp_print_string fm s
  | TAttr (TAExistential _ :: attrs, ty) -> print' fm @@ TAttr (attrs, ty)
  | TAttr (TARec r :: attrs, ty) ->
      Format.fprintf fm "(%a as %a)" print' (TAttr (attrs, ty)) print' (TVar r)
  | TAttr (attrs, ty) ->
      Format.fprintf fm "(%a @@ %a)" print' ty (List.print (print_attr cfg)) attrs
  | TVariant (_, []) -> Format.fprintf fm "(|)"
  ...
  | TVariant (VPoly { closed; tvar; lower }, labels) ->
      let s = if not closed then ">" else if lower = None then "" else "<" in
      let pr_lower fm =
        if not closed then ()
        else
          match lower with
      ...
      in
      let pr_tvar fm = match tvar with None -> () | Some _ -> Format.fprintf fm " as _" in
      Format.fprintf fm "[%s @[%a%t@]%t ]" s
        (print_list (print_row cfg true) "@ | ")
        labels pr_lower pr_tvar
  | TRecord fields ->
      let pr fm (l, (f, typ)) =
  ...
  | TConstr (c, tys) ->
      Format.fprintf fm "@[@[(%a)@] %a@]" (print_list print' ",@ ") tys Path.print c
  ...
  | TConstraint (ty, cs) ->
      let pr fm (p, c) = Format.fprintf fm "@ constraint %a = %a" print' p print' c in
      Format.fprintf fm "@[%a%a@]" print' ty (print_list pr "") cs
  | TOpen -> Format.fprintf fm ".."
