  ...
  let hole_insert target state typed =
	...
	if !Flag.Termination.disjunctive then
              (* let s_x = if * then
                 ...
                 s_prev_x *)
	  Term.(let_ [extract_id statevar, Term.(if_ update_flag (restore_type state argvar) prev_statevar)] cont)
	else
              (* let s_x = x *)
	...
	if Id.eq id target.id then
	  if !Flag.Termination.disjunctive then
		 ...
		 (let_
            ...
            Term.(
              (* if prev_set_flag then assert __HOLE__; *)
	      seq (if_ prev_set_flag (assert_ hole_term) unit_term)
      ...
      in if Id.eq id target.id && !Flag.Termination.split_callsite then
	  let body_update =
	    (* let set_flag = true in *)
	    make_let
...
let construct_LLRF {variables = variables_; prev_variables = prev_variables_; coefficients = coefficients_} =
  ...
  let rank cs vs = try List.fold_left2
