...
let warning fmt =
  let cont s =
    if not (List.mem s !warning_history) then (
...
let[@inline] ( |*@> ) x _f = x
let[@inline] ( |@*> ) x _f = x
let[@inline] ( ^> ) b f = if b then f else Fun.id
let[@inline] ( @> ) b f = if b then f else ignore
...
let[@inline] ( ||| ) f g x = f x || g x
let[@inline] ( !? ) x = Lazy.force x
let[@inline] ( let@ ) f k = f k
let[@inline] ( => ) b1 b2 = (not b1) || b2
let ( @@@ ) = List.rev_append
...
let assert_ ?__LOC__ f x =
  ...
  with Assert_failure _ as e -> (
    match __LOC__ with None -> raise e | Some loc -> raise_assert_failure loc)

let assert_false ?__LOC__ _ =
  match __LOC__ with None -> failwith "assert_false" | Some loc -> raise_assert_failure loc
...
let print_list_with_flag print ?open_box ?(first = false) ?(last = false) ?limit punc fm xs =
  if xs <> [] then (
    let exceed =
      match limit with None -> false | Some limit -> limit >= 0 && List.length xs > limit
    in
    let xs = if exceed then BatList.take (Option.get limit) xs else xs in
    let punc' = format_of_string punc in
    (match open_box with None -> Format.fprintf fm "@[" | Some open_ -> open_ fm);
...
let print_list print ?open_box ?(first = false) ?(last = false) ?limit punc fm xs =
  let print ~first ~last fm x = print fm x in
  print_list_with_flag print ?open_box ~first ~last ?limit punc fm xs
[@@ocaml.warning "-27"]
...
module Triple = struct
  ...
  let of_list xs = match xs with [ x; y; z ] -> (x, y, z) | _ -> invalid_arg "%s" __FUNCTION__
...
module Quadruple = struct
  ...
  let of_list xs =
    match xs with [ x; y; z; w ] -> (x, y, z, w) | _ -> invalid_arg "%s" __FUNCTION__
...
module Option = struct
  ...
  let apply = map
  let ( let* ) = bind
  let ( let** ) x f = match x with None -> None | Some y -> ( try f y with _ -> None)
  let ( >>= ) = bind
  let ( >=> ) f g x = f x >>= g
  let return = some
  let lift = map
  ...
  let get x = match x with None -> raise No_value | Some x -> x
  let make check x = if check x then Some x else None
  let some_if f x = if f x then Some x else None
  let to_list x = match x with None -> [] | Some x' -> [ x' ]
  let of_list xs = match xs with [] -> None | [ x ] -> Some x | _ -> invalid_arg "Option.of_list"

  let print pr fm x =
    match x with None -> Format.fprintf fm "None" | Some x' -> Format.fprintf fm "Some %a" pr x'
  ...
  let for_all f x = match x with None -> true | Some y -> f y
  let exists f x = match x with None -> false | Some y -> f y
  ...
  let join = merge
  let flatten x = match x with None -> None | Some x' -> x'
end
...
module List = struct
  ...
  let hd_default xs x = match xs with [] -> x | y :: _ -> y
  let get xs = match xs with [ x ] -> x | _ -> invalid_arg "List.get"
  let is_singleton xs = length xs = 1
  let print ?limit pr fm xs = Format.fprintf fm "@[<hov 1>[%a]@]" (print_list ?limit pr ";@ ") xs
  let rec unfold_right f s = match f s with None -> [] | Some (x, s') -> x :: unfold_right f s'

  let rec unfold_left acc f s =
    match f s with None -> acc | Some (x, s') -> unfold_left (x :: acc) f s'

  let unfold_left f s = unfold_left [] f s
  let rec init n f i acc_rev = if i >= n then rev acc_rev else init n f (i + 1) (f i :: acc_rev)
  ...
  let rec tabulate n f rev_acc =
    if n < 0 then invalid_arg "List.tabulate"
    else if n = 0 then rev rev_acc
    else tabulate (n - 1) f (f n :: rev_acc)

  let tabulate n f = tabulate n f []
  let count f xs = fold_left (fun acc n -> if f n then acc + 1 else acc) 0 xs
  let decomp_cons = function [] -> invalid_arg "List.decomp_cons" | x :: xs -> (x, xs)
  ...
  let rec rev_filter_map acc f xs =
    ...
    | [] -> acc
    | x :: xs' ->
        let acc' = match f x with None -> acc | Some r -> r :: acc in
  ...
  let split3 xs =
    fold_right
      (fun (x, y, z) (acc1, acc2, acc3) -> (x :: acc1, y :: acc2, z :: acc3))
      xs ([], [], [])
  ...
  let rec find_mapi f i = function
    | [] -> raise Not_found
    | x :: xs' -> ( match f i x with None -> find_mapi f (i + 1) xs' | Some r -> r)
  ...
  let assoc ?(eq = ( = )) x xs = snd @@ find (eq x -| fst) xs
  ...
  let find_default d f xs = try find f xs with Not_found -> d

  let rec find_map_opt f xs =
    match xs with
    | [] -> None
    | x :: xs' -> ( match f x with None -> find_map_opt f xs' | Some y -> Some y)

  let find_map f xs = match find_map_opt f xs with None -> raise Not_found | Some x -> x
  let find_map_default f x xs = match find_map_opt f xs with None -> x | Some x -> x

  let rec rfind_map_opt f xs =
    match xs with
    | [] -> None
    | x :: xs' -> ( match rfind_map_opt f xs' with None -> f x | Some y -> Some y)

  let rfind_map f xs = match rfind_map_opt f xs with None -> raise Not_found | Some x -> x
  let rfind_map_default f x xs = match rfind_map_opt f xs with None -> x | Some x -> x
  let find_pos f xs = fst @@ findi f xs
  ...
  let find_decomp_opt f xs = try Some (find_decomp f xs) with Not_found -> None
  let assoc_default ?(eq = ( = )) x k tbl = try assoc ~eq k tbl with Not_found -> x
  let assoc_opt ?(eq = ( = )) k tbl = try Some (assoc ~eq k tbl) with Not_found -> None

  let rec assoc_map ?(eq = ( = )) k f tbl =
    match tbl with
    | [] -> raise Not_found
    | (k', x) :: tbl' ->
        if eq k k' then (x, (k', f x) :: tbl')
  ...
  let rec decomp_assoc ?(eq = ( = )) k tbl =
    ...
    | [] -> raise Not_found
    | (k', x) :: tbl' ->
        if eq k k' then (x, tbl') else decomp_assoc ~eq k tbl' |> Pair.map_snd @@ cons (k', x)
  ...
  let remove_lower is_lower xs =
    let rec aux acc_rev xs =
      ...
      | x :: xs' ->
          let acc_rev' =
            if exists (is_lower x) acc_rev || exists (is_lower x) xs' then acc_rev else x :: acc_rev
          in
          aux acc_rev' xs'
    in
    aux [] xs

  let assoc_all ?(eq = ( = )) k tbl =
    filter_map (fun (k', x) -> if eq k k' then Some x else None) tbl
  ...
  let for_alli f xs = for_all (Fun.uncurry f) @@ mapi Pair.pair xs
  ...
  let transpose xss =
    ...
    | xs :: _ ->
        let m = length xss in
        let n = length xs in
        init n (fun i -> init m (fun j -> nth (nth xss j) i))

  let cons_unique ?(eq = ( = )) x xs = if mem ~eq x xs then xs else x :: xs
  ...
  let to_string ?(delimiter = ";") f xs = "[" ^ BatString.join delimiter (List.map f xs) ^ "]"
...
module Compare = struct
  ...
  let topological ?(eq = ( = )) ?dom edges =
    let map =
      ...
      in
      let no_edge =
        List.filter_out (fun x -> List.exists (fun (y, z) -> eq x y || eq x z) edges) dom'
      in
      edges |> topological_sort ~eq |> ( @ ) no_edge |> List.mapi (fun i x -> (x, i))
    in
...
module String = struct
  ...
  let last s = s.[length s - 1]
  let split_nth s n = (sub s 0 n, sub s n (length s - n))
  let count_line s = fold_left (fun n c -> if c = '\n' then n + 1 else n) 0 s
  let remove_char c s = filter (( <> ) c) s
  ...
  let sign_to_letters s =
    let map c = match of_sign c with None -> make 1 c | Some s -> "_" ^ s ^ "_" in
    replace_chars map s
  ...
  let split_blanc s =
    ...
    let rec parse quot acc cs =
      ...
      | Some q, _, c :: cs' when List.mem c quots ->
          if c = q then parse None acc cs' else parse quot (acc @ [ c ]) cs'
  ...
  let escape_for_sh s =
    ...
    if List.exists Char.is_whitespace @@ to_list s then s'
    else if contains s '\'' then s'
    else if String.length s' > String.length s + 2 then s'
    else s

  let mem c s = index_opt s c <> None

  let remove_prefix_aux ~prefix s =
    match split s ~by:prefix with "", s2 -> Some s2 | _ -> None | exception Not_found -> None

  let remove_prefix ~prefix s =
    match remove_prefix_aux ~prefix s with None -> invalid_arg "%s" __FUNCTION__ | Some r -> r

  let remove_prefix_if_any ~prefix s =
    match remove_prefix_aux ~prefix s with None -> s | Some r -> r
end

module Math = struct
  let rec gcd m n = if m < n then gcd n m else if n = 0 then m else gcd n (m mod n)
  let pi = 2. *. acos 0.
end
...
module Filename = struct
  ...
  let decomp_extension filename =
    match String.rsplit filename ~by:"." with exception Not_found -> None | ss -> Some ss

  let chop_extension_if_any filename =
    try chop_extension filename with Invalid_argument _ -> filename

  let change_extension filename ext = chop_extension_if_any filename ^ "." ^ ext
end
...
module IO = struct
  module CPS = struct
    let open_in file k =
      ...
      Exception.finally (fun () -> close_in cin) k cin

    let open_out file k =
      let cout = open_out file in
      Exception.finally (fun () -> close_out cout) k cout
  end
...
module Combination = struct
  ...
  let product xs ys = List.flatten_map (fun p -> List.map (fun p' -> (p', p)) xs) ys
end
...
module Arg = struct
  ...
  let align args =
    ...
    |> List.mapi (fun i (s, f, desc) ->
           if s = "" then
             let nr = if i <> 0 then "\n  " else "" in
             let s' = desc |> String.trim |> Format.sprintf "%s(*** %s ***)" nr in
             (s', f, " ")
           else
             let aux c =
               if c = '\n' then
                 let rec len i = if desc.[i] = ' ' then 1 + len (i + 1) else 3 in
                 let len' = String.length s + len 0 in
                 let len'' = match f with Symbol _ -> len' + 1 | _ -> len' in
                 "\n" ^ String.make len'' ' '
               else String.of_char c
             in
             let desc = String.replace_chars aux desc in
             (s, f, desc))
end
...
module Sys = struct
  ...
  let runtime_parameters_list () =
    ...
    |> List.filter_map (function [ l; v ] -> Some (l, v) | _ -> None)
end
...
module Marshal = struct
  ...
  let from_file file default =
    if Sys.file_exists file then IO.CPS.open_in file Marshal.from_channel else default ()

  let from_file_exn file = IO.CPS.open_in file Marshal.from_channel
  let size ?(flag = []) x = String.length (to_string x flag)
end
...
module Lexing = struct
  ...
  let lexeme_line lb =
    if lexeme_start lb = lexeme_end lb then ""
    else
      let pos = lexeme_start_p lb in
      let rec find_eol i =
        match sub_lexeme_char_opt lb i with
        | None -> i - 1
        | Some '\n' -> i
        | Some _ -> find_eol (i + 1)
        | exception _ -> i - 1
      in
      let eol = find_eol (lexeme_start lb) in
      sub_lexeme lb pos.pos_bol eol

  let print_error ?(fm = Format.err_formatter) lb =
    let pos = Lexing.lexeme_start_p lb in
    let line = lexeme_line lb in
    if line <> "" then Format.fprintf fm "%s@." line;
    Format.fprintf fm "%s%s@."
      (String.make (pos.pos_cnum - pos.pos_bol) ' ')
      (String.make (lexeme_end lb - lexeme_start lb) '^');
    Format.fprintf fm "%a: Syntax error@." print_position pos
end
...
module Time = struct
  ...
  let string_of_tm
    ...
    Format.sprintf "%04d/%02d/%02d %02d:%02d:%02d" (tm_year + 1900) (tm_mon + 1) tm_mday tm_hour
      tm_min tm_sec

  let string_of_tm_simple
      {
        Unix.tm_sec;
        tm_min;
        tm_hour;
    ...
    Format.sprintf "%04d%02d%02d%02d%02d%02d" (tm_year + 1900) (tm_mon + 1) tm_mday tm_hour tm_min
      tm_sec

  let string_of_current_local () = string_of_tm @@ Unix.localtime !!Unix.time
  let print fm t = Format.pp_print_string fm @@ string_of_tm @@ Unix.localtime t
  let print_simple fm t = Format.pp_print_string fm @@ string_of_tm_simple @@ Unix.localtime t
end
  ...
  let confirm ?(fm = Format.std_formatter) ?(default = true) ?(yes = false) s =
    ...
    let rec aux () =
      ...
      if yes then (
        Format.fprintf fm "y@?";
        true)
      else match read_line () with "" -> default | "y" -> true | "n" -> false | _ -> aux ()
    in
    aux ()

  let rec read_int ?(fm = Format.std_formatter) ?(check = fun _ -> true) msg =
    Format.fprintf fm "%s: @?" msg;
    match read_int_opt () with Some n when check n -> n | _ -> read_int ~check msg
end
...
module LazyList = struct
  ...
  let is_singleton l =
    match next l with
    | Nil -> false
    | Cons (_, l') -> ( match next l' with Nil -> true | Cons _ -> false)

  let rec eager_to_list l =
    if Lazy.is_val l then match !?l with Nil -> [] | Cons (x, l') -> x :: eager_to_list l' else []

  let rec of_lazy_list l =
    match l with [] -> lazy Nil | x :: l' -> lazy (Cons (!?x, of_lazy_list l'))
...
module Memoize = struct
  let create_gen size =
    ...
    fun ~found ~key_of x ~result_of f ->
      ...
      | Some x -> found x
      | None -> f k |@> Hashtbl.add tbl k -| result_of
...
let rec fixed_point ?(eq = ( = )) ?(limit = -1) f init =
  let x = f init in
  if eq x init || limit = 0 then x else fixed_point ~eq ~limit:(limit - 1) f x

let rec fixed_point_update ?(limit = -1) f init =
  let b, x = f init in
  if (not b) || limit = 0 then x else fixed_point_update ~limit:(limit - 1) f x

let transitive_closure ?(eq = ( = )) edges =
  let eq' = Pair.eq eq eq in
  let cons (x, y) es = if List.mem ~eq:eq' (x, y) es then es else (x, y) :: es in
  let f edges' =
    let aux acc (x, y) =
...
let assign_id_by_eq (type t) ?(eq = ( = )) ?(size = 0) ?(origin = 0) (xs : t list) :
    int * (t -> int) * (t * int) list =
...
let topological_index (scc_of : (int * int) list -> int list list) ?(eq = ( = ))
    (edges : ('a * 'a) list) : ('a * int) list =
  ...
  let edges_repr : (int * int) list =
    ...
    in
    edges_id |> List.filter_map trans |> List.sort_uniq compare
  in
...
module Print_ = struct
  ...
  (* Format.printf "%a" (f |> pr) x = Format.printf "%a" pr (f x) *)
  let ( |> ) f pr fm x = pr fm (f x)
