...
let rec ref_of_inter env cond atyp ityp =
      ...
      let aux p ityp =
        ...
        let is_false ityp =
          if !Flag.ModelCheck.church_encode then
            ...
            | _ -> false
          else ityp = IT.Base IT.False
        in
        if is_true ityp then [ p ]
        else if is_false ityp then [ CS.make_not p ]
        else if ityp = IT.Inter [] then []
        else (
          Format.eprintf "%a@." IT.print ityp;
          assert false)
      in
  ...
  | AT.TBase (AT.TUnit, _), IT.Base (IT.State _) -> RT.Base (RT.Unit, "", CS.Const CS.True)
  | AT.TBase (base, _), IT.Base (IT.State _) when base = AT.typ_result_base ->
      RT.Base (RT.Unit, "", CS.Const CS.True)
  | _ ->
      Format.eprintf "atyp:%a@.ityp:%a@." CEGAR_print.typ atyp IT.print ityp;
      assert false
