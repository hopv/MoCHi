...
let divide (spec : Spec.t) t ref_env =
  ...
  let make_spec f : Spec.t =
    ...
    { spec with ref_env; ext_ref_env = ext_ref_env @ spec.ext_ref_env }
...
let main orig spec parsed =
  let verify (s, spec, t) =
    ...
    (s, Main_loop.run ~orig (Problem.safety ~spec t))
  in
