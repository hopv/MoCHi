...
let extract_result cin =
  let rec discard s =
    try
      ...
      if String.length l = 0 || l.[0] = '#' then discard s else discard (s ^ l)
    with _ -> s
  in
  let s = discard "" in
  let s = String.strip s in
  if s = "{ TRUE }" then true else if s = "{ FALSE }" then false else raise Unknown
...
let is_valid_forall_exists xs ys cond p =
  ...
  let input_file = Temp.S.ext "oc" in
  let result_file = Temp.S.ext "oc_out" in
  let vars fm xs = print_list CEGAR_print.var "," fm xs in

  let cout = open_out input_file in
  let input_fm = Format.formatter_of_out_channel cout in
  if xs = [] then (
    Debug.printf "{[]:exists(%a: not (%a) || (%a) )};@." vars ys (print_conj CEGAR_print.term) cond
      CEGAR_print.term p;
    Format.fprintf input_fm "{[]:exists(%a: not (%a) || (%a) )};@." vars ys
      (print_conj CEGAR_print.term) cond CEGAR_print.term p)
  else (
    Debug.printf "{[]:forall(%a: exists(%a: not (%a) || (%a) ))};@." vars xs vars ys
      (print_conj CEGAR_print.term) cond CEGAR_print.term p;
    Format.fprintf input_fm "{[]:forall(%a: exists(%a: not (%a) || (%a) ))};@." vars xs vars ys
      (print_conj CEGAR_print.term) cond CEGAR_print.term p);
  close_out cout;

  let omega = Option.get !Flag.External.omega in
  let cmd = Format.sprintf "%s %s > %s" omega input_file result_file in
