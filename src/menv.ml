...
struct
  ...
  let assoc_option k env = List.assoc_opt ~eq:eq_key k env
  ...
  let merge env1 env2 =
    let aux (k, x) acc =
      if mem_assoc k acc then
        List.flatten_map
          (fun (k', x') ->
            let xs = if eq_key k k' then Value.merge x x' else [ x' ] in
            List.map (fun y -> (k', y)) xs)
          acc
      else (k, x) :: acc
    in
    List.fold_right aux env1 env2

  let normalize env = List.fold_right (fun v acc -> merge [ v ] acc) env []
