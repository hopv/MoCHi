...
module Dbg_orig = Debug.Make(struct let check = Flag.Debug.make_check (__MODULE__^".orig") end)

type env =
  {id_env : id list;                           (** Environment for matching types of each bound variable and its occurrence *)
   tconstrs : P.t list;                        (** Set of type constructors used  *)
   tvar_env : (int * typ) list;                (** Environment of type variables *)
   shadowed_primitive : string list;           (** primitevs functions/operators shadowed by user-defined ones *)
...
let add_vars_to_env x env =
  env.vars <- StringSet.add (Id.name x) env.vars
...
(* Unused? *)
let prim_typ_canonical =
  ["CamlinternalFormatBasics.format6", "Stdlib.format6"] (* actual definition is `type ... Stdlib.format6 = ... CamlinternalFormatBasics.format6` *)

(* TODO: Fix *)
let prim_typ_constr =
  prim_typ_constr
  |> List.rev_flatten_map (fun (s,c) ->
         s :: (if Char.is_uppercase s.[0] then [Id.name stdlib ^ "__" ^ s; Id.name stdlib ^ "." ^ s] else [])
         |> List.map (Pair.pair -$- c))

let assoc_prim_typ c tys =
  let s = Path.to_string c in
  match List.assoc_opt s prim_typs with
  | Some r -> assert (tys = []); r
  | None ->
      match List.assoc_opt s prim_typ_constr with
      | Some s' -> TConstr(s', tys)
      | None -> TConstr(c, tys)
...
let name_of_longident id =
  Format.asprintf "%a" Pprintast.longident id
let name_of_longident_loc loc =
  name_of_longident loc.Asttypes.txt
...
let wrap_letters_with_sign s =
  ...
  let s'' =
    if String.(contains "!$%&*+-./:<=>?@^|~" (last s')) then
      "(" ^ s' ^ ")"
    else
      s'
  in
...
let remove_slash_one s =
  if String.ends_with s "/1" then
    String.sub s 0 (String.length s - 2)
  else
    s

let tconstr_of_ident ident =
  tconstr_of_string (name_of_ident ident)
...
let normalized_path_of ~tenv path =
  let s =
    path
    |> Env.normalize_module_path None tenv
    |> path_of_path
    |> Path.map_name remove_slash_one
  in(*
...
let encode_nonrec_type_decl (decls : type_decls) : type_decls list =
  ...
  let tcs2 = List.rev_flatten_map (snd |- snd |- get_tconstr_typ |- List.filter_map (fst |- Type.Val._LId)) decls in
  if List.Set.disjoint tcs1 tcs2 then (* WORKAROUND: The else case may be buggy *)
    ...
    let rec gen_unique n cs =
      ...
      if List.exists (Id.name |- String.ends_with -$- postfix) cs then
        gen_unique (n+1) cs
      else
        List.map (Id.add_name_after postfix) cs
    in
    let cs = gen_unique 1 @@ List.map fst decls in
    let decls1 = List.map2 (fun (_,pty) c' -> c', pty) decls cs in
    let decls2 = List.map2 (fun (c,(params,_)) c' -> c, (params, TConstr(LId c', params))) decls cs in
    [decls1; decls2]
...
let trans_decls env (decls : decl list) : Syntax.declaration list =
  let gen_fresh =
    ...
    let rec regen x =
      if StringSet.mem (Id.name x) env.vars then
        regen (gen x)
      else
        (add_vars_to_env x env; x)
    in
    gen |- regen
  in
  let trans decl (map,acc) = (* [map] maps shadowed variables to renamed variables *)
    let is_rec = match decl with Decl_letrec _ -> true | _ -> false in
    let rename (decl : Syntax.declaration) map =
      match decl with
      | Decl_let defs when is_rec ->
          let map,defs =
            List.L.fold_right_map defs
              ~init:map
              ~f:(fun (f,t) map ->
                let map',f' =
                ...
                in
                let t' = subst_var_map_without_typ (Fmap.(list (snd _LId)) map) t in
                map', (f', t'))
          in
      ...
      | Decl_let defs ->
          ...
          in
          let sbst mp =
            let mp' = Fmap.(list (snd _LId)) mp in
          ...
          if b then Dbg.printf "       map': %a@." Print.(list (id * id)) map';
          let defs = sbst map' defs in
    ...
    let decls : Syntax.declaration list =
      ...
      | Decl_type_nonrec decls ->
          decls
          |> encode_nonrec_type_decl
          |> List.map Syntax._Decl_type
      | Ext_type(s,row) -> [Type_ext(Ext_type(s,row))]
      | Ext_rebind(s1,s2) -> [Type_ext(Ext_rebind(s1, s2))]
      | Include t -> [Include t]
    in
    let map,decls = List.fold_right_map rename decls map in
    map, decls @ acc
  in
  let map,decls = List.fold_right trans decls ([],[]) in
  List.map_append (fun (x,y) -> Syntax.Decl_let [y, Term.var x]) map decls
...
let primitives ~env ~loc ~typ =
  let randb = function
    | [t] -> make_app (make_rand Ty.bool) [t]
    | _ -> assert false
  in
  let randi = function
    ...
    | [t] ->
        ...
        [%term let x = randi in assume (0 <= x && x < `t); x]
    | _ -> assert false
  in
  let make_binop (s,f) =
    s,
  ...
  ["List.length", (function [t1] -> Term.(length t1) | _ -> assert false);
   ...
   "read_int", (function [t] ->
                              let t' =
                                randint_term
                                |& Flag.(List.mem !mode [NonTermination; FairNonTermination]) &>
                                  add_attr AAbst_under
                              in
                              Term.(t' @@ [t])
                       | _ -> assert false);
...
let tmp_remove_prefix_target_exn m f =
  ...
  | Some m ->
      ...
      let removed =
        !Flag.Method.target_exn
        |> List.map (String.remove_prefix_if_any ~prefix)
      in
...
let rec col_typ_def ~env ~tenv ~(from_types_type_declaration:from_types_type_declaration) acc rest =
  ...
  | [] -> env, acc
  | path::rest' ->
      let s = normalized_path_of ~tenv path in
      let env,acc',rest'' =
...
let col_typ_def ~env ~tenv ~from_types_type_declaration paths = col_typ_def ~env ~tenv ~from_types_type_declaration [] paths

let rec filter_decls ?(acc:(path * (_ * typ)) list=[]) tconstrs (decls:(path * (_ * typ)) list) =
  ...
  if acc' = [] then
    ...
    let tconstrs =
      acc'
      |> List.rev_flatten_map (snd |- snd |- get_tconstr_typ)
      |> List.map fst
    in
    filter_decls ~acc tconstrs decls'

let make_ty_decl tconstrs ~env ~tenv ~(from_types_type_declaration:from_types_type_declaration) =
  let string_of_path = normalized_path_of ~tenv |- Path.to_string in
  let env,decls =
    env.tconstrs
    |> List.filter (P.heads |- List.hd |- Ident.global)
    |> List.filter_out (string_of_path |- List.mem_assoc -$- Predef.builtin_idents)
    |> List.filter_out (string_of_path |- List.mem_assoc -$- prim_typ_constr)
    |> List.filter_out (string_of_path |- String.starts_with -$- "CamlinternalFormatBasics.") (* If we remove this line, [ext_typ] will be messy *)
    |> col_typ_def ~env ~tenv ~from_types_type_declaration
  in
  env,
  decls
  |> List.filter_out (fun (_,(_,ty)) -> ty = Ty.abst)
  |> List.filter_out (fun (s,(params,ty)) -> TConstr(s,params) = ty)
  |> filter_decls tconstrs

let normalize_ids =
  Trans.map_var
    (Id.map_name (function (* TODO: check conflicts with [opt] & [sth] *)
         | "*opt*" -> "opt"
         | "*sth*" -> "sth"
         | "( *opt* )" -> "opt"
         | "( *sth* )" -> "sth"
...
let rec apply_tpackage_constraint add_prefix constr ty =
  ...
  | TModSig {sig_types; sig_values; sig_ext} ->
      let rest, sig_types =
        List.L.fold_right sig_types ~init:(constr,[]) ~f:(fun decls (rest,acc) ->
            let rest',decls' =
              List.L.fold_right decls ~init:(rest,[]) ~f:(fun (t,(params,ty)) (rest,acc) ->
                  match List.partition (fun (path,_) -> path = add_prefix (Type.LId t)) rest with
      ...
      in
      let rest, sig_values =
        List.L.fold_right sig_values ~init:(rest,[]) ~f:(fun x (rest,acc) ->
            let rest1,rest2 = List.partition (fst |- Path.is_prefix (add_prefix (LId (Id.set_typ x ())))) rest in
            let add_prefix = add_prefix -| Path.cons x in
            let x' =
              if is_mod_var x then
                Id.map_typ (apply_tpackage_constraint add_prefix rest1) x
              else
                x
            in
            rest2, x'::acc
...
let apply_tpackage_constraint ?(add_prefix=Fun.id) constr ty =
  if constr = [] then
    ...
    apply_tpackage_constraint add_prefix constr ty

let subst_tconstr_without_copy,subst_tconstr_without_copy_typ =
  let tr = Tr2.make () in
  let tr_typ map ty =
    match ty with
    | TConstr(LId s,[]) -> Id.List.assoc_default ty s map
    | _ -> tr.typ_rec map ty
  in
  tr.typ <- tr_typ;
  tr.term, tr.typ
...
let unify_newtype (ss:string list) ty t =
  if ss = [] then
    ...
    (try
       unify ~ignore_non_poly_fail:true ty t'.typ
     with CannotUnify _ ->
       [%unsupported]); (* TODO: must use env, but we do not have env here, so we have to delay the unification *)
    t'
...
let decomp_tunivar_name s =
  ...
  | ["Tunivar"; s'] -> s'
  | _ -> invalid_arg "%s" __FUNCTION__
let make_tunivar ~env = function
  | None -> unsupported "Tunivar None"
  | Some s ->
      let name = make_tunivar_name s in
      TConstr(LId (Id.make ~env name ()), [])
let decomp_tunivar = function
  | TConstr(LId x, []) -> x
  | _ -> invalid_arg "%s" __FUNCTION__

let canonize_typ_constrs =
  let map =
    prim_typ_canonical
    |> List.map (Pair.map_same Path.of_string)
  in
  Trans.map_typ (subst_path_map map)

let assoc_tvar (env:env) (ty:type_expr) =
  if not (Btype.is_Tvar ty) then [%invalid_arg];
  let id = Types.get_id ty in
  match List.assoc_opt id env.tvar_env with
  | None ->
      let ty = if Types.get_level ty = Btype.generic_level then !!new_tvar else !!new_tvar_weak in
      let tvar_env = (id, ty)::env.tvar_env in
      {env with tvar_env}, ty
  | Some ty -> env, ty

let from_constructor_description tenv desc =
  match desc.cstr_tag with
  | Cstr_constant _ -> lid_of_string desc.cstr_name
  | Cstr_block _ -> lid_of_string desc.cstr_name
  | Cstr_extension(path, _) -> normalized_path_of ~tenv path
  | Cstr_unboxed -> lid_of_string desc.cstr_name

let from_constructor_description_normalize tenv desc =
  let name = from_constructor_description tenv desc in
      ...
      begin
        ...
        | Type_variant _ as kind ->
            ...
            let aux {cd_id} =
              ...
              if name' = Path.name name then
                Some (lid_of_string name')
              else
                None
            in
...
let from_list ~f ~env xs =
  List.fold_right_map (fun x env -> f ~env x) xs env
...
let rec from_types_label_declaration ~env ~tenv {ld_id;ld_mutable;ld_type} =
...
and from_types_type_declaration ~env ~tenv ?name ?path (decl:Types.type_declaration) : env * (typ list * typ) =
  ignore name;
(*
...
*)
  ...
  let env,params_rev,constraints,_ =
    List.L.fold_left decl.type_params
      ~init:(env,[],[],[])
  ...
  let env,ty =
    match
      ...
      | Some ty ->
          ...
          | Some p when ty = TConstr(p, params) -> None
          | _ -> Some (env,ty)
          end
      | None -> None
    with
    ...
    | None ->
        ...
        | Type_variant _ as kind ->
            let constrs = decomp_Type_variant kind in
            let env,constrs =
              from_list constrs ~env
                ~f:(fun ~env {cd_id;cd_args;cd_res} ->
...
and from_row_desc ~env ~tenv ~visited row_desc =
  ...
  let tvar =
    match Types.get_desc row.more with
    | Tvar (Some _s) -> Some ()
    | _ -> None
  in
  let env,rps =
    from_list ~env row.fields
      ~f:(fun ~env (s,field) ->
        ...
        let env,r =
          ...
          | Rpresent (Some type_expr) ->
              ...
              env, Some (tys, true)
          | Reither _ as field ->
              let b,tys = decomp_Reither field in
              begin match b, tys with
              | true, [] -> env, Some ([], false)
              | true ,_ | _, [] -> assert false
              | _, [ty] ->
                  let env,ty = from_ty ~env ty in
                  env, Some ([ty], false)
              | _ -> unsupported "Conjunctive arguments of polymorphic variants"
              end
          | Rabsent ->
              env, None
        in
        env,
        Option.map (fun (row_args,present) -> {row_constr; row_args; row_ret=None}, present) r)
  in
  let rps = List.filter_map Fun.id rps in
  let lower =
    if List.for_all snd rps then
      None
    else
      rps
  ...
  let rows =
    rps
    |> List.map fst
    |> List.sort (Compare.on row_constr)
  in
...
*)
  ...
  if List.mem id visited then unsupported "Recursive polymorphic variants? (%a)" Printtyp.type_expr ty0;
...
(*
      let typ' = Ctype.full_expand ~may_forget_scope:false tenv typ in
      let typ' =
        (* WORKAROUND: from_type_expr does not terminate if using typ' here for "type t = [`A of t];; type t' = t" *)
        match Types.get_desc ty1, Types.get_desc typ' with
        | Tconstr _, Tvariant _ -> typ
        | _ -> typ'
      in
      Dbg.printf "ty0: %a@." Printtyp.type_expr ty0;
      Dbg.printf "ty1: %a@." Printtyp.type_expr typ;
      Dbg.printf "ty2: %a@.@." Printtyp.type_expr typ';
      (*
      if false then Debug.printf "%sty3: %a@.@." space Printtyp.raw_type_expr typ';
       *)
*)
      typ
    else
      Btype.proxy ty0 (* WORKAROUND to stop recursion (the resulting type is wrong if the type has polymorphic type constraints) *)
  in
  let env = add_tconstr ~tenv (Types.get_desc typ) env in
  let typ =
    if Btype.is_Tvar typ && not (Btype.is_Tvar ty0) then
      ty0
    else
      typ
  in
  match Types.get_desc @@ expand_if_arrow ~tenv typ with
  ...
  | Ttuple typs ->
      ...
      env, make_ttuple tys'
  | Tconstr(path, typs, _) ->
      let c = normalized_path_of ~tenv path in
      (match c with LId c' when (Id.name c').[0] = '$' -> unsupported "Existential types in GADT" | _ -> ());
      let env,tys = from_list ~f:from ~env typs in
      let ty = assoc_prim_typ c tys in
      env, ty
  | Tobject _ -> (* TODO *)
      env, TConstr(C.obj, [])
  | Tfield _ -> (* TODO *)
  ...
  | Tpoly(typ, typs) ->
      ...
      let vars =
        try
          List.map decomp_tunivar tys
        with Invalid_argument _ -> unsupported "Tpoly"
      in
  ...
  | Tpackage _ as desc ->
      ...
      let env, ty =
        ...
        | exception Not_found -> env, TConstr(normalized_path_of ~tenv path, [])
      in
      let env,constr =
        from_list
          idtys
...
and from_core_type ~env ~tenv (ty:core_type) =
  from_type_expr ~env ~tenv ty.ctyp_type
...
and from_typedtree_label_declaration ~env ~tenv ({ld_id;ld_mutable;ld_type}:Typedtree.label_declaration) =
  let s = label_of_string (name_of_ident ld_id) in
  let flag = from_mutable_flag ld_mutable in
...
and from_types_constructor_arguments ~tenv ~env (cd_args:Types.constructor_arguments) =
  match cd_args with
  | Cstr_tuple args ->
      from_list ~f:(from_type_expr ~visited:[] ~tenv) ~env args
  | Cstr_record fields ->
      let env,fields' = from_types_label_declarations ~env ~tenv fields in
      env, [TRecord fields']
...
and from_typedtree_constructor_arguments ~tenv ~env (cd_args:Typedtree.constructor_arguments) =
  match cd_args with
  | Cstr_tuple args ->
      from_list ~f:(from_type_expr ~visited:[] ~tenv) ~env @@ List.map (fun arg -> arg.ctyp_type) args
  | Cstr_record fields ->
      let env,fields' = from_typedtree_label_declarations ~env ~tenv fields in
      env, [TRecord fields']

and from_typedtree_type_declaration ~tenv ~env (decl:Typedtree.type_declaration) : env * (tconstr * (typ list * typ)) =
...
and from_ident_path ~env ~tenv (path:P.t) mk_ty =
  ...
  match path' with
  | Pident _id ->
      let env,typ = mk_ty env in (* This trick can be removed if memoization/caching is implemented appropriately *)
...
and module_type_of_ident ~env ~tenv m =
  ...
  if Ident.global m && List.mem_assoc ~eq:Ident.equal m env.mod_typ_env then
    env, ty_global
  else
    let env,ty =
      ...
      Cache.mod_type_of_ident ~found ~key_of m ~result_of (fun _ ->
          let decl =
            try
              Env.find_module (Pident m) tenv
            with Not_found -> assert false
          in
          from_types_module_type ~env ~tenv decl.md_type)
    in
    if Ident.global m then
      add_mod_typ m m' ty env,
      ty_global
    else
      env, ty
...
and from_pattern_desc : type k. env:env -> tenv:Env.t -> k pattern_desc' -> typ -> env * pat_desc =
  ...
  | Tpat_constant(Const_float s) -> env, PConst (make (Const(Float (float_of_string s))) typ ~attr:const_attr)
  ...
  | Tpat_constant(Const_nativeint n) -> env, PConst (make (Const(Nativeint n)) typ ~attr:const_attr)
  | Tpat_tuple ps ->
      let env,ps' = from_list ~f:from_pattern ~env ps in
      env, PTuple ps'
  | Tpat_construct _ ->
      let loc,cstr_desc,ps = decomp_Tpat_construct desc in
      begin match loc.txt, name_of_longident_loc loc, ps with
      | Longident.(Ldot (Lident "*predef*", "None")), _, [] -> env, PNone  (* TODO: fix the following four patters *)
      ...
      | _ ->
          let env,ps' = from_list ~f:from_pattern ~env ps in
          let c = from_constructor_description_normalize tenv cstr_desc in
...
and from_pattern : type k. env:env -> k pattern' -> env * pattern =
  ...
  let env,pat_desc =
    if is_Tpat_value desc then
      ...
      from_pattern_desc ~env ~tenv desc pat_typ
  in
  env,
...
and from_value_kind = function
  ...
  | v when is_Val_unbound v -> Format.eprintf "Val_unbound@."; assert false
  | _ -> assert false
[@@ocaml.warning "-11"]
...
and from_attribute {Parsetree.attr_name; attr_payload} =
  if attr_name.txt = "mochi" then
    ...
    | PStr [{pstr_desc=Pstr_eval({pexp_desc=Pexp_ident x},_)}] when name_of_longident_loc x = "target" ->
        Some ()
    | PStr _ -> None
    | PSig _ -> None
    | PTyp _ -> None
...
and from_expression ~env {exp_desc; exp_loc; exp_type; exp_env=tenv; exp_attributes; exp_extra} : env * term =
  let env,typ = from_type_expr ~env ~tenv exp_type in
  let newtypes = List.filter_map (function (Texp_newtype s, _, _) -> Some s | _ -> None) exp_extra in
  let env,t =
    match exp_desc with
    | Texp_ident(path, _, _) ->
        ...
        if !Flag.Print.id_loc <> "" && String.exists (Lid.to_string lid) !Flag.Print.id_loc then
          Format.fprintf !Flag.Print.target "Found variable %a: %a@." Print.lid lid Print.location exp_loc;
        env, Term.var_lid ~typ lid
    | Texp_constant c ->
        env, (make (Const (from_constant c)) typ ~attr:const_attr)
    ...
    | Texp_let(Nonrecursive, pats, e) ->
        ...
        in
        let ps,ts =
          if Flag.EvalStrategy.tuple_left_to_right then
            ps, ts
          else
            List.rev ps, List.rev ts
        in
        ...
        let env,t2 =
          let env =
            ...
            {env with id_env = bvs @ env.id_env}
            |> check_shadow bvs
          in
    ...
    | Texp_apply(e, args) -> (* TODO: fix the evaluation when `xs <> []` *)
        ...
        let env,xs,ts =
          let env,tys =
            if List.exists (snd |- (=) None) args then (* Minor optimization *)
              take_arg_types ~env ~tenv:e.exp_env e.exp_type (List.length args)
            else
              env, List.map (Fun.const Ty.unit) args (* Dummy *)
          in
          List.L.fold_right2 args tys ~init:(env,[],[])
            ~f:(fun (label,arg) ty (env,xs,ts) ->
              let (env,t),xs =
                match arg with
                | Some e ->
                    let e =
                      match label with
                      | Optional _ -> {e with exp_env=tenv} (* I don't know why, but the type environment of e is not appropriate for this context *)
                      | _ -> e
                    in
    ...
    | Texp_match(e,pats,tp) ->
        ...
        let pats,pats_exn = List.partition_map (fun (is_exn,p,t) -> if is_exn then Right(p,t) else Left(p,t)) pats in
        let pats =
          match tp with
          | Total -> pats
        ...
        let t =
          if pats_exn = [] then
            ...
            Term.(let_ [x, try_ (left t typ) e pats_exn]
                  (if_ (is_left (var x))
                   (match_ ~typ (get_left (var x)) pats)
                   (get_right (var x))))
        in
        env, t
    | Texp_try(e,pats) ->
    ...
    | Texp_construct(loc,desc,es) ->
        ...
        let env,desc =
          ...
          | "None",[] -> env, TNone
          | "Some",[t] -> env, TSome t
          | "CamlinternalFormatBasics.Format", [_;t2] -> env, t2.desc (* treat [_ format6] as [string] *)
          | _ ->
              let c = from_constructor_description_normalize tenv desc in
    ...
    | Texp_array es ->
        ...
        let typ' =
          ...
          | [] ->
              try
                array_typ typ
              with Invalid_argument _ -> unsupported "Texp_array"
        in
    ...
    | Texp_while(e1,e2) ->
        ...
        let t2' = [%term if `t1 then (`t2; f ())] in
        env, [%term let f x = `t2' in f ()]
    | Texp_for(x, _, e1, e2, dir, e3) ->
        ...
        if can_unify t3.typ Ty.unit then
          Type_util.unify t3.typ Ty.unit
        else
        ...
        let t =
          if !Flag.Method.abst_for_loop then
            let cond =
              match dir with
              | Upto -> [%term init <= last]
              | Downto -> [%term init >= last]
            in
            let t3' =
              match dir with
              | Upto -> [%term assume (init <= x' && x' <= last); `t3]
              | Downto -> [%term assume (last <= x' && x' <= init); `t3]
            in
            [%term if `cond then let x' = randi in `t3']
          else
            let t31 =
              match dir with
              | Upto -> [%term x' <= last]
            ...
            let t32 =
              ...
              [%term `t3; f `x'']
            in
            [%term let f x' = if `t31 then `t32 in f init]
        in
        env, [%term let init = `t1 in
                    let last = `t2 in
                    `t]
    | Texp_send _
      | Texp_new _ ->
        Flag.Abstract.use "class/object";
        env, Term.rand typ
    ...
    | Texp_letmodule(mb_id, _loc, _, mb_expr, e) ->
        let mb_id_opt =
          if is_named_module mb_id then
            Some (get_module_id mb_id)
          else
            unsupported "expression (let module _ = ...)"
        in
        let env, m, mdl = from_module_expr_top ~env mb_id_opt mb_expr in
        let m = Option.get m in
        let env,t = from_expression ~env e in
        env, [%term let m = `mdl in `t]
    ...
    | Texp_assert e ->
        ...
        if !Flag.Print.assert_loc then
          if !!Mochi_util.is_silent then
            Format.fprintf !Flag.Print.target "%s@." s
          else
            Format.fprintf !Flag.Print.target "Found assertion: %s@." s;
  ...
  env,
  t
  |> unify_newtype newtypes typ
  |> set_typ -$- typ
  |> add_attr (ALoc exp_loc)

and from_case : type k. env:env -> k case' -> env * (Syntax.pattern * term) =
  fun ~env {c_lhs;c_guard;c_rhs} ->
  let env,p =
    try
      from_pattern ~env c_lhs
    with PatternExcep _ -> assert false
  in
  let env,t =
...
and from_types_signature ~env ~tenv ?path (sgn:Types.signature) =
  ...
  let sig_types =
    let aux acc_rev x =
      match acc_rev, x with
      | _,                     `Type(s, ty, Trec_not)    -> (`Nonrec, [s, ty])::acc_rev
      | _,                     `Type(s, ty, Trec_first)  -> (`Rec, [s, ty])::acc_rev
      | (flag,decl)::acc_rev', `Type(s, ty, Trec_next)   -> (flag, (s, ty)::decl)::acc_rev'
      | [],                    `Type(_, _,  Trec_next)   -> assert false
      | _                                                -> acc_rev
    in
     (* I don't know why cyclic definitions appear; we have to see the stamp of Ident.t?
        (e.g., the type "variant" of Solver in
         https://github.com/reasonml/reason/blob/master/src/menhir-recover/synthesis.ml) *)
  ...
  let sig_ext = List.filter_map (function `Ext x -> Some x | _ -> None) items in
  env, {sig_types; sig_values; sig_ext}
...
and from_types_signature_item ~env ~tenv ?path sig_item =
  ...
  | Sig_type(id, ty_decl, rec_status, _) ->
      ...
      let env =
        match path with
        | None -> env
        | Some m -> cons_tconstr_env (Pdot(m, Ident.name id)) env
      in
      env, `Type(constr_of_string s, ty, rec_status)
  | Sig_typext(id,ext,st,_) ->
      if st = Text_next then unsupported "Text_next";
      let ext_row_path = Type.LId (constr_of_string @@ name_of_ident id) in
      let env,(tc,params,ext_row_args,ext_row_ret) = from_types_extension_constructor ~tenv ~env ext in
...
and from_types_class_type ~env ~tenv:_ _cty =
  env, typ_class

and from_types_module_type ~env ~tenv mty =
  (*  if not dummy then Dbg_orig.printf "from_types_module_type: %a@." Printtyp.modtype mty;
  Cache.mod_type (Pair.pair env) mty snd (fun mty ->*)
  match Env.scrape_alias tenv (Mtype.scrape tenv mty) with
  | Mty_ident path when Ident.global (P.head path) -> [%unsupported]
  | Mty_ident path -> env, TConstr(normalized_path_of ~tenv path, [])
  | Mty_signature sgn ->
  ...
  | Mty_functor _ as mty ->
      ...
      let m =
        match id with
        | None -> Id.make ~env "_" ty1
        | Some id -> from_ident ~env id ty1
      in
      let env, ty2 = from_types_module_type ~env ~tenv mty2 in
      env, TFun(m, ty2)
  | Mty_alias path ->
      try
        from_types_module_type ~env ~tenv (Env.find_module path tenv).md_type
      with Not_found ->
        env, TConstr(normalized_path_of ~tenv path, [])(* ) *)

and from_typedtree_signature ~env ~tenv (sgn:Typedtree.signature) =
  from_types_signature ~env ~tenv sgn.sig_type

and from_typedtree_module_type ~env ~tenv mty =
  from_types_module_type ~env ~tenv mty.mty_type(*
...
and from_module_type_declaration ~env ~tenv ({mtd_name; mtd_type; _} : Typedtree.module_type_declaration) =
...
and from_modtype_declaration ~env ~tenv ({mtd_type; _} : Types.modtype_declaration) =
  match mtd_type with
  | None -> env, Ty.abst
  | Some mty -> from_types_module_type ~env ~tenv mty

and from_module_expr_top ~env mb_id_opt mb_expr : env * id option * term =
  let env,mdl =
    tmp_remove_prefix_target_exn mb_id_opt
      (fun () -> from_module_expr ~env mb_expr)
  in
...
and from_module_expr ~env mb_expr : env * term =
  ...
  | Tmod_structure struc ->
      ...
      let id_env =
        let map = List.flatten_map (function Syntax.Decl_let defs -> List.map fst defs | _ -> []) decls in
        map @ env.id_env
      in
      {env with id_env}, mdl
  | Tmod_ident(path,_loc) ->
      ...
      let hd = P.head path in
      if Ident.global hd then
        let m = Lid.head lid in
        let typ = TModLid (path_of_lid lid) in
        let m' = Id.set_typ m typ in
        let lid' = Lid.subst_head [m', LId m'] lid in
        let env,_ = module_type_of_ident ~env ~tenv hd in (* for add_mod_typ *)
        env, Term.var_lid ~typ lid'
      else
        env, Term.var_lid lid
  | Tmod_functor _ as desc ->
      let arg,expr = decomp_Tmod_functor desc in
      let env,m =
        match arg with
        | Unit ->
            env, Id.new_var ~env ~name:"U" (TModSig empty_signature)
        | Named(id, _, mty) ->
...
and from_types_extension_constructor ~tenv ~env ext =
  let from_ty = from_type_expr ~visited:[] ~tenv in
  let ext_row_path = normalized_path_of ~tenv ext.ext_type_path in
...
and from_typedtree_extension_constructor ~tenv ~env ext_constr : env * (constr * _) =
  ...
  | Text_rebind(path,_) ->
      let p = normalized_path_of ~tenv path in
      env, (s, `Ext_rebind p)
...
and from_str_item env str_item : env * decl list =
  ...
  | Tstr_value(Nonrecursive,pats) ->
      ...
      let decl,bvs =
        if List.exists (fst |- function `Match _ -> true | `Var _ -> false) decls then
          Decl_let_match (List.map (Pair.map_fst @@ function `Match p -> p | `Var x -> Pat.var x) decls),
          List.rev_flatten_map (fst |- function `Match p -> get_bv_pat p | `Var x -> [x]) decls
        else
          Decl_let (List.map (Pair.map_fst @@ function `Var x -> x | _ -> assert false) decls),
          List.map (fst |- function `Var x -> x | _ -> assert false) decls
      in
      check_shadow bvs env,
      [decl]
  | Tstr_primitive v ->
  ...
  | Tstr_type(rec_flag,decls) ->
      ...
      let decl =
        match flag with
        | Nonrecursive -> Decl_type_nonrec decls'
        | Recursive -> Decl_type decls'
      in
      env, [decl]
  | Tstr_typext ext ->
      let env,params = from_list ~f:(fun ~env (p,_) -> from_core_type ~tenv ~env p) ~env ext.tyext_params in
      assert (List.for_all Is'._TVar params);
      let env,rows =
        from_list ext.tyext_constructors ~env
          ~f:(fun ~env constr ->
            let env,(s,kind) = from_typedtree_extension_constructor ~tenv ~env constr in
            match kind with
            | `Ext_decl(ext_row_args, ext_row_ret) -> env, {ext_row_path = LId s; ext_row_args; ext_row_ret}
            | _ -> assert false)
      in
      let s = normalized_path_of ~tenv ext.tyext_path in
...
let from_top_level_phrase (tenv,(env,decls)) ptop : Env.t * (env * decl list) =
  match ptop with
  | Parsetree.Ptop_dir _ -> unsupported "toplevel_directive"
  | Parsetree.Ptop_def struc ->
      let struc',tenv' = Typemod.type_structure tenv struc in
...
let rec col_ext tenv (summary : Env.summary) =
  match summary with
  | Env_empty -> []
  | Env_extension(summary,id,ext) -> (id, (lid_of_string (name_of_ident id), ext)) :: col_ext tenv summary
  ...
  | Env_open(summary,path) ->
      col_ext_mod_decl tenv (normalized_path_of ~tenv path) (Env.find_module path tenv) @ col_ext tenv summary
  ...
  | Env_persistent(summary,_) -> col_ext tenv summary
...
let parse_lexbuf ?tenv ?env lb =
  ...
  tenv, orig, env, trans_decls env fdecls

let parse_file ?tenv ?env wrap filename =
  let mod_name =
    filename
    |> Filename.basename
  ...
  let lb =
    ...
    |&wrap&> Format.sprintf "module %s = struct\n%s\nend" mod_name
    |> Lexing.from_string
  in
  lb.Lexing.lex_curr_p <- {pos_fname = filename; pos_lnum = 1; pos_cnum = 0; pos_bol = 0};
...
let make_tdecl tdecls t =
  let tdecls' =
    tdecls
    |> List.filter_out (function (s, (_, TConstr(s', _))) -> s = s' | _ -> false)
    |> List.sort_unique (Compare.on fst)
    |> List.map (function (Type.LId s, ty) -> s, ty | (p,ty) -> Id.new_var ~name:("TEST." ^ string_of_path p) (), ty)
  in
  t
    ...
    | Syntax.Decl_type decls ->
        ...
        |> List.flatten_map (fun (s,(params,ty)) ->
              ...
              | TVariant(VNonPoly,rows) ->
                  let rows',decls =
                    ...
                    |> List.split_map (fun row ->
                        match row.row_args with
                        | [TRecord fields] ->
                            let c' = constr_of_string (Id.name s ^ "." ^ Id.name row.row_constr) in
              ...
              | _ ->
                [s, (params, ty)])
        |> _Decl_type
    | Type_ext (Ext_type(s, (params, {ext_row_path; ext_row_args = [TRecord _ as ty]; ext_row_ret}))) ->
        assert (ext_row_ret = None);
        ...
        let decl2 = Type_ext (Ext_type(s, (params, {ext_row_path; ext_row_args = [ty']; ext_row_ret}))) in
...
type orig = Parsetree.toplevel_phrase list
type ext_typ = Problem.ext_typ list
type ext_exc = Problem.ext_exc list
...
and exts =
  {ext_typ : ext_typ;
   ext_exc : ext_exc;
   ext_mod_typ : ext_mod_typ}

let ext_of ~env ~tenv parsed =
  let ext_mod_typ = List.map (fun (_,(x,ty)) -> Id.set_typ x (), ty) env.mod_typ_env in
  let used_exn = col_exn_constr parsed in
  let decl_exn = col_exn_decl parsed in
  Dbg.printf "used_exn: %a@." Print.(list (path * __)) used_exn;
  let used_exn =
    let rec find_used (p,x) =
      match x with
      | `Used tys -> tys
      | `Rebind -> find_decl p
    and find_decl p =
      match List.assoc ~eq:Path.eq p decl_exn with
      | `Decl tys -> tys
      | `Rebind p' -> find_decl p'
      | exception Not_found ->
          match p with
          | LDot(m, s) ->
              let env = Tenv.env_of_ext_mod ext_mod_typ in
              let sgn = sig_of_mod_typ ~env:ext_mod_typ @@ Tenv.assoc_value ~env m in
              let _,(ps,row) = List.find (fun (c,(_,row)) -> C.is_exn c && Path.to_string row.ext_row_path = s) sgn.sig_ext in
              assert (ps = []);
              row.ext_row_args
          | _ -> assert false
    in
    List.map (fun (p,x) -> p, find_used (p,x))  used_exn
  in
  let all_exn =
    Dbg.printf "used_exn: %a@." Print.(list (path * list typ)) used_exn;
    Dbg.printf "decl_exn: %a@." Print.(list (path * __)) decl_exn;
    List.map fst used_exn @@@ List.map fst decl_exn
    |> List.map Path.to_string
    |> List.sort_unique compare
  in
  if !Flag.Method.target_exn <> [] then
    begin
      let unused = List.Set.(!Flag.Method.target_exn - all_exn) in
      if unused <> [] then
        failwith "Missing target exceptions: %a@." (print_list Format.pp_print_string ", ") unused;
    end;
  if !Flag.Method.targets <> [] then
    failwith "Unused target: %a@." (print_list Format.pp_print_string ", ") !Flag.Method.targets;
  if !Flag.Print.exn then List.iter (Format.fprintf !Flag.Print.target "%s@.") all_exn;
  let tconstrs = List.map fst @@ get_tconstr parsed in
  let _,decls = make_ty_decl tconstrs ~env ~tenv ~from_types_type_declaration in
  let mark = mark_free_as_external_typ (List.map fst decls) in
  let ext_typ = Fmap.(list (snd (snd mark))) decls in
  let ext_exc =
    used_exn
    |> List.filter_out (fun (c,_) -> List.mem_assoc ~eq:Path.eq c decl_exn)
    |> Fmap.(list (snd (list mark)))
  in
  {ext_typ; ext_exc; ext_mod_typ}

let parse_files filenames : result =
  ignore @@ Warnings.parse_options false "-a";
  let init_tenv = !!Compmisc.initial_env in
  let dirs = Load_path.get_paths () in
  List.iter (Load_path.add_dir -| Filename.dirname) !Flag.IO.filenames;
  let origs,tenv,env,cont =
    List.L.fold_right filenames ~init:([],init_tenv,init_env,Fun.id)
      ~f:(fun filename (origs,tenv,env,cont) ->
        let wrap = List.hd filenames <> filename in
        let tenv,orig,env,decls = parse_file ~tenv ~env wrap filename in
