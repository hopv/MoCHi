...
let to_base_env env =
  let rec aux (x, ty) =
    ...
    | RT.Base _ -> [ (x, ty) ]
    | RT.Tuple xtys ->
        xtys |> List.map (Pair.map_fst @@ Id.add_name_after "'") |> List.rev_flatten_map aux
    | _ -> []
  in
  env |> RT.Env.to_list |> List.flatten_map aux |> RT.Env.of_list
...
let rec lift_ty env ?np sty =
  ...
  | TBase base ->
      ...
      let pred =
        let name =
          Option.map
            (fun (name, path) -> String.join ":" @@ (name :: List.rev_map string_of_int path))
            np
        in
...
let lift env ?name t =
  ...
  let r =
    if is_simple_expr t then simple_expr_ty t
    else lift_ty env ?np:(Option.map (Pair.pair -$- []) name) t.typ
  in
...
let print_sub_constrs fm (cs : sub_constr list) =
  let pr fm (env, (rty1, rty2)) =
    ...
    Format.fprintf fm "@[<hov 2>%a |-@ @[%a <:@ %a@]@]" RT.Env.print env' RT.print rty1 RT.print
      rty2
  in
  List.print pr fm cs
...
let const_ty c =
  ...
  | Rand (TBase TInt, false) -> RT.Ty.(fun_ !!unit !!int)
  | Rand (TBase TInt, true) -> RT.Ty.(fun_ !!unit (fun_ (fun_ !!int result) result))
  | _ ->
...
let rec gen_sub env t ty : sub_constr list =
  ...
  | _ when is_simple_expr t ->
      ...
      if RT.equiv aty ty then [] else [ (env, (aty, ty)) ]
  ...
  | Var (LId x) ->
      ...
      if RT.equiv ty' ty then [] else [ (env, (ty', ty)) ]
  ...
  | Local (Decl_let bindings, t1) ->
      ...
      let env0 =
        List.fold_right2
          (fun (x, _) ty env -> if is_fun_var x then RT.Env.add x ty env else env)
          bindings tys env
      in
      let env1 = List.fold_right2 (fst |- RT.Env.add) bindings tys env in
      let sub = gen_sub env1 t1 ty in
      let aux (_x, t) ty = gen_sub env0 t ty in
      ...
      let aty =
        ...
        let atys =
          List.L.map xs ~f:(fun x ->
              if is_base_typ @@ Id.typ x then simple_expr_ty (Term.var x) else RT.Env.assoc x env)
        in
  ...
  | Tuple ts ->
      let t' =
        let xs =
          List.map
            (fun t ->
              match Syntax.Val._Var t with None -> new_var_of_term t | Some x -> Lid.id_of x)
            ts
        in
  ...
  | Proj (i, { desc = Var (LId x) }) ->
      let env', aty =
        ...
        | RT.Tuple xtys ->
            ...
            (RT.Env.of_list @@ List.rev xtys1, snd @@ List.hd xtys2)
        | _ -> assert false
      in
...
let rec flatten env (ty1, ty2) =
  ...
  | RT.Base (b1, x, _), RT.Base (b2, _, _) ->
      if b1 <> b2 then (
        Format.eprintf "%a cannot unify with %a@." RT.print ty1 RT.print ty2;
        invalid_arg "flatten");
      let p = denote_ty x ty2 in
      if p.desc = Const True then [] else [ (denote_ty x ty1 :: denote_env env, denote_ty x ty2) ]
  | RT.Fun (x1, ty11, ty12), RT.Fun (x2, ty21, ty22) ->
      flatten env (ty21, ty11) @ flatten (RT.Env.add x2 ty21 env) (RT.subst_var x1 x2 ty12, ty22)
  ...
  | RT.Constr (s1, [], x, _), RT.Constr (s2, [], _, _) ->
      if s1 <> s2 then (
        Dbg.printf "s1: %s@.s2: %s@." (Path.name s1) (Path.name s2);
        invalid_arg "flatten");
      let p = denote_ty x ty2 in
      if p.desc = Const True then [] else [ (denote_ty x ty1 :: denote_env env, denote_ty x ty2) ]
  | _ ->
  ...
  let wrap =
    Trans.map_var (fun x ->
        ...
        if
          ...
          || String.fold_left
               (fun acc c ->
                 acc && (Char.is_letter c || Char.is_digit c || String.mem c "~!@$%^&*_-+=<>.?"))
               true name
        then x
        else Id.make ("|" ^ name ^ "|") @@ Id.typ x)
  in
...
let check env t ty =
  ...
  try Rec_CHC_solver.check_sat hcs with _ -> false

let print cout env t ty =
  let hcs = gen_hcs env t ty in
  let filename = Temp.S.ext "smt2" in
