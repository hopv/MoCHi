...
let rec generate_check typ_exn make_fail genv cenv x typ =
  ...
  | Fun(y, typ1, Exn(typ2, typ3)) ->
      begin
        ...
        | Some typ_exn' ->
            ...
            genv''', cenv''', U.Term.(let_ [z,t_typ1] (trywith_h t' (fun_ e t_typ3')))
      end
  | Fun(y,typ1,typ2) ->
      let genv',cenv',t_typ1 = generate typ_exn make_fail genv cenv typ1 in
  ...
  | Tuple xtyps ->
      ...
      in
      genv', cenv', U.make_lets (List.mapi (fun i x' -> x', U.Term.(proj i (var x))) xs') @@ U.make_ands ts
  | List(l,p_len,y,p_i,typ1) when p_i.S.desc = S.Const S.True && not @@ occur y typ1 ->
      ...
      let add_len t =
        let t' = U.Term.(t && (l |-> var l') p_len) in
        if t' = U.true_term
        then U.true_term
        else U.Term.(let_ [l', length (var x)] t')
      in
        ...
        then
          ...
          then
            ...
            let t_body =
              ...
              let pat_cons =
                let t_b2 = U.Term.(var f @ [var zs']) in
                U.Pat.(cons (var z) (var zs')), U.Term.(t_b1 && t_b2)
              in
  ...
  | Union(_, typs) ->
      let aux (genv',cenv',ts) typ =
        ...
        let t' =
          ...
          | Some typ_exn' ->
              let e = Id.new_var ~name:"e" typ_exn' in
              U.Term.(trywith_h t (fun_ e false_))
        in
        genv'', cenv'', t'::ts
      in
      let genv'',cenv'',ts = List.fold_left aux (genv,cenv,[]) typs in
...
and generate typ_exn make_fail genv cenv typ =
  ...
  let genv',cenv',t =
    ...
    | Base(T.TInt, _, _) ->
        ...
        genv', cenv', U.Term.(let_ [x',randi] (assume t_check (var x')))
    | Base(T.TBool, _, _) ->
        let x' = Id.new_var T.Ty.bool in
        let genv',cenv',t_check = generate_check typ_exn make_fail genv cenv x' typ in
        genv', cenv', U.Term.(let_ [x',randb] (assume t_check (var x')))
    | Base(T.TUnit, x, _) ->
        let genv',cenv',t_check = generate_check typ_exn make_fail genv cenv x typ in
        genv', cenv', U.Term.(assume t_check unit)
    | Base(T.TPrim _, _, _) ->
    ...
    | Inter(_, ((Fun(_,atyp,rtyp))::_ as typs)) when !Flag.Modular.use_ref_typ_gen_method_in_esop2017 ->
        ...
        let rec wrap typ (genv,cenv,v) =
          ...
          | Exn(typ1,typ2) ->
              ...
              genv'', cenv'', U.Term.trywith_h t1 handler
          | _ ->
        ...
        let genv',cenv',tbs =
          let aux typ1 (genv,cenv,tbs) =
            ...
            let tb' =
              ...
              U.Term.(randb || trywith tb e [U.make_pany (Id.typ e), false_])
