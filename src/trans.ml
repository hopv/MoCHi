...
let map_var : (typ Id.t -> term tid) -> term -> term =
  let tr = Tr2.make () in
...
let remove_var_let : term -> term =
  ...
  tr.desc <- (fun desc ->
    ...
    | Local(Decl_let [x, t], {desc=Var y}) when Lid.(LId x = y) && not (Id.List.mem x (get_fv t)) -> t.desc
    | desc' -> desc');
  tr.term
...
let alpha_rename : ?whole:bool -> ?set_counter:bool -> term -> term =
  ...
  let tr = Tr2.make () in
  tr.desc <- (fun (whole,cnt,names) desc ->
    let new_id x =
      if whole then
        let x' =
          let rec fresh var =
    ...
    | Local(Decl_let bindings, t1) ->
        ...
        let map' = Fmap.(list (snd _LId)) map in
        let sbst = subst_var_map_without_typ map' in
        let bindings' = List.map2 (fun (_,f') (_,t) -> f', sbst t) map bindings in
        Local(Decl_let bindings', sbst t1)
    | Fun(x, t1)->
        let x' = new_id x in
        Fun(x', subst_var_without_typ x (_LId x') t1)
    | Match(t1,pats) ->
        let aux (p,t2) =
...
let rename_tvar,rename_tvar_var,rename_tvar_typ =
  ...
  tr.typ <- (fun map typ ->
    match typ with
    | TVar(b, ({contents=None} as x), n) when List.mem_assq x map -> TVar(b, List.assq x map, n)
...
let rec define_rand ?(name="") (env, defs as ed) typ =
  ...
  | None ->
      ...
      | TVar(_, ({contents=None} as r),_) -> r := Some Ty.int; define_rand ed Ty.int
      ...
      | _ ->
          unsupported "define_rand: %a" Print.typ typ
  ...
  let rec aux apply t =
    let desc =
      ...
      | Var (LId x) ->
          begin
            try
              Local(Decl_let [x, Id.List.assoc x apply], make_var x)
...
let replace_typ_var env x =
  Id.set_typ x @@ List.assoc_default ~eq:Id.eq (Id.typ x) x env
        ...
        let aux (f,t) =
          ...
          if not @@ can_unify (Id.typ f) (Id.typ f') then
            begin
              ...
              failwith "Type of %a in %s is wrong? (please specify monomorphic types if polymorphic types exist)"
...
let elim_fun : term -> term =
  ...
  tr.term <- (fun fun_name t ->
    ...
    | Local(Decl_let bindings, t2) ->
        let aux (f,t) =
          let xs,t' = decomp_funs t in
          f, make_funs xs @@ tr.term ("f_" ^ Id.name f) t' in
        let bindings' = List.map aux bindings in
...
let rec inlined_f inlined fs t : term =
  let desc =
    ...
    | Const c -> Const c
    | Var (LId y) ->
        begin match List.find_opt (Triple.fst |- Id.eq y) fs with
        | Some (_, xs, t') ->
            let f, _ =
              List.L.fold_left xs
                ~init:(Fun.id, t.typ)
                ~f:(fun (f, ty) y ->
                    (fun t -> f (make (Fun(y, t)) ty)),
                    snd @@ Type.ValE._TFun ty)
            in
    ...
    | App(t1, ts) ->
        (match t1.desc with
         | Var (LId f) when List.exists (Triple.fst |- Id.same f) fs ->
             ...
             let f, _ =
               List.fold_left
                 (fun (f, ty) x -> (fun t -> f (make (Fun(x, t)) ty)), match ty with Type.TFun(_, ty') -> ty' | _ -> assert false)
    ...
    | _ -> Format.eprintf "inlined_f: %a@." Print.desc_constr t.desc; assert false
  in
  make desc t.typ
...
let rename_ext_funs : id list -> term -> id list * term =
  let fld = Fold_tr.make () in
  fld.desc <- (fun (funs,env,map) desc ->
    match desc with
    ...
    | Var (LId _) -> fld.desc_rec (funs,env,map) desc
    | Var _ -> assert false
    | Local(Decl_type decls, _) ->
        let env' = Tenv.add_typ_decl decls env in
  ...
  fld.term <- (fun env t ->
    ...
    map, make desc t.typ);
  fun funs t ->
    let (_,_,map),t' = fld.term (funs,Tenv.empty(),[]) t in
    map, t'
...
let make_ext_fun_def f =
  ...
  let make_fun_arg_call x (env,defs,t) =
    ...
    let (env',defs'),args =
      List.L.fold_right_map xs ~init:(env,defs)
        ~f:(fun x (env,defs) -> define_rand (env, defs) @@ Id.typ x)
    in
    let t =
      if xs = []
      then t
      else [%term () |+| ignore (x @ `args); `t]
    in
    env', defs', t
  in
...
*)
let make_ext_fun_def f = f, make_rand_unit (Id.typ f)

let remove_type_decl : term -> term =
  let tr = Tr.make () in
  tr.desc <- (fun desc ->
    match desc with
    | Local(Decl_type _, t1)
    | Local(Type_ext(Ext_type _), t1) -> tr.desc t1.desc
    | _ -> tr.desc_rec desc);
  tr.term

let lift_type_decl ?(decl_ext=[]) t =
  let decls =
    decl_ext :: col_type_decl t
    |> List.flatten
    |> List.L.fold_right ~init:[]
         ~f:(fun (s,(params,ty)) acc ->
           ...
           | Some (_,ty') ->
               if not @@ Type_util.same_shape ty ty' then
                 Format.eprintf "[%a] Assume %a is the same as %a@." Print.tconstr s Print.typ ty Print.typ ty';
               acc)
  in
  t
  |> remove_type_decl
...
let make_ext_funs ?(fvs=[]) env t : term =
  ...
  let (genv,cenv),defs2 =
    List.L.fold_right_map env ~init:([],[])
      ~f:(fun (f,typ) (genv,cenv) ->
...
let assoc_typ : id -> term -> typ =
  ...
  fun f t ->
    ...
    | [] -> raise Not_found
    | [typ] -> typ
    | _ -> Dbg.printf "VAR:%a@.PROG:%a@." Id.print f Print.term t; assert false

let inline_no_effect : term -> term =
  let tr = Tr.make () in
  tr.desc <- (fun desc ->
    match desc with
    | Local(Decl_let [x,t], {desc=Var y}) when Lid.(LId x = y) && not @@ Id.List.mem x @@ get_fv t ->
        (tr.term t).desc
    | Local(Decl_let [x,t], t2) when Id.List.mem x (get_fv t2) && has_no_effect t && not @@ Id.List.mem x @@ get_fv t ->
        let t' = tr.term t in
        let t2' = tr.term t2 in
        (subst x t' t2').desc
    | _ -> tr.desc_rec desc);
...
let normalize_let : ?is_atom:(term -> bool) -> term -> term =
  ...
  tr.term <- (fun is_atom t ->
    ...
    | If(t1, t2, t3) ->
        ...
        let add_attrs' =
          List.filter_out (function AFV _ -> true | _ -> false) t.attr
          |> add_attrs
        in
...
let remove_label : ?label:string -> term -> term =
  ...
  tr.term <- (fun label t ->
    ...
    | None, Label(_, t1) -> tr.term label t1
    | Some l, Label(InfoString l', {desc=Label(_, t1)}) when l = l' -> tr.term label t1
    | Some l, Label(InfoString l', t1) when l = l' -> tr.term label t1
    | _ -> tr.term_rec label t);
  fun ?(label="") t -> tr.term (if label="" then None else Some label) t
...
let elim_unused_let : ?leave_last:bool -> ?cbv:bool -> ?elim_may_div:bool -> term -> term =
  let tr = Tr2.make () in
  let has_no_effect' t = has_no_effect t || has_safe_attr t in
  tr.term <- (fun (leave,cbv,div) t ->
    let t' = tr.term_rec (leave,cbv,div) t in
    match t'.desc with
    | Local(Decl_let [], _) -> assert false
    | Local(Decl_let bindings, t1) when
            not (List.mem ADoNotInline t.attr) &&
              not (is_mod_var (fst (List.hd bindings))) (* WORKAROUND: Must see the occurences of modules in types *) ->
        ...
        let used =
          ...
          in
          bindings
          |> List.fold_left (fun dep (f,t) -> IdMap.add f (IdSet.of_list @@ get_fv_unique t) dep) IdMap.empty
          |> aux fv fv
        in
        let necessary (f,t) =
          Id.List.mem f leave ||
        ...
        in
        let bindings' = List.filter necessary bindings in
  ...
  fun ?(leave_last=false) ?(cbv=true) ?(elim_may_div=false) t ->
    let leave =
      if leave_last then
        List.map (fst |- Lid.id_of) @@ get_last_definition t
      else
        []
    in
    tr.term (leave,cbv,elim_may_div) t

let subst_with_rename_map : ?check:bool -> (id * term) list -> term -> term =
  let tr = Tr2.make () in
  tr.desc <- (fun map desc ->
    if map = [] then
      desc
    else
      match desc with
      | Var y ->
          begin match List.find_opt (fun (x,_) -> Lid.(LId x = y)) map with
          | None -> desc
      ...
      | Local(Decl_let bindings, t2) when is_non_rec bindings ->
          let bindings' = Fmap.(list (tr.var map * tr.term map)) bindings in
          let t2' =
            let map' = List.filter_out (fun (x,_) -> List.exists (fst |- Id.eq x) bindings) map in
            tr.term map' t2
          in
          Local(Decl_let bindings', t2')
      | Local(Decl_let bindings, t2) ->
  ...
  tr.term <- set_afv_shallowly --| tr.term_rec;
  tr.attr <- (fun map attrs ->
    let sbst y =
      match Id.List.assoc_opt y map with
      | None -> [y]
      | Some (_,fv) -> fv
    in
    List.map (function AFV xs -> AFV (List.rev_flatten_map sbst xs) | a -> a) attrs);
  fun ?(check=false) map t2 ->
    if check && List.for_all (fst |- count_occurrence -$- t2 |- (=) 1) map
    then subst_map map t2
...
let elim_unused_branch : term -> term =
  ...
  tr.term <- (fun t ->
    ...
    | If({desc=BinOp(Eq,{desc=Var x},{desc=Var y})}, t1, _) when Lid.(x = y) -> tr.term t1
    | _ -> tr.term_rec t);
  tr.term
...
let replace_base_with_int,replace_base_with_int_typ =
  ...
  tr.desc <- (fun desc ->
    ...
    | Const(String _ | Float _ | Int32 _ | Int64 _ | Nativeint _ as c) -> (** [format] must be abstracted in advance *)
  ...
  tr.typ <- (fun typ ->
    match typ with
    | TConstr(c, []) when is_base_prim c -> Ty.int
    | _ -> tr.typ_rec typ);
  tr.pat <- (fun p ->
    match p.pat_desc with
    | PConst {desc=Const(Char _|String _|Float _|Int32 _|Int64 _|Nativeint _)} ->
        make_pnondet (TBase TInt)
    | _ -> tr.pat_rec p);
  tr.term, tr.typ
  ...
  tr.term <- (fun t ->
    ...
    | App(t1, t2::ts) ->
        begin
          ...
          | {desc=Fun(x,t1')} -> tr.term @@ make_app (subst_with_rename ~check:true x t2 t1') ts
...
let flatten_tuple : term -> term =
  ...
  tr.term <- (fun t ->
    ...
    | Proj(i,t1) ->
        ...
        let ns = List.map (fun typ -> match tr.typ typ with TTuple xs' -> List.length xs' | _ -> 1) @@ decomp_ttuple t1.typ in
...
let inline_next_redex : term -> term =
  ...
  let can_inline x t =
    ...
    Option.default false @@ is_in_redex (LId x) t
  in
  tr.term <- (fun t ->
    match t.desc with
    | Local(Decl_let [x,t1], t2) when is_non_rec [x,t1] ->
  ...
  tr.term <- (fun env t ->
    ...
    | Local(Decl_let [x,({desc=Tuple ts} as t1)], t2) when is_non_rec [x,t1] ->
        ...
        if List.for_all Option.is_some xs then
          ...
          tr.term_rec env t
    | Proj(i,{desc=Var (LId x)}) when Id.List.mem_assoc x env -> make_var_lid @@ List.nth (Id.List.assoc x env) i
    | _ -> tr.term_rec env t);
  tr.term []
...
let beta_no_effect_tuple : term -> term =
  ...
  tr.term <- (fun env t ->
    ...
    | Local(Decl_let [x,({desc=Tuple ts} as t1)], t2) when is_non_rec [x,t1] ->
        ...
        then make_let [x,t1] @@ tr.term ((x,ts)::env) t2
        else tr.term_rec env t
    | Proj(i,{desc=Var (LId x)}) when Id.List.mem_assoc x env -> List.nth (Id.List.assoc x env) i
    | _ -> tr.term_rec env t);
  tr.term []
...
let rename_poly_values : ?env:env -> id list -> term -> (id * typ Id.t) list list * term =
  ...
  let fld_list focus fs map env xs =
    List.L.fold_right_map xs ~init:map
      ~f:(fun x map ->
  ...
  fld.term <- (fun (fs,map,env as acc) t ->
    match t.desc with
    ...
    | Match(t1, pats) ->
        ...
        let acc,pats' =
          Fold_tr.list_left ~acc pats
            ~f:(fun (fs,map,env) (pat,t1) ->
  ...
  fld.pat <- (fun (fs,map,env as acc) p ->
    let acc,desc =
      match p.pat_desc with
      | PWhen(p1, t) ->
  ...
  fun ?(env=Tenv.empty()) fs t ->
    let (_,map,_),t' = fld.term (fs,[],env) t in
    map, t'

let copy_poly_ext_funs ?eq t : term =
  let eq =
    match eq with
    | None ->
        let env = Tenv.add_typ_decl (List.flatten @@ col_type_decl t) !!Tenv.empty in
  ...
  in
  map_var (fun x -> List.assoc_default ~eq x x fv_map) t


let copy_poly_values : term -> term * ((id -> Ref_type.t) -> id -> Ref_type.t) =
  let fld = Fold_tr.make () in
  fld.desc <- (fun (env,map) desc ->
    match desc with
    | Local(Decl_let bindings, t2) when List.exists (is_poly_typ -| Id.typ -| fst) bindings ->
        ...
        let map_renames,t2 =
          ...
          | [map_rename] -> [List.map (fun (f,g) -> f, Id.set_id g (Id.id f)) map_rename], t2
          | _ -> map_renames, t2_renamed
        in
        Dbg.printf "@[<hv 2>COPY%a:@ %a@.@." Print.(list id_typ) fs Print.(list (list (__*id_typ))) map_renames;
        let map,bindingss =
          List.L.fold_left map_renames
            ~init:(map,[])
            ~f:(fun (map,bindingss) map_rename ->
              let _,bindings' =
                List.L.fold_right_map bindings
                  ~init:[]
                  ~f:(fun (x,t) tvar_env ->
              ...
              let map,bindings =
                List.L.fold_right_map bindings' ~init:map
                  ~f:(fun (x,t) map ->
                    ...
                    (try
                       unify_app_args t'
                     with CannotUnify _ -> unsupported "Polymorphic use in let ... and ..." (* TODO: See TODO.md *));
...
let get_set_main t : lid option =
  match List.decomp_snoc_opt @@ get_last_definition t with
  | Some (_, (x, _)) when Lid.name x = main_name -> Some x
  | _ -> None
...
let set_main_for ?(force=false) targets t : term =
  let catch_all = make_catch_all t in
...
let ignore_non_termination : term -> term =
  let tr = Tr2.make () in
  tr.desc <- (fun fail_free desc ->
    match desc with
    | App({desc=Var (LId f)}, ts) when Id.List.mem f fail_free && can_unify Ty.unit (make_app (make_var f) ts).typ->
        Const Unit
    | Local(Decl_let bindings, t1) ->
        ...
        let bindings'' = List.filter_out (fun (f,_) -> Id.List.mem f fail_free' && not @@ Id.List.mem f fv) bindings' in
...
let beta_full_app : id * id list * term * (term -> term) -> term -> term =
  let tr = Tr2.make () in
  tr.term <- (fun (f,xs,t1,cont as arg) t2 ->
    match t2.desc with
    | App({desc=Var (LId g)}, ts) when Id.(f = g) && List.length xs = List.length ts ->
        subst_with_rename_map ~check:true (List.combine xs ts) t1
        |> cont
...
let beta_affine_fun : term -> term =
  ...
  tr.term <- (fun t ->
    ...
    match t.desc with
    | Local(Decl_let [f, ({desc=Fun _} as t1)], t2) when not (List.mem ADoNotInline t.attr) && is_non_rec [f,t1] ->
        begin
          match decomp_funs t1 with
          | xs, ({desc=App _} as t1) ->
              let used = Id.List.Set.(get_fv t1 && xs) in
              let not_rand_int = (* for non-termination *)
                match t1.desc with
                | App({desc=Const(Rand(TBase TInt,_))}, _) -> false
                | _ -> true
              in
              if Id.List.is_unique used &&
                 not_rand_int &&
                 count_occurrence f t2 <= 1
              then
                let t2' = beta_full_app (f, xs, t1, tr.term) t2 in
                if Id.List.mem f @@ get_fv t2' then
                  Term.(let_ [f, funs xs t1] t2')
                else
                  t2'
              else
                t
...
let reconstruct : term -> term =
  ...
  tr.term <- (fun t ->
    let t' =
      ...
      | Local(Decl_let bindings, t) -> make_let (List.map (Pair.map_snd tr.term) bindings) @@ tr.term t
      ...
      | Field(t1, s) -> make_field (tr.term t1) s
      | TryWith(t1, {desc=Fun(x,{desc=Match({desc=Var (LId y)},pats)})}) when Id.(x = y) -> make_trywith t1 x pats
      | _ -> tr.term_rec t
    in
...
let reduce_fail_unit : term -> term =
  ...
  tr.term <- (fun t ->
    let t = tr.term_rec t in
    match t.desc with
    | Local(Decl_let [_,({desc=App({desc=Event("fail",false)},[{desc=Const Unit}])} as t1)], _) -> t1
    | _ -> t);
  tr.term
...
exception AllArgumentsRemoved (* WORKAROUND: we must support the case that all the arguments can be removed *)

let elim_redundant_arg : term -> term =
  let tr = Tr2.make () in
  tr.desc <- (fun (vars:id list) desc ->
    match desc with
    | Local(Decl_let [f,t1], t) when not @@ is_non_rec [f,t1] ->
        begin
          let xs,t1 = decomp_funs t1 in
          let fixed_args = find_fixed_args (LId f) (List.map Lid._LId xs) t1 in
          let default () = Local(Decl_let [f, make_funs xs @@ tr.term_rec (xs@vars) t1], tr.term_rec vars t) in
          if fixed_args = [] then
            !!default
          else
            let ixs = List.filter_mapi (fun i x -> if List.mem ~eq:Lid.eq (LId x) fixed_args then Some (i,x) else None) xs in
            try
              ...
              | args::argss ->
                  ...
                  let redundant =
                    ...
                    |> List.filter (fun (i,_) ->
                           ...
                           | Some (LId x) ->
                               Id.List.mem x vars &&
                                 List.for_all (fun args'' -> Option.exists Lid.((=) (LId x)) @@ List.nth args'' i) argss'
                           | Some _ -> assert false
                           | exception _ -> false)
                  in
...
let inline_record_type : term -> term =
  ...
  tr.desc <- (fun desc ->
    match desc with
    | Local(Decl_type decls, t) ->
        let cs = List.rev_flatten_map (snd |- snd |- get_tconstr_typ |- List.filter_map (fst |- Type.Val._LId)) decls in
        ...
        let decls1,decls2 = List.partition (function (_,(_,TRecord _)) -> true | _ -> false) decls in
        t
        |> tr.term
        |> Term.type_ decls2
...
let remove_pnondet : term -> term =
  ...
  tr.pat <- (fun p ->
    ...
    match p'.pat_desc with
    | PNondet
    | POr({pat_desc=PAny},{pat_desc=PAny}) -> Pat.__ p'.pat_typ
    | _ -> p');
  tr.term <- set_afv_shallowly -| tr.term_rec;
  tr.term
...
let simplify_match : term -> term =
  ...
  tr.term <- (fun t ->
    ...
    | Match(t1, pats) ->
        ...
        let pats' =
          pats
          |> List.map (Pair.map simplify_pattern tr.term)
          |> elim_unused
        in
        begin
          match pats' with
          | [] -> assert false
          | [{pat_desc=PAny}, t] -> [%term `t1; `t]
          | [{pat_desc=PConst {desc=Const Unit}}, t] -> [%term `t1; `t]
          | [{pat_desc=PVar x}, t] -> make_let [x, t1] t
          | [{pat_desc=PWhen({pat_desc=PVar x},t2)}, t3] ->
              [%term let x = `t1 in if `t2 then `t3 else bot [@ty t.typ]]
  ...
  tr.term <- (fun t ->
    ...
    | Match(t1, pats) ->
        ...
        let remove (p,t) acc =
          ...
          | PWhen({pat_desc=PVar y}, cond) -> [%term let y = x in if `cond then `t else `acc]
          | PWhen(_, cond) -> [%term if `cond then `t else `acc]
          | PLazy _ -> [%unsupported]
          | PEval _ -> [%unsupported]
        in
        List.fold_right remove pats (Term.bot t.typ)
        |> bind
    | _ -> t);
  tr.term
...
let split_type_decls_aux (decls : type_decls) : type_decls list =
  ...
  in
  let idx = topological_index Graph_wrapper.scc ~eq:Id.eq edges in
  decls
  |> List.map (fun (c,pty) -> - Id.List.assoc c idx, (c, pty))
...
let recover_pat_bv ~tr_typ ~before:(p_before,_) ~after:(p_after,t) =
  ...
  in
  let bindings = (* WORKAROUND: ~force:true (this must be removed if the attributes are correct) *)
    get_bv_pat ~force:true p_before
    |> Id.List.Set.diff -$- (get_bv_pat ~force:true p_after)
    |> List.map make
  in
  let p_after =
    match p_after.pat_desc with
    | PWhen(p1,cond) ->
        if has_pwhen p1 then [%unsupported];
...
let abst_recdata : string -> (typ -> type_decls list -> bool) -> term -> term =
  let tr = Tr2.make () in
  tr.term <- (fun (s,check,env as arg) t ->
    let desc =
      match t.desc with
      | Local(Decl_type decls, t) ->
          ...
          Local(Decl_type decls', t')
      | Match(_, _) ->
          trans_match_with_recover_pat_bv_desc ~tr_desc_rec:(tr.desc_rec arg) ~tr_typ:(tr.typ arg) t.desc
...
let replace_list_with_int : term -> term =
  ...
  tr.typ <- (fun ty ->
    match ty with
    | TConstr(c, _) when c = C.list -> Ty.int
    | _ -> tr.typ_rec ty);
  tr.pat <- (fun p ->
    match p.pat_desc with
    | PNil | PCons _ ->
...
let abst_ext_recdata : term -> term =
  let typ_not_in_env ty env =
    ...
    | TConstr(LId s,_) -> not (List.exists (Id.List.mem_assoc s) env) && not (is_prim_constr (Type.LId s))
    | TVariant _ -> false
    | _ -> true
  in
...
let replace_data_with_int =
  abst_recdata "Data with int" (fun ty _ -> not (is_prim_tconstr ty))

let replace_data_with_int_but tys : term -> term =
  abst_recdata "Data with int except specific types" (fun ty _ -> not @@ List.mem ty tys && not (is_prim_tconstr ty))

let replace_poly_variant_with_int : term -> term =
  let check ty env =
    let rec is_poly_variant = function
      | TConstr(LId s,_) ->
          List.find_map_opt (Id.List.assoc_opt s) env
          |> Option.exists (snd |- is_poly_variant)
...
let replace_abst_with_int : term -> term =
  abst_recdata "Data with int except specific types"
    (fun ty _ -> match ty with TConstr(c,[]) -> C.is_abst c | _ -> false)
...
let mark_free_as_external : term -> term =
  let tr = Tr2.make () in
  let is_bound_id bs = Id.List.mem -$- bs ||| Id.is_primitive in
  let rec is_bound_module bv (path:path) =
    match path with
    | LId c -> is_bound_id bv (Id.set_typ c Ty.unknown)
    | LDot(p, _)
    | LApp(p, _) -> is_bound_module bv p
  in
  let is_bound (bv,bt,_) (path:path) =
    match path with
    | LId c -> is_bound_id bt c || List.mem ~eq:Path.eq path C.prims
    | LDot(p, _)
    | LApp(p, _) -> is_bound_module bv p
  in
  tr.desc <- (fun (bv,bt,cache as env) desc ->
    match desc with
    | Var lid when not (is_bound env (path_of_lid lid)) -> Var (Lid.map ~f:(Id.add_attr Id.External) lid)
    | Constr(false, c, ts) when not (is_bound env c) ->
    ...
    | Fun(x, _) ->
        tr.desc_rec (x::bv,bt,cache) desc
    | Match(t1, pats) ->
  ...
  tr.pat <- (fun env p ->
    match p.pat_desc with
    | PConstr(false, c, ps) when not (is_bound env c) ->
        let c' = Path.map ~f:(Id.add_attr Id.External) c in
        let ps' = List.map (tr.pat env) ps in
        make_pattern ~attr:p.pat_attr (PConstr(false, c', ps')) p.pat_typ
    | _ -> tr.pat_rec env p);
  tr.lid <- (fun env lid ->
    if not (is_bound env (path_of_lid lid)) then
      Lid.map ~f:(Id.add_attr Id.External) lid
    else
      lid);
  tr.typ <- (fun (_,_,cache as env) ty ->
    match ty with
    | TConstr(c,tys) when not (is_bound env c) ->
        let c' = Path.map ~f:(Id.add_attr Id.External) c in
        tr.typ_rec env (TConstr(c',tys))
    | TModSig sgn ->
        begin match Hashtbl.find_option cache ty with
        | Some r -> r
        | None ->
            let decls =
              let values = List.map (fun x -> Decl_let [x, Term.dummy (Id.typ x)]) sgn.sig_values in
              let types = List.map _Decl_type sgn.sig_types in
              let exts = List.map (_Type_ext -| _Ext_type) sgn.sig_ext in
              (types @ exts @ values)
              |> trans_decls_as_term (tr.term env)
            in
            let sig_values = List.filter_map (function Decl_let [x,_] -> Some x | _ -> None) decls in
            let sig_types = List.filter_map Val._Decl_type decls in
            let sig_ext = List.filter_map Option.(Val._Type_ext >=> Type.Val._Ext_type) decls in
            let r = TModSig {sig_values; sig_types; sig_ext} in
...
let complete_precord : term -> term =
  let tr = Tr2.make () in
  tr.desc <- (fun decls desc ->
    match desc with
    | Local(Decl_type decls', _) ->
        tr.desc_rec (decls'@decls) desc
...
let instansiate_weak_poly_types : term -> term =
  ...
  tr.term <- (fun env t ->
    let uni ty1 ty2 =
      if is_weak_poly_typ ty1 || is_weak_poly_typ ty2 then (* WORDAROUND: [unify ~sub] must be used *)
    ...
    | Tuple ts ->
        ...
        in
        tr.term_rec env t
    | App({desc=Var (LId f)} as t1, ts) when List.exists (is_weak_poly_typ -| typ) (t1::ts) ->
        let rec unify ty ts =
    ...
    | Local(Decl_let (_::_::_ as bindings), t) ->
        ...
        let bindings' =
          if only_top then
            ...
            List.map (Pair.map_snd @@ tr.term only_top) bindings
        in
        let bindings'' =
...
let encode_bool_as_int : term -> term =
  ...
  tr.desc <- (fun desc ->
    ...
    | Not t ->
        let t' = boo_of_int @@ tr.term t in
        Term.(int_of_bool (not t')).desc
    | BinOp((And|Or as op), t1, t2) ->
        let op' = match op with And -> Term.(&&) | Or -> Term.(||) | _ -> assert false in
        let t1' = boo_of_int @@ tr.term t1 in
        let t2' = boo_of_int @@ tr.term t2 in
...
let remove_tattr : term -> term =
  {!!Tr.make with typ = elim_tattr_all}.term

(** [reduce_rand] reduces terms with [rand_unit] into more simple terms *)
(** E.g., [%term randi + 1] is replaced with [%term randi] *)
let reduce_rand : term -> term =
  let tr = Tr2.make () in
  let rec is_rand_unit t =
    ...
    | BinOp((Eq|Lt|Gt|Leq|Geq), t1, t2) -> is_rand_unit t1 && has_safe_attr t2 || is_rand_unit t2 && has_safe_attr t1
    | BinOp((Add|Sub|Mult), t1, t2) -> is_rand_unit t1 && has_safe_attr t2 || is_rand_unit t2 && has_safe_attr t1
    | BinOp(Div, t1, {desc=Const c}) when c <> Int 0 -> is_rand_unit t1
    | If(t1,t2,t3) -> has_safe_attr t1 && is_rand_unit t2 && is_rand_unit t3
    | _ -> false
  in
  tr.term <- (fun rand_funs t ->
    match t.desc with
    | Local(Decl_let bindings, t) ->
        let bindings' = List.map (Pair.map_snd @@ tr.term rand_funs) bindings in
        let rand_funs' = List.filter_map Option.(some_if (snd |- decomp_funs |- snd |- is_rand_unit) |- map fst) bindings' in
        let t' = tr.term (rand_funs'@rand_funs) t in
        make_let bindings' t'
    | _ ->
        let t' = tr.term_rec rand_funs t in
        match t'.desc with
        | _ when is_rand_unit t' -> Term.rand t.typ
        | App({desc=Var (LId f)}, ts) when Id.List.mem f rand_funs && List.for_all has_safe_attr ts -> Term.rand t.typ
...
let reduce_ignore : term -> term =
  ...
  tr.term <- (fun ignore_funs t ->
    ...
    | _ ->
        ...
        match t'.desc with
        | App({desc=Var (LId f)}, ts) when Id.List.assoc_opt f ignore_funs |> Option.exists ((=) (List.length ts)) ->
...
let insert_extra_param t : term =
  ...
  |@> Dbg.printf "insert_extra_param (%d added)::@. @[%a@.@." (List.length !Fpat.RefTypInfer.params) Print.term
  ...
  let collect_app =
    ...
    let col_term t =
      ...
      | Local(Decl_let defs, t2) ->
          filter (List.map fst defs) @@ col.term t2
      | _ -> col.term_rec t
    in
    col.term <- col_term;
    col.term |- trans_apps
  ...
  let collect_assume =
    ...
    let col_term t =
      ...
      | If(t1, t2, {desc=Bottom}) ->
          union [t1] (union (col.term t1) (col.term t2))
      | If(t1, _, _)
        | Match(t1, _) -> col.term t1
      | Fun _ -> []
...
let lift_pwhen : term -> term =
  let collect_side_cond =
    let col = Col.make Term.true_ Term.(&&) in
    let col_pat p =
      match p.pat_desc with
      | PWhen(p,c) -> Term.(col.pat p && c)
      | POr(p1,p2) ->
          assert ((col.pat p1).desc = Const True);
          assert ((col.pat p2).desc = Const True);
          Term.true_
      | _ -> col.pat_rec p
    in
    col.pat <- col_pat;
...
let decompose_match : term -> term =
  ...
  let rec tr_pat_list ps =
    let aux p =
      ...
      | _ ->
          ...
          let bind' (default,t) = Term.(match_ (var x) [p', bind (bot, t); Pat.(__ p'.pat_typ), default t.typ]) in
  ...
  tr.desc <- (fun desc ->
    ...
    | Match(t1, pats) ->
        ...
        if Id.List.mem x @@ get_fv t' then
    ...
    | _ ->
        ...
        let t'' =
          if !Flag.Method.verify_ref_interface then
            if env <> [] then
              failwith "%a not found (unused functions are removed)" (print_list Print.id ",") (List.map fst env)
            else
...
let set_assert_ref_typ ?catch_all x ty t =
  ...
  let term =
    ...
    let main = [%term let r = `t_app in assert `t_check'] in
    let rec aux t =
      match t.desc with
      | Local(Decl_let bindings, t2) ->
...
let merge_deref : term -> term option =
  ...
  in
  let pp_ref fm (x,r,t) =
    Format.fprintf fm "(%a, %a, %a)" Print.id x Print.id r Print.term t
  in
  let _pp_refs = Print.list pp_ref in
  let let_refs refs t =
    Term.lets (List.map Triple.take13 refs) t
  in
  let (++) refs1 refs2 =
    let aux (x,r,t) acc =
      let acc' = List.map (fun (x',r',t') -> x', r', if Id.(r = r') then Term.var x else t') acc in
      (x,r,t)::acc'
    in
...
let abstract_exn : term -> term =
  ...
  tr.term <- (fun (_,target,_ as env) t ->
    let t' = tr.term_rec env t in
    match t'.desc with
    | Constr(false, s, ts) when is_exn_ty env t.typ -> (* TODO: fix for the terms with side-effects *)
    ...
    | _ -> t');
  tr.typ <- (fun env ty ->
    if is_exn_ty env ty then Ty.bool else tr.typ_rec env ty);
  tr.pat <- (fun (_,target,non_target as env) p ->
  ...
  fun t ->
    ...
    | Some ty_exn ->
        ...
        if List.Set.(target - !used <> []) then failwith "Unused target: %a@." (print_list Format.pp_print_string ", ") target;
        t'
  ...
  let has_effect decl =
    ...
    | Decl_let defs ->
        if List.exists (not -| has_no_effect -| snd) defs then
          unsupported "Trans.top_def_to_funs (there may exist side-effect on some 'let ... and ...' introduced by a prepprocess?)";
        None
    | _ -> None
  in
...
let set_to_primitive : term -> term =
  let tr = Tr2.make () in
  let make_is_set_op sm to_s x = List.exists (fun (m,_) -> Id.is_in_module_string ~m (to_s x)) sm in
  tr.desc <- (fun set_modules desc ->
    ...
    | Local(Decl_let [m, {desc=App({desc=Var make}, [elt])}] as decl, t) when Lid.name make = "Stdlib.Set.Make" ->
        let set_modules' =
          let _,(_,ty) =
            match elt.typ with
            | TModSig {sig_types} -> List.find_map (List.find_opt (fst |- Id.name |- (=) "t")) sig_types
            | _ -> assert false
          in
          (m,ty)::set_modules
        in
  ...
  tr.typ <- (fun set_modules ty ->
    match ty with
    | TConstr(s,_) when make_is_set_op set_modules Path.to_string s ->
        let _,ty = List.find (fun (m,_) -> Id.is_in_module_string ~m @@ Path.to_string s) set_modules in
        Ty.set ty
...
let abst_div : term -> term =
  ...
  tr.term <- (fun t ->
    ...
    | BinOp(Div, t1, t2) ->
        ...
        Term.(lets [abs,t_abs; x,t1; y,t2; q,randi; r,randi]
              (assume (x_ = q_ * y_ + r_ && (abs_ @ [r_]) < (abs_ @ [y_]))
               (var q)))
(*
    | BinOp(Div, t1, t2) ->
        let x = new_var_of_term t1 in
          ...
          let rec tr_pat p =
            ...
            let bind,pat_desc =
              ...
              | PEval(x,t,p) ->
                  ...
                  in
                  let p'' =
                    let cond = Term.(match_ t' [p', Term.true_; Pat.__ p.pat_typ, Term.false_]) in
...
let copy_poly_type t =
  ...
  let declss,body =
    try
      split_type_decl_and_body ~force:true t
    ...
    if List.mem ~eq (s,tys) acc then
      ...
      | Some pty ->
          try
            ...
            |> extract_list acc'
          with Invalid_argument _ when
                  List.exists (function TConstr(LId c,[]) -> (Id.name c).[0] = '$' | _ -> false) tys ->
  ...
  let fresh x tcs =
    gen_fresh_name_var_int (List.map snd tcs) x
  in
  let name_map =
    tconstrs
    |> List.filter_map (fun ((s:tconstr), tys) ->
        if tys = [] || is_prim_constr (LId s) then
          None
        else
          let name =
            List.map to_id_string tys @ [string_of_tconstr s]
            |> String.join "_"
          in
          let name =
            if String.length name < Flag.long_name_len then
              name
            else
              List.map to_id_string_short tys @ [string_of_tconstr s]
              |> String.join "_"
          in
          let name =
            if String.length name < Flag.long_name_len then
  ...
  let declss =
    ...
    in
    List.map (List.concat_map aux) declss
  in
  List.fold_right make_let_type declss (sbst body)
  |> split_type_decls
...
let inline_type_alias : term -> term =
  ...
  let is_alias (_,(params,ty)) =
    match ty with
    | TConstr(_,params') ->
        assert (List.L.for_all params' ~f:(function
                    | TVar(_,r,_) -> List.L.exists params ~f:(function TVar(_,r',_) -> r == r' | _ -> false)
                    | _ -> true));
        true
    | TBase _ -> true
    | _ -> false
...
let inline_ext_rebind : term -> term =
  ...
  tr.term <- (fun t ->
    ...
    | Local(Type_ext(Ext_rebind(s,LId s')), t1) ->
        t1
        |> subst_tconstr_label_map [s,s']
        |> subst_constr (LId s) (LId s')
        |> tr.term
...
let abst_magic : term -> term =
  ...
  tr.term <- (fun t ->
    match t.desc with
    | App({desc = Var x}, ts) when List.mem (Lid.name x) ["Stdlib.Obj.magic";"Stdlib__Obj.magic"] ->
        let ts = List.map tr.term ts in
...
let remove_dummy : term -> term =
  ...
  tr.desc <- (fun desc ->
    ...
    | Const (Dummy ty) ->
        begin
          match (make_default_term ty).desc with
          | Const (Dummy _) -> [%unsupported]
          | desc -> desc
        end
    | _ -> tr.desc_rec desc);
  tr.term
...
let remove_por : term -> term =
  let tr = Tr.make () in
  let rec expand_list ps =
    ps
    |> List.map expand
    |> Combination.take_each
...
let abst_poly_variant : term -> term =
  ...
  tr.desc <- (fun env desc ->
    ...
    | Constr(true, LId c, ts) ->
        Flag.Abstract.use "polymorphic variant";
        let ts = List.map (tr.term env) ts in
        Id.List.assoc c env
        |> Term.int
        ...
        |> Syntax.desc
    | Match _ ->
        trans_match_with_recover_pat_bv_desc ~tr_desc_rec:(tr.desc_rec env) ~tr_typ:(tr.typ env) desc
    | _ -> tr.desc_rec env desc);
  tr.term <- set_afv_shallowly --| tr.term_rec;
  tr.pat <- (fun env p ->
    match p.pat_desc with
  ...
  tr.typ <- (fun env ty ->
    match ty with
    | TVariant(VPoly _, _) -> Ty.int
    | _ -> tr.typ_rec env ty);
  fun t ->
    let env =
      t
      |> col_poly_constr
      |> List.mapi Pair.pair_rev
    in
    tr.term env t
...
let abst_format_const : term -> term =
  let tr = Tr.make () in
  tr.term <- (fun t ->
    match t.desc, elim_tattr t.typ with
    | Const(String _), TConstr(c, _) when List.mem (string_of_path c) types_of_format ->
        Term.rand t.typ
    | _ -> tr.term_rec t);
  tr.term
  ...
  let binds, pat_desc, t =
          ...
          let rest =
            try
              ...
              |> List.filter_map (fun (label,(_,ty)) ->
                     if Id.List.mem_assoc label fields then
                       None
                     else
    ...
    | POr(p1, p2) ->
        ...
        if name_por then
          ...
          binds, desc, Term.dummy p.pat_typ (* term must not be used *)
    | PWhen(p, cond) ->
...
let add_def_of_free_types t =
  let decls : type_decl list =
    get_free_types t
    |> List.map (fun (c,n) ->
        tconstr_of_string (Path.to_string c),
        (List.init n (fun _ -> Type.new_tvar ()),
         Ty.abst))
  in
...
let inline_tvar =
  ...
  tr.typ <- (fun env ty ->
    match ty with
    | TConstr(LId s, []) when Id.List.mem_assoc s env ->
        Id.List.assoc s env
    | TModSig _
    | TModLid _ ->
        Flag.Print.signature := true;
    ...
    | Decl_let defs ->
        ...
        |> List.map (fun (f,t) ->
               if is_menhir_fun (f,t) then
                 let () = Flag.Abstract.use "polymorphic variant" in
                 if has_fail t then
                   let xs,t' = decomp_funs t in
                   f, Term.funs xs Term.(seq fail (bot t'.typ))
                 else
                   f, Term.rand ~use:false t.typ
               else
                 f, tr.term t)
        |> _Decl_let
    | _ -> tr.decl_rec decl);
