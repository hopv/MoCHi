...
let only_LId =
  ...
  it.typ <- (fun ty ->
    ...
    | TConstr(path, _) ->
        check_path path
    | TModSig sgn ->
        List.iter check_ext sgn.sig_ext
    | _ -> ());
  it.decl <- (fun decl ->
    it.decl_rec decl;
    match decl with
...
let check_afv =
  ...
  let col_term fm t =
    ...
    match fvs with
    | [fv'] ->
        if Id.List.Set.eq fv fv' && List.length fv = List.length fv' then
          col.term_rec fm t
        else
    ...
    | _ ->
        if fvs = [] then
          Format.fprintf fm "AFV not found@."
        else
          Format.fprintf fm "Redundant AFV@.";
        Format.fprintf fm "  t: %a@." Print.term t;
  ...
  let col_pat fm p =
    ...
    | _ ->
        if fvs = [] then
          Format.fprintf fm "PAFV not found@."
        else
          Format.fprintf fm "Redundant PAFV@.";
        Format.fprintf fm "  p: %a@." Print.pattern p;
        Format.fprintf fm "  fvs: %a@." Print.(list (list id)) fvs;
        false
  in
...
let check_fv ~force ~before ~after =
  let get_fv' p =
    ...
    let ext_funs = List.map Triple.fst (Problem.spec p).ext_ref_env in
    Id.List.Set.(fv_term - ext_funs)
  in
  let fv =
    Id.List.Set.(get_fv' after - get_fv' before)
...
let check_free_types ~before ~after =
  let get_free_types' p =
    let ft_term =
      Problem.term p
      |> get_free_types
      |> List.map fst
    in
    let ext_types = List.map fst p.ext_typ in
    List.Set.(ft_term - ext_types)
  in
...
let check_type problem =
  ...
  in
  let env =
    Tenv.env_of_ext_mod problem.ext_mod_typ
    |> List.fold_right (Fun.uncurry Tenv.add_exn) problem.ext_exc
    |> List.fold_right (Fun.uncurry Tenv.add_external_type) problem.ext_typ
  in
  try
    Type_check.check ~force:true ~env ~ty:t.Syntax.typ t
  with
  | Type_check.Failed loc ->
      let len = (3 * Format.pp_get_margin Format.err_formatter ()) / 2 in
      let pr_delim () = Format.eprintf "%a%s%t@.@." Color.set Color.Green (String.make len '=') Color.reset in
      ...
      assert false
  | Sig_of_Lid m ->
      Format.eprintf "[%s] Sig_of_Lid failed: %a@." __FUNCTION__ Print.path m

let check_lid {Problem.attr; term; _} =
  try
    if not (List.mem Problem.Module attr) then
      only_LId term
  with Failed _ ->
    begin
