...
let rec remove_pred (ty : _ t) : unit t =
  ...
  | TModSig {sig_types; sig_values; sig_ext} ->
      let sig_values = List.map (Id.map_typ rm) sig_values in
      let sig_types = Fmap.(list (list (snd (list rm * rm)))) sig_types in
      let sig_ext = Fmap.(list (snd (list rm * rm_ext_row))) sig_ext in
      TModSig {sig_types; sig_values; sig_ext}
  | TModLid m -> TModLid m
...
let subst_tconstr_aux params pty =
...
let is_ref_typ ty =
  ...
  | TConstr(c, [_]) -> C.is_ref c
  | _ -> false

let is_base_prim c = List.mem c C.base_prims
let is_prim_constr c = List.mem c C.prims
...
let rec flatten typ =
  match typ with
  | TVar(_,{contents = Some typ'},_) -> flatten typ'
  | _ -> typ
...
let occurs =
  let aux r _ ty =
    match ty with
    | TVar(_,({contents=None} as r'),_) -> r == r'
    | _ -> false
  in
...
let rec unify
    ?(for_check = false)
    ?(env = Tenv.empty())
    ?(sub = false)        (** [unify ~sub:true ty1 ty2] succeeds if [ty1] is a subtype of [ty2] *)
    ?(only_weak = false)
    ?(acc = ref[])
    ?(ignore_non_poly_fail = false) (** WORKAROUND: ignore the unification error for types without type variables if this is true *)
    typ1 typ2 =
(*
  try
*)
  ...
  let print_error f =
    if for_check then
      Format.ifprintf Format.std_formatter f
    else
      Format.eprintf f
  in
  let cannot_unify () =
  ...
  | TVar(false, _, _), _
  | _, TVar(false, _, _) when only_weak -> ()
  | TVar(_, r, _), typ
  | typ, TVar(_, r, _) when occurs r typ ->
  ...
  | TConstr(c1,typs1), TConstr(c2,typs2) when
          Path.to_string c1 = Path.to_string c2 (* WORKAROUND *)
          || (List.mem (c1,c2) !acc || List.mem (c2,c1) !acc) && List.compare_lengths typs1 typs2 = 0 (* BUGGY? *) ->
      check_len_eq typs1 typs2;
      List.iter2 uni typs1 typs2
  | TConstr(c,[]), _ when C.is_exn c ->
      uni (Tenv.assoc_exn env) typ2
  | _, TConstr(c,[]) when C.is_exn c ->
      uni typ1 (Tenv.assoc_exn env)
  | TConstr(s,tys), _ when Tenv.mem_assoc_type s env ->
      let tys',typ1' = Tenv.assoc_type s env in
      check_len_eq tys tys';
      begin
        match typ2 with
        | TConstr(s',_) -> acc := (s,s')::!acc
        | _ -> ()
      end;
  ...
  | TVariant(VPoly {closed=true; _}, rows1), TVariant(VPoly {closed=true; _},rows2) when sub -> (* TODO *)
  ...
  | TVariant(VPoly {closed=false; _}, rows1), TVariant(VPoly {closed=false; _},rows2) when sub ->
      check_bool (List.length rows1 >= List.length rows2);
      let check row2 =
  ...
  | TModSig sgn1, TModSig sgn2 when !Flag.Debug.check_tmodule && sub ->
      check_bool (List.length sgn1.sig_types >= List.length sgn2.sig_types);
      let check (s2,(_,ty2)) =
        match List.find_map (Id.List.assoc_opt s2) sgn1.sig_types with
        | _,ty1 -> uni ty1 ty2
        | exception Not_found ->
            print_error "Not_found %a in %a@." Pr.constr s2 Pr.typ typ2;
            cannot_unify()
      in
      List.iter (List.iter check) sgn2.sig_types;
      check_bool (List.length sgn1.sig_values >= List.length sgn2.sig_values);
      let check y =
        match List.find Id.((=) y) sgn1.sig_values with
        | x -> uni (Id.typ x) (Id.typ y)
        | exception Not_found ->
            print_error "Not_found %a in %a@." Id.print y Pr.typ typ1;
            cannot_unify()
      in
      List.iter check sgn2.sig_values
  | TModSig sgn1, TModSig sgn2 when !Flag.Debug.check_tmodule ->
      check_len_eq sgn1.sig_types sgn2.sig_types;
      let env =
        List.L.fold_left2 sgn1.sig_types sgn2.sig_types
          ~init:env
          ~f:(fun env decls1 decls2 ->
            ...
            List.fold_left (fun env (s1,(params1,ty1)) -> Tenv.add_typ s1 (params1,ty1) env) env decls1)
      in
  ...
  | TPoly(_, ty1), _ when sub ->
      uni (snd @@ copy_tvar_typ ty1) typ2
  ...
  | _, _ ->
      ...
      if Type.eq ty1 typ1 && Type.eq ty2 typ2 then
        cannot_unify ()
      else
        uni ty1 ty2
(*
  with CannotUnify _ as e when !!Dbg.check ->
    Format.printf "unify typ1: %a@." Print_typ.typ_init typ1;
    Format.printf "unify typ2: %a@.@." Print_typ.typ_init typ2;
    raise e
 *)
...
let rec eq ty1 ty2 =
  ...
  | TModSig sgn1, TModSig sgn2 -> (** BUGGY *)
      let aux = Fmap.(list (list (snd Stdlib.snd))) in
      Eq.(list (list ((=) * eq))) (aux sgn1.sig_types) (aux sgn2.sig_types) &&
      Eq.(list eq_id) sgn1.sig_values sgn2.sig_values
...
let rec is_instance_of ?(strict=false) ty1 ty2 =
  ...
  | TModSig sgn1, TModSig sgn2 -> (** BUGGY *)
      let aux = Fmap.(list (list (snd Stdlib.snd))) in
      Eq.(list (list ((=) * eq))) (aux sgn1.sig_types) (aux sgn2.sig_types) &&
      Eq.(list (on Id.typ ~eq)) sgn1.sig_values sgn2.sig_values
  | TModLid m1, TModLid m2 -> Path.(m1 = m2)
  | _ -> false
...
let rec to_id_string = function
  ...
  | TVariant(_,rows) ->
      rows
      |> List.take 2
      |> List.map (row_constr |- string_of_constr)
      |> String.join "_"
...
let rec is_functor_typ ?default env ty =
  ...
  | [], TModLid x ->
      begin match Tenv.assoc_value x ~env:(Tenv.env_of_ext_mod env) with
...
let map_path f =
  make_trans (fun tr ty ->
      match ty with
      | TConstr(c,tys) -> Some (TConstr(f c, List.map tr tys))
      | _ -> None)
...
let typ_event_cps () = List.fold_right make_tfun [TBase TUnit; make_tfun (TBase TUnit) typ_result] typ_result

let row_of_ext_row {ext_row_path; ext_row_args; ext_row_ret} =
  {row_constr = constr_of_path ext_row_path;
   row_args = ext_row_args;
   row_ret = ext_row_ret}

let ext_row_of_row {row_constr; row_args; row_ret} =
  {ext_row_path = LId row_constr;
   ext_row_args = row_args;
   ext_row_ret = row_ret}

let map_row_typ ~f {row_constr; row_args; row_ret} =
  {row_constr;
   row_args = List.map f row_args;
   row_ret = Option.map f row_ret}
