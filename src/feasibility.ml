...
type env = {
  ce : int list;
      (** counterexample: list of integers whose element represents which branch is taken *)
  ...
  tenv : (string * CEGAR_syntax.t CEGAR_type.t) list;
      (** type environment (actually, types are unused as all variables are assumed to be integers) *)
}

let init_env ce = { ce; sat = true; n = 0; constr = Const True; tenv = [] }

let checksat { constr; _ } =
  constr |> Encode_set_theory.add |> FpatInterface.conv_formula |> Fpat.SMTProver.is_sat_dyn
...
let get_solution { tenv; constr; _ } =
  ...
  in
  Dbg.printf "sol: %a@." Print.(list (string * int)) sol;
...
let add_randint_precondition map_randint_to_preds ext_ce rand_precond r = function
  ...
  | Some n -> (
      ...
      let abst_preds =
        try (List.assoc n map_randint_to_preds) new_var
        with Not_found ->
          Format.fprintf !Flag.Print.target "not found: %d@." n;
          []
      in
      ...
      | (m, bs) :: ext_ce' when m = n ->
          ...
          let asm_cond =
            List.fold_left2
              (fun acc p b -> make_and (if b then p else make_not p) acc)
              (Const True) abst_preds bs
          in
...
let rec check_aux ?(pr : CEGAR_syntax.t -> int -> int -> unit = fun _ _ _ -> ())
    (* printer; see print_ce_reduction *)
      (env : env) (defs : CEGAR_syntax.fun_def list) (* function definitions*) (t : CEGAR_syntax.t)
    (* the term to be reduced *) k (* continuation *) =
  Dbg.printf "check_aux[%d]: %a@." (List.length env.ce) CEGAR_print.term t;
  match t with
  ...
  | App _ when is_app_randint t ->
      pr t 0 1;
      if Flag.(!mode = FairNonTermination) && !ext_ce_ref = [] then init_cont (env, t)
      else
        let t', randnum =
  ...
  | App (t1, t2) -> (
      ...
      match env.ce with
      | [] ->
          if Flag.(!mode = FairNonTermination) then init_cont (env, App (t1, t2)) else assert false
      | c :: ce' ->
          let t1', ts = decomp_app (App (t1, t2)) in
          let { args = xs } = List.find (fun def -> Var def.fn = t1') defs in
          if List.length xs > List.length ts then k (env, App (t1, t2))
          else
            let num, { args = xs; cond = tf1; body = tf2 } = assoc_def defs c t1' in
            let constr, tenv =
              if num > 1 && tf1 = Const True then
                let r = new_id "r" in
                (make_and (make_eq_int (Var r) (make_int c)) env.constr, (r, typ_int) :: env.tenv)
              else (env.constr, env.tenv)
            in
            ...
            assert (ts2 = []);
            pr t1' c num;
            let env = { ce = ce'; sat; n; constr; tenv } in
...
let check ce { defs; main } =
  ...
  Verbose.printf ~color:Green "%a(%d-3)[%.3f] Checking counterexample ...%t @?" Color.set Green
    !Flag.Log.cegar_loop !!Time.get Color.reset;
  ...
  Dbg.printf "constr: %a@." CEGAR_print.term env.constr;
  let result =
    if checksat env then
...
let check_non_term ?(map_randint_to_preds = []) ?(ext_ce = []) ce { defs; main } =
  ...
  Verbose.printf "Spurious counterexample::@.  %a@.@." CEGAR_print.ce ce;
  let time_tmp = Time.get () in
  Verbose.printf ~color:Green "(%d-3)[%.3f] Checking counterexample ... @?" !Flag.Log.cegar_loop
    !!Time.get;
  set_status @@ Flag.Log.Other (Format.sprintf "(%d-3) Feasibility checking" !Flag.Log.cegar_loop);
  if false then Format.fprintf !Flag.Print.target "ce:        %a@." CEGAR_print.ce ce;
  let ce' = List.tl ce in
  let { body = t } = List.find (fun def -> def.fn = main) defs in
  init_refs map_randint_to_preds ext_ce;
  (* initialize abstraction predicate path of randint *)
  let env = check_aux (init_env ce') defs t init_cont in
  ...
  let result =
    if checksat { env with constr = make_and env.constr !rand_precond_ref } then
      ...
      *)
      FeasibleNonTerm
        ( FpatInterface.implies
            [ FpatInterface.conv_formula !rand_precond_ref ]
            [ FpatInterface.conv_formula env.constr ],
          env.tenv,
          solution )
    else Infeasible prefix
  in
...
let rec trans_ce ce ce_br env defs t k =
  Dbg.printf "trans_ce[%d]: %a@." (List.length ce) CEGAR_print.term t;
  match t with
  | Const (Rand (TInt, _)) -> assert false
  ...
  | App
      ( App
          ( Const
              ((And | Or | Lt | Gt | Leq | Geq | EqUnit | EqBool | EqInt | Add | Sub | Mul | Div) as
               op),
            t1 ),
        t2 ) ->
...
*)
  ...
  | App (t1, t2) ->
      ...
      let t1', ts = decomp_app (App (t1, t2)) in
      let { args = xs } = List.find (fun def -> Var def.fn = t1') defs in
      if List.length xs > List.length ts then k (ce, ce_br, env, App (t1, t2))
      else
...
let trans_ce ce { defs; main } =
  Dbg.printf "ce:        %a@." CEGAR_print.ce ce;
  let ce' = List.tl ce in
  let { body = t } = List.find (fun def -> def.fn = main) defs in
  trans_ce ce' [] [] defs t init_cont

let print_ce_reduction ?(map_randint_to_preds = []) ?(ext_ce = []) ce { defs; main } =
  let ce' = List.tl ce in
  let { body } = List.find (fun def -> def.fn = main) defs in
  (* t: Term to be printed *)
  (* br: Branch which will be taken *)
