...
module Dbg_ty = Debug.Make (struct
  let check = Flag.Debug.make_check (__MODULE__ ^ ".ty")
end)

module Dbg = Debug.Make (struct
  let check = Flag.Debug.make_check __MODULE__ ||| Dbg_ty.check
end)
...
let get_elem pos f =
  ...
  let cond1 =
    let indices = Combination.product ns ns |> List.filter (Fun.uncurry ( < )) in
    Term.ands @@ List.map Term.(fun (i, j) -> not (proj' i && proj' j)) indices
  in
  let cond2 = Term.ors @@ List.map proj' ns in
  Term.(let_ [ (r, t) ] (assume (cond1 && cond2) (var r)))
                 ...
                 |> List.filter_map (function
                      ...
                      | ty' ->
                          if List.mem_assoc ~eq:Path.eq (Type.LId s) (get_tconstr_typ ty') then
                            ...
                            let () = Flag.Debug.print "ty': %a@." Print.typ ty' in
                            unsupported
                              "%s: non-simple recursion (use -data-to-int, -ignore-data-arg, etc.)"
                              __MODULE__
                          else Some (abst_recdata.typ env ty'))
               in
               Ty.(bool * tuple tys))
        |> Ty.tuple
      in
...
let abst_recdata_typ env typ =
  match typ with
  | TConstr (LId s, []) -> (
      match Id.List.assoc_opt s env with None -> typ | Some (_, _, ty) -> ty)
...
let is_rec_type env ty =
  ...
  | TConstr (LId s, _) when not (is_prim_constr (LId s)) -> (
      try Triple.snd @@ Id.List.assoc s env with Not_found -> assert false)
  | _ -> false

let expand_typ env ty =
  match ty with
  | TConstr (LId s, _) when not (is_prim_constr (LId s)) -> (
      try Triple.fst @@ Id.List.assoc s env with Not_found -> assert false)
  | _ -> ty

let expand_enc_typ env ty =
  match ty with
  | TConstr (LId s, _) when not (is_prim_constr (LId s)) -> (
      try Triple.trd @@ Id.List.assoc s env with Not_found -> assert false)
  | _ -> ty

let make_add_bind bind = List.fold_right (fun (t, p) t' -> Term.match_ t [ (p, t') ]) bind
let get_c ty = ty |> Val'._TConstr |> Option.map fst
...
let rec abst_recdata_pat env p =
  ...
  let desc, cond, bind =
    ...
    | PConstr (false, LId c, ps) when not (is_rec_type env p.pat_typ) ->
        let p', cond, bind =
          let ps, cond, bind =
            List.L.fold_right ps ~init:([], true_term, []) ~f:(fun p (ps, cond, bind) ->
                let p', cond', bind' = abst_recdata_pat env p in
        ...
        let ps' =
          ...
          |> List.map (fun row ->
                 check_row_ret row;
                 if Id.(c = row.row_constr) then Pat.(true_ * p')
                 else
    ...
    | PConstr (false, LId c, ps) ->
        ...
        let constr_ix, _ =
          expand_typ env p.pat_typ |> ValE'._TVariant |> snd |> assoc_row_with_pos ~sort:false c
        in
          ...
          let make_bind (acc, i, j) p (p', _, _) =
            let i', j', t' =
              if Eq.(option ~eq:path) path (get_c p.pat_typ) then (
                ...
                let () =
                  Dbg.printf "%a has non-rec type %a@." Print.pattern p Print.typ p.pat_typ
                in
        ...
        in
        Dbg.printf "binds = %a@."
          Print.(list (fun fm (x, p) -> triple pattern typ term fm (p, x.typ, x)))
          binds;
        let cond =
          ...
          let conds' =
            let make_cond (t, pt) (p, cond, _) =
              ...
              | PAny | PVar _ -> cond
              | _ -> Term.match_ t [ (pt, cond); (Pat.__ p.pat_typ, Term.false_) ]
            in
    ...
    | PRecord fields ->
        ...
        (PTuple ps, cond, bind)
    | POr ({ pat_desc = PConst _ }, _) | POr (_, { pat_desc = PConst _ }) ->
        (p.pat_desc, true_term, [])
    | POr (p1, p2) ->
        let p1', cond1, bind1 = abst_recdata_pat env p1 in
        let p2', cond2, bind2 = abst_recdata_pat env p2 in
  ...
  in
  (make_pattern desc typ, cond, bind)

let abst_recdata_term (env : env) t =
  let typ = elim_tattr t.typ in
  match t.desc with
  | BinOp (Eq, t1, _) when is_rec_type env t1.typ -> unsupported "Equality for recursive data types"
  ...
  | Constr (false, LId c, ts) when not (is_rec_type env typ) ->
      let aux row =
        ...
        if Id.(c = row.row_constr) then
          ...
          let t = List.map (abst_recdata.typ env) row.row_args |> Ty.tuple |> Term.rand in
          [%term false, `t]
      in
  ...
  | Constr (false, LId c, ts) ->
      ...
      let top =
        let make_return ts' =
          ...
          |> List.map (fun row ->
                 check_row_ret row;
                 if Id.(c = row.row_constr) then Term.(pair true_ (tuple ts'))
                 else
  ...
  | Match (t1, pats) ->
      ...
      let pats' =
        List.L.map pats ~f:(fun (p, t) ->
            ...
            (Pat.when_ p' (add_bind c), add_bind t'))
      in
      Term.match_ t1' pats'
  | Record [] -> assert false
  | Record fields ->
      if List.exists (fun (_, (f, _)) -> f = Mutable) @@ ValE'._TRecord typ then
        unsupported "Mutable records"
      else Term.tuple @@ List.map (abst_recdata.term env -| snd) fields
  | Field (t, s) -> (
      let fields = ValE'._TRecord typ in
      match Id.List.assoc_opt s fields with
      | None -> assert false
      | Some (Mutable, _) -> unsupported "Mutable records"
      | _ ->
          Term.proj (List.find_pos (fun _ (s', _) -> Id.(s = s')) fields) @@ abst_recdata.term env t
      )
  ...
  | Local (Decl_type [ (s, (params, ty)) ], t) ->
      ...
      let env' =
        (s, (ty, List.mem_assoc ~eq:Path.eq (Type.LId s) @@ get_tconstr_typ ty, ty')) :: env
      in
...
let gather_env : Syntax.term -> env =
  let rec go env t =
    ...
    | Local (Decl_type [ (s, (params, ty)) ], t) ->
        ...
        let ty' = encode_recdata_typ env s ty in
        let env' =
          (s, (ty, List.exists (fst |- Path.( = ) (LId s)) @@ get_tconstr_typ ty, ty')) :: env
        in
        go env' t
        (*| Local(_, t) -> go env t*)
        (* can be ignored because of Preprocess.Lift_type_decl *)
    | _ -> env
  in
  go []
...
let trans_pred =
  ...
  let term (x, sym_env) t =
    ...
    | Match ({ desc = Var (LId y) }, pats) when Id.(x = y) ->
        ...
        let _, preds =
          List.Labels.fold_right pats ~init:(labels, []) ~f:(fun (p, t) (remain, arms) ->
              match p.pat_desc with
              ...
              | PConstr (false, LId c, ps) ->
                  ...
                  let sbst =
                    List.Labels.filter_map (List.mapi Pair.pair ps) ~f:(fun (j, p) ->
                        match p.pat_desc with
                        | PAny -> None
                        | PVar x -> (
                            try Some (x, Map.find j map)
                            with Not_found ->
                              None (* TODO assert x is not in FV(t) when Not_found *))
                        | _ -> assert false)
                  in
...
let mk_sym_env s t (lts : lts) =
  List.Labels.mapi lts ~f:(fun i (l, (tys, tys')) ->
      ...
      let map : (int, term) Map.t =
        ...
        let rec go m i j tys tys' =
          match (tys, tys') with
          | _, [] -> m
          | TConstr (s', _) :: tys, _ when Path.(s' = s) -> go m (i + 1) j tys tys'
          | _ :: tys, _ty' :: tys' ->
              let m' = Map.add i Term.(proj j (snd t_base)) m in
              go m' (i + 1) (j + 1) tys tys'
          | _ -> assert false
        in
        go Map.empty 0 0 tys tys'
      in
      (l, (is_l, map)))
...
let trans_rty_nonrec_data (s, x, t) (ty_before : typ) (ty_after : typ) =
  ...
  | TVariant (VNonPoly, rows), TTuple [ { Id.typ = ty1 }; { Id.typ = ty2 } ] ->
      ...
      let lts =
        List.L.map2 rows xs ~f:(fun row x ->
            check_row_ret row;
      ...
      Type_check.check ~ty:Ty.bool t';
      Ref_type.(Tuple [ (v, of_simple ty1); (u, Base (TUnit, u, t')) ])
  | _, _ -> assert false
...
let make_trans_env { Problem.term } =
  ...
  let sbst =
    let f _ rty =
      match rty with Ref_type.Constr (LId s, _, _, _) -> Id.List.assoc_opt s map | _ -> None
    in
    Ref_type.make_trans f
  in
  fun (x, rty) ->
    let rty' = sbst rty in
    (Id.set_typ x (Ref_type.to_simple rty'), rty')
