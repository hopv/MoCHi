...
let abst_recdata_term (env: env) t =
  ...
  | Record fields ->
      ...
      else Term.tuple @@ List.map (abst_recdata.tr2_term env -| snd) fields
  | Field(t,s) ->
      ...
      if Mutable =
        ...
        Term.proj (List.find_pos (fun _ (s',_) -> s = s') fields) @@ abst_recdata.tr2_term env t
...
let make_trans_env {Problem.term} =
  let map =
    ...
    let xs = List.map (fun (s,_) -> Id.new_var (TData s)) decls in
    let _,body =
    ...
    | Tuple ts ->
        ts
        |> List.map (decomp_var |- Option.get |- Id.typ |- Ref_type.of_simple)
        |> List.combine (List.map fst decls)
    | Var x ->
        let s,_ = List.get decls in
        [s, Ref_type.of_simple @@ Id.typ x]
    | _ -> assert false
  in
  let sbst =
    let f _ rty =
      match rty with
      | Ref_type.ADT(s,_,_) -> List.assoc_opt s map
      | _ -> None
    in
