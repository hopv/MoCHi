...
type t = Base of base * CS.var * CS.t | Fun of CS.var * t * t | Inter of CS.typ * t list
...
let rec print fm = function
  | Base (base, _, CS.Const CS.True) -> Format.fprintf fm "%a" print_base base
  | Base (base, x, p) ->
      Format.fprintf fm "{%a:%a | %a}" CEGAR_print.var x print_base base CEGAR_print.term p
  | Fun (x, typ1, typ2) ->
      if occur x typ2 then
        Format.fprintf fm "(@[%a:%a@ ->@ %a@])" CEGAR_print.var x print typ1 print typ2
