...
let check_eq loc x y t =
  if x <> y then check_failed loc t
...
and check env bv t typ =
  ...
  | Event(_s,true), typ' ->
      if not @@ cu typ' !!Ty.event_cps then check_failed __LOC__ t
  | Tuple ts, TTuple xs ->
  ...
  | Field(t1,s), typ ->
      begin
        ...
        | TRecord tfields ->
            begin
              try
                if not @@ cu typ @@ snd @@ Id.List.assoc s tfields then check_failed __LOC__ t
              with Not_found ->
                check_failed __LOC__ t
            end
        | _ ->
  ...
  | Constr _, TConstr(c,_) when C.is_exn c ->
      ch t (Tenv.assoc_exn env)
  | Constr(false,s,ts), TVariant(VNonPoly,rows) ->
      let row =
        try
          assoc_row (constr_of_path s) rows
        with Not_found ->
  ...
  | Constr(true,s,ts), TVariant(VPoly {closed; tvar=_; lower=_}, rows) ->
      let row =
        try
          Some (assoc_row (constr_of_path s) rows)
        with Not_found ->
          if closed then
            (print_error "t: %a@." Print.term' t;
             print_error "t.typ: %a@." Print.typ t.typ;
      ...
      Option.iter (fun row ->
          check_eq __LOC__  (List.length ts) (List.length row.row_args) t;
          List.iter2 ch ts row.row_args) row
  | Match(t1,pats), typ' ->
      ch t1 t1.typ;
  ...
  | Bottom, _ -> ()
  | Ref t, TConstr(c, [typ]) when c = C.ref ->
      ch t typ
  | Deref t, typ ->
      ch t (make_tref typ)
  ...
  | TSome t, TConstr(c, [typ]) when c = C.option ->
      ch t typ
  | Lazy t, TConstr(c, [typ]) when c = C.lazy_ ->
      ch t typ
  | _, TConstr(s,_) ->
      begin
  ...
  | Module decls, TModSig {sig_values; sig_types} -> (* TODO: update env, use sig_types to check decls *)
      let sgn = sig_of_mod_typ ~env:env.menv (Term.module_ decls).typ in
      if not @@ Id.List.Set.(sig_values <= sgn.sig_values) then check_failed __LOC__ t;
      let tys_of = List.flatten_map (List.map fst) in
      if not @@ List.Set.(tys_of sig_types <= tys_of sgn.sig_types) then check_failed __LOC__ t;
      ch Term.(locals decls eod) Ty.unit
  | Pack t, TPackage ty ->
      ch t ty
  | Unpack t, ty ->
      ch t (TPackage ty)
...
let check ?(force=false) ?(env=Tenv.empty()) ?(ty=Ty.unit) t =
