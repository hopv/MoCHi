...
let verifyFile_aux filename =
  ...
  let result_file = Filename.change_extension !!Flag.IO.temp "horsatp_out" in
  let oc = open_out result_file in
  output_string oc default;
  close_out oc;
  let ver = Option.get @@ version () in
  if ver < required_ver then
    (Format.fprintf !Flag.Print.target "HorSatP: minimum version required is %s, but %s@." required_ver ver;
...
let verifyFile filename =
  ...
  | "Unsatisfied" | "unsafe" ->
      let fname = Filename.change_extension !!Flag.IO.temp "error_hors" in
