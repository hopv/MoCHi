...
and 'a tvar = bool * 'a t option ref * int
(* Boolean denotes weakness, The third argument (counter) is used just for printing *)

and variant_kind =
  | VNonPoly
  | VPoly of { closed : bool; tvar : unit option; (* TODO *) lower : constr list option }
(* TODO *)

and 'a row = { row_constr : constr; row_args : 'a t list; row_ret : 'a t option }
...
and 'a attr =
  ...
  | TAId of string * int
  | TARaise of 'a t
  | TAExistential of string
  | TARec of 'a tvar

and effect = EVar of int | EEvent | ENonDet | EDiv | EExcep
and 'a signature = 'a sig_elem list
and 'a sig_elem = Sig_type of 'a declaration | Sig_ext of 'a ext | Sig_value of 'a tid
and 'a declaration = 'a decl_item list
and 'a decl_item = tconstr * ('a params * 'a t)

and 'a ext = path * ('a params * 'a ext_row)
(** [p, (['a; 'b], ext)] represents [type ('a, 'b) p += ext] *)

and 'a params = 'a t list

and 'a ext_row = {
  ext_row_path : path;
      (* I don't know when LDot (or LApp) is used here
         (The original type in the OCaml compiler is Path.t) *)
  ext_row_args : 'a t list;
  ext_row_ret : 'a t option; (* for GADT *)
}
(** [type (a,b) t += A of int ] is represented like
    [Ext_type ("t", (["a";"b"],
                     {ext_row_path = "A"; ext_row_args = ["int"]; ext_row_ret = None}))] *)
...
let _TConstraint ty cs = if cs = [] then ty else TConstraint (ty, cs)
...
let _Sig_type decl = Sig_type decl
let _Sig_ext ext = Sig_ext ext
let _Sig_value x = Sig_value x
let row_constr { row_constr; _ } = row_constr
let row_args { row_args; _ } = row_args
let row_ret { row_ret; _ } = row_ret
let rec elim_tattr : 'a t -> 'b t = function TAttr (_, typ) -> elim_tattr typ | typ -> typ
...
let make_trans2 f =
  let rec trans env ty =
    match f trans env ty with
    | None -> (
        let tr = trans env in
        let tr_row { row_constr; row_args; row_ret } =
          let row_args = List.map tr row_args in
          let row_ret = Option.map tr row_ret in
          { row_constr; row_args; row_ret }
        in
        let tr_ext_row { ext_row_path; ext_row_args; ext_row_ret } =
          let ext_row_args = List.map tr ext_row_args in
          let ext_row_ret = Option.map tr ext_row_ret in
          { ext_row_path; ext_row_args; ext_row_ret }
        in
        match ty with
        | TBase b -> TBase b
        | TVar (_, { contents = Some ty }, _) -> tr ty
        | TVar (b, r, id) -> TVar (b, r, id)
        | TFun (x, ty) -> TFun (Id.map_typ tr x, tr ty)
        | TConstr (c, tys) -> TConstr (c, List.map tr tys)
        | TTuple xs -> TTuple (List.map (Id.map_typ tr) xs)
        | TAttr (attr, ty) -> TAttr (attr, tr ty)
        | TFuns (xs, ty) -> TFuns (List.map (Id.map_typ tr) xs, tr ty)
        | TVariant (poly, rows) -> TVariant (poly, List.map tr_row rows)
        | TRecord fields -> TRecord (Fmap.(list (snd (snd tr))) fields)
        | TModSig sgn ->
            sgn
            |> List.map (function
                 | Sig_value v -> Sig_value (Id.map_typ tr v)
                 | Sig_type ty -> Sig_type (Fmap.(list (snd (snd tr))) ty)
                 | Sig_ext ext -> Sig_ext (Fmap.(snd (snd tr_ext_row)) ext))
            |> _TModSig
        | TModLid m -> TModLid m
        | TPackage ty -> TPackage (tr ty)
        | TPoly (vars, ty) -> TPoly (vars, tr ty)
        | TConstraint (ty, cs) -> TConstraint (tr ty, Fmap.(list (tr * tr)) cs)
        | TOpen -> TOpen)
    | Some ty -> ty
  in
  trans

let make_trans f ty = make_trans2 (fun tr () ty -> f (tr ()) ty) () ty
...
let make_reduce ~f ~app:( ++ ) ~default =
  let rec reduce ty =
    ...
    | None -> (
        ...
        | TModSig sgn ->
            sgn
            |> reduce_list (function
                 | Sig_type decls -> reduce_list (snd |- snd |- reduce) decls
                 | Sig_value v -> reduce_var v
                 | _ -> default)
        | TModLid _ -> default
...
let make_find (type a) (f : (a t -> unit) -> a t -> bool) =
  ...
  let rec find ty =
    ...
    if f find ty then raise (Found ty)
    else
      match ty with
      | TBase _ -> ()
      ...
      | TModSig sgn ->
          sgn
          |> List.iter (function
               | Sig_type decls -> List.iter (find -| snd -| snd) decls
               | Sig_value x -> find_var x
               | _ -> ())
      | TModLid _ -> ()
      | TPackage ty -> find ty
      | TPoly (_, ty) -> find ty
...
let make_fold f =
  let rec fold acc ty =
    ...
    | None -> (
        ...
        | TConstr (c, tys) -> wrap (_TConstr c) ~fld:(fld_list ~fld) ~acc tys
        ...
        | TRecord fields ->
            let fld_label ~acc (s, (flag, ty)) =
              wrap (Pair.pair s -| Pair.pair flag) ~fld ~acc ty
            in
            wrap_list _TRecord ~fld:fld_label ~acc fields
        | TModSig sgn ->
            let acc, sgn =
              fld_list ~acc sgn ~fld:(fun ~acc elem ->
                  match elem with
                  | Sig_type decls ->
                      let acc, decls =
                        fld_list ~acc decls ~fld:(fun ~acc (s, (params, ty)) ->
                            wrap (Pair.pair s -| Pair.pair params) ~fld ~acc ty)
                      in
                      (acc, Sig_type decls)
                  | Sig_value x ->
                      let acc, x = fld_var ~acc x in
                      (acc, Sig_value x)
                  | Sig_ext (s, (params, { ext_row_path; ext_row_args; ext_row_ret })) ->
                      let acc, ext_row_args = fld_list ~fld ~acc ext_row_args in
                      let acc, ext_row_ret = fld_option ~fld ~acc ext_row_ret in
                      (acc, Sig_ext (s, (params, { ext_row_path; ext_row_args; ext_row_ret }))))
            in
            (acc, TModSig sgn)
        | TModLid _ -> (acc, ty)
        ...
        | TConstraint (ty, cs) ->
            let acc, ty' = fld ~acc ty in
            let acc, cs' =
              fld_list ~acc cs ~fld:(fun ~acc (p, ty) ->
...
let sig_types sgn = List.filter_map (function Sig_type decl -> Some decl | _ -> None) sgn
let sig_values sgn = List.filter_map (function Sig_value x -> Some x | _ -> None) sgn
let sig_exts sgn = List.filter_map (function Sig_ext ext -> Some ext | _ -> None) sgn
...
let rec equal ?(env = []) ty1 ty2 =
  ...
  | TVar (b1, r1, _), TVar (b2, r2, _) ->
      ...
      && (* TODO: need? *)
      (r1 == r2
      || List.exists (fun (r3, r4) -> (r1 == r3 && r2 == r4) || (r1 == r4 && r2 == r3)) env)
  | TFun (x1, ty1), TFun (x2, ty2) -> eq (Id.typ x1) (Id.typ x2) && eq ty1 ty2
  | TFuns _, _ | _, TFuns _ -> [%unsupported]
  ...
  | TRecord fields1, TRecord fields2 ->
      List.equal
        (fun (l1, (f1, ty1)) (l2, (f2, ty2)) -> l1 = l2 && f1 = f2 && eq ty1 ty2)
        fields1 fields2
...
and equal_signature ?(env = []) sgn1 sgn2 =
  ...
  in
  List.equal (List.equal eq_decl_item) (sig_types sgn1) (sig_types sgn2)
  && List.equal
       (fun x y -> Id.eq x y && eq (Id.typ x) (Id.typ y))
       (sig_values sgn1) (sig_values sgn2)
  && List.equal ( = ) (sig_exts sgn1) (sig_exts sgn1)
(* TODO *)
...
let ref_of_tvar typ = match typ with TVar (_, r, _) -> r | _ -> invalid_arg "Type.ref_of_tvar"
...
let copy_tconstr_params (params, ty) =
  ...
  (* (params,ty) is assumed to be closed??? *)
  (env', (params', ty'))
...
let tconstr ?id ?(attr = []) name =
  match id with None -> Id.new_var ~name ~attr () | Some id -> Id.make ~id name ~attr ()

let path ?id ?attr name : path = LId (tconstr ?id ?attr name)

let prim_path name =
  (* [id] must be different from those of variables generated in Parser_wrapper  *)
  let id = -1 in
  let attr = [ Id.Primitive ] in
  path ~id ~attr name
...
module Path = struct
  ...
  let id_of lid = match lid with LId x -> x | _ -> [%invalid_arg]
  let id_opt lid = match lid with LId x -> Some x | _ -> None
  ...
  let append_opt path1 path2 = match path1 with None -> path2 | Some path1' -> append path1' path2
  let rec head lid = match lid with LId x -> x | LDot (p, _) -> head p | LApp _ -> [%invalid_arg]

  let rec heads lid =
    match lid with LId x -> [ x ] | LDot (p, _) -> heads p | LApp (p1, p2) -> heads p1 @ heads p2

  let rec last lid =
    match lid with LId x -> Id.name x | LDot (_, s) -> s | LApp (_, lid2) -> last lid2

  let rec print fm lid =
    match lid with
    | LId x ->
        let s = if !Flag.Print.var_id then Id.to_string x else Id.name x in
  ...
  let rec of_string s =
    ...
    if String.last s = ')' then unsupported "%s" __FUNCTION__
    else
  ...
  let rec is_external lid =
    match lid with LId id -> Id.is_external id | LDot (p, _) | LApp (p, _) -> is_external p

  let rec is_primitive lid =
    match lid with LId id -> Id.is_primitive id | LDot (p, _) | LApp (p, _) -> is_primitive p
  ...
  let rec has_app path =
    match path with LId _ -> false | LDot (path', _) -> has_app path' | LApp _ -> true

  let rec is_prefix path1 path2 =
    if path1 = path2 then true
    else match path2 with LDot (path2', _) -> is_prefix path1 path2' | _ -> false

  let rec subst_head_map map path =
    let sbst = subst_head_map map in
    match path with
    | LId x -> map x |> Option.default path
    | LDot (path1, s) -> LDot (sbst path1, s)
    | LApp (path1, path2) -> LApp (sbst path1, sbst path2)

  let subst_head x lid path = subst_head_map (fun y -> if Id.(x = y) then Some lid else None) path
  ...
  let string_of_option path = match path with None -> "" | Some path -> to_string path ^ "."
...
module C = struct
  ...
  let bytes = prim_path "bytes"
  let result = prim_path "X"
  let conj = prim_path "&" (* conjunctive argument types for polymorphic variants *)
  ...
  let base_prims = [ char; float; string; bytes ]

  let prims =
    [ obj; class_; list; set; ref; option; array; lazy_; unknown; abst; exn; bytes; result ]
    @ base_prims
end

(** [instantiate p1 p2] instantiates [p1] by [p2] ([p1] must be a type variable) *)
let instantiate p1 p2 =
  match p1 with TVar (_, ({ contents = None } as r), _) -> r := Some p2 | _ -> [%invalid_arg]

exception Sig_of_Lid of path

type 'a mod_env = 'a mod_typ list
and 'a mod_typ = unit Id.t * 'a t [@@deriving show]

(* TODO: Support TConstr _ *)
let rec sig_of_mod_typ ~(env : 'a mod_env) ty : _ signature =
  match elim_tattr ty with
  | TModSig sgn -> sgn
  | TModLid p -> sig_of_path ~env p
  | TConstr (_, []) -> [%unsupported]
  | _ -> [%invalid_arg]

and sig_of_path ~env (path : path) : _ signature =
  match path with
  | LId x ->
      let ty = Id.List.assoc x env in
      if ty = TModLid path then [%invalid_arg];
      sig_of_mod_typ ~env ty
  | LDot (p, s) -> (
      let sgn = sig_of_path p ~env in
      try
        List.find_map
          (function
            | Sig_value x when Id.name x = s -> Some (sig_of_mod_typ ~env @@ Id.typ x) | _ -> None)
          sgn
      with Not_found -> [%invalid_arg])
  | LApp _ -> [%invalid_arg]

let sig_of_id ~(env : 'a mod_env) m = sig_of_mod_typ ~env @@ Id.typ m
let is_mod_typ ty = match elim_tattr ty with TModSig _ | TModLid _ -> true | _ -> false
let is_mod_var x = is_mod_typ @@ Id.typ x

(* [Type_util.is_functor_typ] is more appropriate *)
let rec is_functor_typ ty =
  match elim_tattr ty with
  | TFun (x, ty2) -> is_mod_typ (Id.typ x) || is_mod_typ ty2 || is_functor_typ ty2
  | _ -> false
...
let subst_path_head x lid = subst_path_head_map (fun y -> if Id.(x = y) then Some lid else None)
...
let add_prefix_opt prefix path = match prefix with None -> path | Some p -> Path.append p path

let map_sig_elem f elem =
  match elem with
  | Sig_type decls -> Sig_type (List.map (fun (c, (p, ty)) -> (c, (p, f ty))) decls)
  | Sig_ext (c, (p, { ext_row_path; ext_row_args; ext_row_ret })) ->
      ...
      Sig_ext (c, (p, { ext_row_path; ext_row_args; ext_row_ret }))
  | Sig_value x -> Sig_value (Id.map_typ f x)

let cons_prefix m (p, e) =
  let m' = Id.set_typ m () in
  let p' = match p with None -> Some (LId m') | Some path -> Some (Path.cons m' path) in
  (p', e)

let rec flatten_signature ~env sgn : (path option * _ sig_elem) list =
  let f elem =
    (None, elem)
    ::
    (match elem with
    | Sig_value x when is_mod_var x -> flatten_signature_of_mod ~env x
    | _ -> [ (None, elem) ])
  in
  List.flatten_map f sgn

and flatten_signature_of_mod ~env m =
  let sgn = m |> sig_of_id ~env |> flatten_signature ~env in
  let path_map =
    sgn
    |> List.flatten_map (function
         | p, Sig_type decl ->
             ...
             |> List.map (fun (c, _) ->
                    ...
                    (path, Path.cons m path))
         | _ -> [])
  in
  sgn |> List.map (cons_prefix m) |> Fmap.(list (snd (map_sig_elem (subst_path_map path_map))))

let exts_in_signature ~(env : 'a mod_env) sgn =
  sgn
  |> flatten_signature ~env
  |> List.filter_map (function p, Sig_ext x -> Some (p, x) | _ -> None)

and exts_in_module ~(env : 'a mod_env) m =
  m
...
let types_in_signature ~(env : 'a mod_env) sgn : (path option * _ decl_item) list =
  sgn
  |> flatten_signature ~env
  |> List.flatten_map (function p, Sig_type decl -> List.map (Pair.pair p) decl | _ -> [])
...
let fields_in_decl_item (decl : _ decl_item) =
  match decl with _, (_, TRecord fields) -> List.map (fst |- Path._LId) fields | _ -> []

let fields_in_declaration (decl : _ declaration) = decl |> List.rev_flatten_map fields_in_decl_item
...
let rec fields_in_signature ~(env : 'a mod_env) sgn : path list =
  (fields_in_declarations @@ sig_types sgn)
  @@@ (sig_values sgn
...
let rec constrs_in_signature ~(env : 'a mod_env) sgn =
  ...
  in
  let constrs2 = sgn |> sig_types |> constrs_in_declarations |> List.rev_map Path._LId in
  constrs1 @@@ constrs2

(* TODO: use types_in_module or flatten_signature_of_mod *)
and constrs_in_module ?(add_prefix = false) ~(env : 'a mod_env) m =
  try sig_of_id ~env m |> constrs_in_signature ~env |> add_prefix ^> List.map (Path.cons m)
...
let values_in_signature ~(env : 'a mod_env) sgn : (path option * _ tid) list =
  sgn
  |> flatten_signature ~env
  |> List.filter_map (function p, Sig_value x -> Some (p, x) | _ -> None)
...
let add_module_prefix_tconstr_map mod_path (prefix, decl) =
  let prefix' = match prefix with None -> LId mod_path | Some p -> Path.cons mod_path p in
  let make (c, _) = (c, LDot (prefix', Id.name c)) in
  List.map make decl

let assoc_row constr rows = List.find (row_constr |- Id.( = ) constr) rows

let assoc_row_with_pos ~sort constr rows =
  rows
  |> sort ^> List.sort (Compare.on (row_constr |- Id.name))
  |> List.find_mapi (fun i row ->
         if Id.(constr = row.row_constr) then Some (i, row.row_args) else None)
