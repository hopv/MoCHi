...
type t =
  ...
  | Record of (string * (T.mutable_flag * t)) list
  | Exn of t * t
  [@@deriving show]
...
let make_trans ?(tr=S.id_trans) f =
  let tr_term = tr.S.tr_term in
  let tr_var = tr.S.tr_var in
  let rec trans ty =
    match f trans ty with
    | None ->
        begin match ty with
        | Base(base,y,p) -> Base(base, tr_var y, tr_term p)
        | ADT(s,y,p) -> ADT(s, tr_var y, tr_term p)
        ...
        | List(y,p_len,z,p_i,typ) -> List(tr_var y, tr.S.tr_term p_len, tr_var z, tr.S.tr_term p_i, trans typ)
        | App(constr, ty) -> App(constr, trans ty)
...
let rec occur x ty =
  ...
  | Inter(_, typs)
  | Union(_, typs) -> List.exists f typs
...
let rec print is_arg fm ty =
  ...
  match ty with
  | Base(base,_,p) when p = U.true_term ->
  ...
  | Base(_,x,{desc=BinOp(Eq, {desc=Var y}, t)}) when x = y ->
      Format.fprintf fm "{%a}" Print.term t
  | Base(_,x,{desc=BinOp(Eq, t, {desc=Var y})}) when x = y ->
      Format.fprintf fm "{%a}" Print.term t
  | Base(_,_,p) when p.desc = Const False ->
      Format.fprintf fm "Bot"
  | Base(base,x,p) ->
      Format.fprintf fm "@[{%a:%a |@ %a}@]" Id.print x Type.print_base base Print.term p
  | ADT(s,_,p) when p.desc = Const True ->
...
let map_pred f =
  let tr = {(S.make_trans ()) with S.tr_term=f} in
...
let subst_adt ?(force=false) s ty1 =
  let aux _ ty2 =
    match ty2 with
    | ADT(s',_,t) when s = s' ->
...
let rec rename full var ty =
  ...
  | App(constr, ty) -> App(constr, f var ty)
...
let rec of_simple typ =
  ...
  | T.TData s -> ADT(s, Id.new_var typ, U.true_term)
  ...
  | T.TVariant(_,labels) -> Variant (List.map (Pair.map_snd @@ List.map of_simple) labels)
  | T.TRecord fields -> Record (List.map (Pair.map_snd @@ Pair.map_snd of_simple) fields)
  | _ ->
...
let rec to_abst_typ ?(decomp_pred=false) ?(with_pred=false) typ =
  ...
  | Base(base, x, ({S.desc=S.Const _} as p)) ->
      ...
      if with_pred then
        ...
        let ps = if decomp_pred then U.decomp_and p else [p] in
  ...
  | Inter(styp, typs)
  | Union(styp, typs) ->
      List.fold_right (U.merge_typ -| f) typs styp
  ...
  | List(x,p_len,y,p_i,typ1) ->
      if p_i.S.desc <> S.Const S.True || occur y typ1 then
        ...
        let x' = Id.new_var ~name:"xs" @@ T.make_tlist @@ f typ1 in
        if p_len = U.true_term then
          Id.typ x'
        else
          U.(add_tapred x' Term.[(x |-> length (var x')) p_len] @@ Id.typ x')
  | App(constr, ty) -> T.make_tconstr constr @@ f ty
...
let rec to_simple ?(with_pred=false) typ =
  ...
  | Fun(x,typ1,typ2) ->
      ...
      T.TFun(x', f typ2)
  | Tuple xtyps -> T.make_ttuple (List.map (f -| snd) xtyps)
  | Inter(sty, _)
  | Union(sty, _) ->
      if with_pred then
        to_abst_typ typ
  ...
  | List(_,_,_,_,typ) -> T.make_tlist @@ f typ
  | App(constr, ty) -> T.make_tconstr constr @@ f ty
  | Variant labels -> T.TVariant(false, List.map (Pair.map_snd @@ List.map f) labels)
...
let copy_fun_arg_to_base =
  let aux f ty =
    match ty with
    | Fun(x,typ1,typ2) ->
        Some (Fun(x, set_base_var x @@ f typ1, f typ2))
    | _ -> None
  in
  make_trans aux
...
let rec has_no_predicate typ =
  ...
  match typ with
  | Base(_, _, t) -> t = U.true_term
  | ADT(_, _, t) -> t = U.true_term
  | Fun(_,typ1,typ2) -> f typ1 && f typ2
  | Tuple xtyps -> List.for_all (f -| snd) xtyps
  | Inter(_, typs)
  | Union(_, typs) -> List.for_all f typs
  | ExtArg _ -> unsupported "f"
  | List(_,p_len,_,p_i,typ1) -> p_len = U.true_term && p_i = U.true_term && f typ1
  | App(_,ty) -> f ty
...
let rec subtype env typ1 typ2 =
  ...
  | Tuple xtyps1, Tuple xtyps2 ->
      let aux (env,acc) (x1,typ1) (_x2,typ2) = (* TODO: Fix to use x2 *)
  ...
  | Inter(styp, typs) ->
      let _Inter' styp typs =
        if typs <> [] && List.for_all (function Tuple _ -> true | _ -> false) typs then
          ...
          Tuple (List.map2 (fun x typs -> x, f @@ _Inter (to_simple @@ List.hd typs) typs) xs typss')
        else
          _Inter styp typs
      in
...
let replace_data_with_int =
  let new_base p = Base(T.TInt, Id.new_var T.Ty.int, p) in
  let aux _ ty =
    match ty with
    | ADT(_, _, p) ->
        ...
        Some (new_base p)
    | App _
    | Variant _ -> Some (new_base U.Term.true_)
    | _ -> None
  in
  make_trans aux
...
let replace_list_with_int =
  let aux _ ty =
    match ty with
    | App("list", _) -> Some (Base(T.TInt, Id.new_var T.Ty.int, U.Term.true_))
    | _ -> None
  in
  make_trans aux

let replace_data_with_int_but_exn exn_constrs =
  let new_base p = Base(T.TInt, Id.new_var T.Ty.int, p) in
  let aux _ ty =
    match ty with
    | ADT(s, _, _) when List.mem s exn_constrs -> None
    | ADT(_, _, p) ->
        if p <> U.true_term then unsupported "-data-to-int-but-exn";
        Some (new_base p)
    | App _
    | Variant _ -> Some (new_base U.Term.true_)
    | _ -> None
...
let rec is_safe_fun ty =
  ...
  | App(_,_) -> unsupported "is_safe_fun App"
...
let adt_to_prim =
  let aux _ ty2 =
    match ty2 with
    | ADT(s,x,t) when List.mem s Type.prim_base_types -> Some (Base(Type.(TPrim s), x, t))
    | _ -> None
  in
  make_trans aux
