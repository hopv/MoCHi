...
let make_trans ?(tr=S.Tr.id) f =
  ...
  let rec trans ty =
    ...
    | None ->
        ...
        | List(y,p_len,z,p_i,typ) -> List(tr_var y, tr.term p_len, tr_var z, tr.term p_i, trans typ)
...
let rec print is_arg fm ty =
  ...
  match ty with
  | Base(base,_,p) when p = SU.true_term ->
      Format.fprintf fm "%a" Print_typ.base base
  | Base(T.TBool,x,p) when SU.make_var x = p ->
      Format.fprintf fm "{true}"
  | Base(T.TBool,x,p) when SU.make_not (SU.make_var x) = p ->
  ...
  | Constr(s,[],_,p) when p.desc = Const True ->
      Format.fprintf fm "%a" Print_typ.path s
  | Constr(s,[],x,p) ->
  ...
  | Constr(constr, tys, _, p) when p.desc = Const True ->
      ...
      Format.fprintf fm "%s@[%a %s@]%s" s1 (print_list (print true) ",") tys (TU.string_of_path constr) s2
  | Constr(constr, tys, x, p) ->
      Format.fprintf fm "@[{%a:%a %s |@ %a}@]" Id.print x (print_list (print true) ",") tys (TU.string_of_path constr) Print.term p
  ...
  | Variant labels ->
      let pr fm (s, typs) =
        if typs = [] then
          ...
          Format.fprintf fm "@[%a of %a@]" Print_typ.constr s (print_list pr "@ *@ ") typs
      in
...
let rec of_simple typ =
  ...
  | TConstr(c,[typ]) when c = T.C.list ->
      List(Id.new_var TU.Ty.int,
           SU.true_term,
           Id.new_var TU.Ty.int,
           SU.true_term,
           of_simple typ)
...
let rec to_abst_typ ?(decomp_pred=false) ?(with_pred=false) typ =
  ...
  | Base(base, x, ({S.desc=S.Const _} as p)) ->
      ...
      if with_pred then
        TU.add_tattr (T.TARefPred(x,p)) sty
      else
  ...
  | Base(base, x, t) ->
      ...
      let ps =
        if !Flag.PredAbst.decomp_pred then
          SU.decomp_bexp @@ SU.subst_var x x' t
        else
          [SU.subst_var x x' t]
      in
  ...
  | Inter(styp, typs)
  | Union(styp, typs) ->
      List.fold_right (SU.merge_typ -| f) typs styp
  ...
  | List(x,p_len,y,p_i,typ1) ->
      if p_i.S.desc <> S.Const S.True || occur y typ1 then
        ...
        if p_len = SU.true_term then
          ...
          SU.(add_tapred x' Term.[(x |-> length (var x')) p_len] @@ Id.typ x')
...
let rec to_simple ?(with_pred=false) typ =
  ...
  | Base(b, x, p) ->
      T.TBase b
      |&with_pred&> TU.add_tattr (T.TARefPred(x,p))
  | Constr(s, tys, x, p) ->
      T._TConstr s @@ List.map f tys
      |&with_pred&> TU.add_tattr (T.TARefPred(x,p))
  | Fun(x,typ1,typ2) ->
  ...
  | Variant labels -> T.TVariant(VNonPoly, List.map (fun (row_constr,row_args) -> {Type.row_constr; row_args = List.map f row_args; row_ret = None}) labels)
...
let make_ref ty = Constr(TU.lid_of_string "ref", [ty], Id.new_var (TU.Ty.ref @@ to_simple ty), SU.true_term)
...
let rec same typ1 typ2 =
  ...
  | Base(base1,x1,p1), Base(base2,x2,p2) -> base1 = base2 && SU.same_term p1 @@ SU.subst_var x2 x1 p2
...
let rec subtype env typ1 typ2 =
  ...
  | List(x1,p_len1,y1,p_i1,typ1'), List(x2,p_len2,y2,p_i2,typ2') ->
      let typ1'' = Tuple [x1,Base(T.TInt,x1,p_len1);
                          Id.new_var TU.Ty.int,Fun(y1,Base(T.TInt,y1,p_i1),typ1')] in
      let typ2'' = Tuple [x2,Base(T.TInt,x2,p_len2);
                          Id.new_var TU.Ty.int,Fun(y2,Base(T.TInt,y2,p_i2),typ2')] in
...
and simplify typ =
  ...
  | Constr(s, tys, x, p) ->
      ...
      if p' = SU.false_term
      then Union(to_simple typ, [])
      else Constr(s, List.map f tys, x, p')
...
let replace_list_with_int =
  let aux _ ty =
    match ty with
    | List _ -> Some (Base(T.TInt, Id.new_var TU.Ty.int, SU.Term.true_))
    | _ -> None
  in
  make_trans aux
