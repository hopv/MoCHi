...
let rec from_pattern_desc : type k. env -> k pattern_desc -> typ -> Env.t -> env * pat_desc =
  fun env desc typ pat_env ->
  match desc with
  | Tpat_any -> env, PAny
  ...
  | Tpat_constant(Const_int n) -> env, PConst {desc=Const(Int n);typ; attr=[]}
  | Tpat_constant(Const_char c) -> env, PConst {desc=Const(Char c);typ; attr=[]}
  | Tpat_constant(Const_string(s,_,_)) -> env, PConst {desc=Const(String s);typ; attr=[]}
  | Tpat_constant(Const_float s) -> env, PConst {desc=Const(Float (float_of_string s));typ; attr=[]}
  | Tpat_constant(Const_int32 n) -> env, PConst {desc=Const(Int32 n);typ; attr=[]}
  | Tpat_constant(Const_int64 n) -> env, PConst {desc=Const(Int64 n);typ; attr=[]}
  | Tpat_constant(Const_nativeint n) -> env, PConst {desc=Const(Nativeint n);typ; attr=[]}
  | Tpat_tuple ps ->
  ...
  | Tpat_value _ -> assert false
  | Tpat_exception _ -> unsupported "pattern match (exception)"

and from_pattern : type k. env -> k general_pattern -> env * pattern =
  fun env ({pat_desc=desc; pat_loc=_; pat_type=typ; pat_env} as pat) ->
  let env,pat_typ = from_type_expr env pat_env typ in
  let env,pat_desc =
    match desc with
    | Tpat_value p ->
        let p' = Option.get @@ fst @@ split_pattern {pat with pat_desc=Tpat_value p} in
        from_pattern_desc env p'.pat_desc pat_typ pat_env
    | _ -> from_pattern_desc env desc pat_typ pat_env
  in
  env, {pat_desc; pat_typ}
...
let rec from_expression env {exp_desc; exp_loc; exp_type; exp_env=tenv; exp_attributes} : env * term =
  ...
  let env,t =
    ...
    | Texp_letmodule(None, _loc, _, _mb_expr, _e) -> unsupported "expression (let module _ = ...)"
    | Texp_letmodule(Some m, _loc, _, mb_expr, e) ->
...
and from_case : type k. Parser_wrapper_common.env -> k case -> Parser_wrapper_common.env * (Syntax.pattern * term) =
...
and from_module_binding env _tenv mb =
  match mb.mb_id with
  | None -> unsupported "unanmed modules"
  | Some mb_id -> from_module_expr_top env mb_id mb.mb_expr
...
and from_module_type env tenv mty =
  ...
  | Mty_signature sgn ->
      ...
      env, TModule sgn
  | Mty_functor(Unit, _mty2) -> unsupported "Mty_functor"
  | Mty_functor(Named (None, _mty1), _mty2) -> unsupported "Mty_functor"
  | Mty_functor(Named (Some id, mty1), mty2) ->
...
and from_module_expr env mb_expr : env * term =
  ...
  | Tmod_functor(Unit, _expr) -> unsupported "Tmod_functor"
  | Tmod_functor(Named (None, _loc, _mty), _expr) -> unsupported "Tmod_functor"
  | Tmod_functor(Named (Some id, _loc, mty), expr) ->
      let ty =
