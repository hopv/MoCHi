...
let add_id id ty = _TAttr [ _TAId id ] ty
...
let get_id ty = ty |> decomp_tattr |> fst |> get_id_attr
...
let rec flatten_sub ty1 ty2 constr =
  let flatten_sub_var x y = flatten_sub (Id.typ x) (Id.typ y) in
  Debug.printf "FLATTEN: @[@[%a@] <:@ @[%a@." Print.typ ty1 Print.typ ty2;
  match (ty1, ty2) with
  | TAttr (attr1, TBase _), TAttr (attr2, TBase _) -> flatten_sub_attr attr1 attr2 constr
  | TBase _, _ -> assert false
  | TVar _, _ -> unsupported "%s" __FUNCTION__
  | TAttr (attr1, TFun (x1, ty12)), TAttr (attr2, TFun (x2, ty22)) ->
      constr |> flatten_sub_attr attr2 attr1 |> flatten_sub_var x2 x1 |> flatten_sub ty12 ty22
  | TFun _, _ -> assert false
  | TFuns _, _ -> unsupported "%s" __FUNCTION__
  | TAttr (attr1, TTuple xs1), TAttr (attr2, TTuple xs2) ->
      constr |> flatten_sub_attr attr1 attr2 |> List.fold_right2 flatten_sub_var xs1 xs2
  ...
  | TAttr (attr1, TConstr (_, tys1)), TAttr (attr2, TConstr (_, tys2)) ->
      constr |> flatten_sub_attr attr1 attr2 |> List.fold_right2 flatten_sub tys1 tys2
  | TConstr _, _ -> assert false
  | TAttr (_, ty1'), ty2' | ty1', TAttr (_, ty2') -> flatten_sub ty1' ty2' constr
...
let rec gen_constr env l t ty =
  ...
  let env = env |> add_constr l l_t |> flatten_sub typ ty in
  let env, desc =
    match t.desc with
    | Const (Rand (_, true)) -> [%unsupported]
    | Const (Rand (_, false)) | Const _ | Bottom ->
        let env = add_constr l_t (get_id typ) env in
        (env, t.desc)
    | Var (LId x) ->
        let typ' =
          if Id.List.mem x env.fv then set_id 0 ty
          else
            try Id.List.assoc x env.tenv
            with Not_found ->
              Format.eprintf "%a@." Print.id x;
              assert false
        in
        let env = env |> flatten_sub typ' typ |> add_constr l_t (get_id typ) in
    ...
    | Fun (x, t1) ->
        ...
        let env = env |> add_constr l_t l' |> add_constr l_t l_f in
    ...
    | If (t1, t2, t3) ->
        ...
        let env =
          ...
          if env.respect_div && List.mem EDiv effect then add_constr l' care env else env
        in
...
let solve env t =
  ...
  Array.iteri
    (fun i x -> if x then Debug.printf "  %d%s@." i (if List.mem i ids then "*" else ""))
    sol;
  fun x ->
...
let can_remove sol t =
  let ty = t.typ in
  let effect = effect_of_typ ty in
  is_base_typ ty && sol (get_id ty) && effect = []

let remove_dont_care =
  let tr = Tr2.make () in
  tr.term <-
    (fun sol t ->
      if can_remove sol t && t.desc <> Const Unit then
        let () = Debug.printf "REMOVE %d@." (get_id t.typ) in
        let () = Debug.printf "%a@.@." Print.term' t in
        make_dummy t.typ |> add_attr (AInfo (Trans.Removed [ (new_var_of_term t, t) ]))
      else tr.term_rec sol t |> remove_attr_if (function AId _ -> true | _ -> false));
  tr.typ <- Fun.snd;
  Fun.uncurry tr.term

let pr s t = Debug.printf "%s: %a@." s Print.term' t

(* [respect_div = false] is buggy *)
let slice ?(respect_div = true) t =
  ...
  |> Trans.elim_unused_let ~annot:true
  |> Trans.reconstruct
  |@> Debug.printf "SLICED: %d@." -| size

let slice_problem p : Problem.t * Preprocess_common.trans_cex =
  let p' = Problem.map (slice ~respect_div:true) p in
  ( Problem.map Trans.remove_info_removed p',
    function [ (_, _, (lazy (ce, main))) ] -> (Eval.trans ce p', main) | _ -> assert false )
