...
let print_constr fm = function x, y -> Format.fprintf fm "{%d} = {%d}" x y
...
let rec print fm typ =
  ...
  | TTuple typs -> Format.fprintf fm "(@[<hov 2>%a@])" (print_list print "@ *@ ") typs
...
let add_env_var env x =
  if false then Format.fprintf !Flag.Print.target "ADD: %a@." Id.print x;
  add_env env @@ IVar (Id.to_string x)
...
let rec unify typ1 typ2 =
  ...
  | TTuple typs1, TTuple typs2 -> List.flatten @@ List.map2 unify typs1 typs2
  | TVar r1, TVar r2 when r1 == r2 -> []
  | TVar ({ contents = None } as r), typ | typ, TVar ({ contents = None } as r) ->
      r := Some typ;
      []
  | _ -> assert false
...
let rec infer (env, counter) t =
  match t.desc with
  | Event _ | Const _ | Bottom -> unify (get_typ env t) @@ from_type t.typ
  ...
  | App (t1, ts) ->
      ...
      if false then
        if !id <> None then Format.fprintf !Flag.Print.target "LET: %d@." @@ Option.get !id;
  ...
  | Local (Decl_let bindings, t2) ->
      let aux (f, t1) =
        ...
        if false then
          Format.fprintf !Flag.Print.target "id[%a]: %a@." Id.print f
            (Option.print Format.pp_print_int)
            !id;
        if false then
          if !id <> None then Format.fprintf !Flag.Print.target "LET: %d@." @@ Option.get !id;
        let constr1 =
  ...
  | BinOp (_, t1, t2) -> infer (env, counter) t1 @ infer (env, counter) t2
  | Not t1 -> infer (env, counter) t1
  | If (t1, t2, t3) -> infer (env, counter) t1 @ infer (env, counter) t2 @ infer (env, counter) t3
  | Tuple ts -> List.flatten_map (infer (env, counter)) ts
  | Proj (_, t) -> infer (env, counter) t
  | _ ->
      Format.eprintf "%a@." Print.term t;
...
let rec solve sets constr =
  if false then
    if constr <> [] then Format.fprintf !Flag.Print.target "%a@." print_constr @@ List.hd constr;
  if false then Format.fprintf !Flag.Print.target "rest: %d@." @@ List.length constr;
  if false then
    Format.fprintf !Flag.Print.target "sets: %a@.@." (List.print @@ List.print Format.pp_print_int)
    @@ List.map IntSet.elements sets;
  match constr with
  | [] ->
  ...
  | Var (LId x) ->
      let rec aux typ1 typ2 =
        ...
        | typs, typ1' ->
            ...
            Type.TFun
              ( new_var_of_term @@ make_tuple @@ List.map make_var xs1,
                aux typ1' @@ List.fold_right _TFun xs2 typ2' )
      in
  ...
  | Local (Decl_let bindings, t2) ->
      let aux (f, t1) =
        ...
        let aux' ys t1 =
          ...
          | _ ->
              make_fun y @@ subst_map (List.mapi (fun i z -> (z, make_proj i @@ make_var y)) ys) t1
        in
        let t1' = uncurry.term (env, sol) t1 in
...
let uncurry t =
  ...
  t' |> uncurry (env, sol) |> Trans.remove_id
...
let to_tfuns_desc (env, sol) desc =
  ...
  | Local (Decl_let bindings, t) ->
      ...
      let bindings'' =
        List.map (Pair.map_snd @@ List.fold_right (Fun.uncurry subst_var) map) bindings'
      in
      Local (Decl_let bindings'', t')
