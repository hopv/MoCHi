...
and node = {
  ...
  var_env : var_env;
      (* Scope environment: var_env(x) is a set of variables which are visible from x *)
...
let rec pr_bind depth fm (x, vl) =
  ...
  else if depth <= 0 then Format.fprintf fm "@[%a := %a@]" Id.print x Print.term t
  else Format.fprintf fm "@[%a := %a%a@]" Id.print x (pr_env (depth - 1)) val_env Print.term t
...
and print_nlabel fm nlabel =
  match nlabel with
  | App (f, map) when true -> Format.fprintf fm "@[App %a %a@]" Id.print f pr_env map
  | App (f, _) -> Format.fprintf fm "@[App %a ...@]" Id.print f
  | Let (f, t) when false -> Format.fprintf fm "@[Let %a =@ %a@]" Id.print f Print.term t
  | Let (f, _) -> Format.fprintf fm "@[Let %a =@ ...@]" Id.print f
...
and print_value fm (Closure (_var_env, _val_env, t)) = Format.fprintf fm "Value %a" Print.term t

and print_node fm { nid; val_env; ce_env; nlabel } =
  if false then
    Format.fprintf fm "%d,@ @[(*%a*)@],@ @[%a@]" nid (List.print Id.print) (List.map fst val_env)
      print_nlabel nlabel
  else Format.fprintf fm "%d,@ @[%a@],@ %a" nid print_nlabel nlabel (List.print print_ce) ce_env

let rec print fm (Rose_tree.Node (node, ts)) =
  Format.fprintf fm "(@[<hov>%a,@ %a@])" print_node node (List.print print) ts

let counter = new counter
let make_new_tid () = counter#gen
let get_tid t = get_id t
let get_tid_option t = get_id_option t
let rename_tid_if = Tr2.make ()

let rename_tid_if_term map t =
  match (t.desc, get_tid_option t) with
  | If _, Some tid -> replace_id tid (List.assoc tid map) @@ rename_tid_if.term_rec map t
  | _ -> rename_tid_if.term_rec map t

let () = rename_tid_if.term <- rename_tid_if_term
let rename_tid_if = rename_tid_if.term
let term_of_value (Closure (_, _, t)) = t
let var_env_of_value (Closure (env, _, _)) = env
let val_env_of_value (Closure (_, env, _)) = env
let is_fail t = match t.desc with App ({ desc = Event ("fail", _) }, [ _ ]) -> true | _ -> false

let rec arity fun_env val_env t =
  assert (is_value t);
  match t.desc with
  | Var (LId f) when Id.List.mem_assoc f fun_env -> List.length @@ fst @@ Id.List.assoc f fun_env
...
let assoc_fun _nid f _var_env val_env =
  let (Closure (var_env_f, val_env_f, t)) = Id.List.assoc f val_env in
  let ys, t' =
    match t.desc with
    | Var (LId f) ->
    ...
    | _ -> decomp_funs t
  in
  let ys' = List.mapi (fun i y -> Id.make (Format.asprintf "%a@%d" Id.print f i) (Id.typ y)) ys in
  Dbg.printf "    ALPHA: %a => %a@\n" (List.print Id.print) ys (List.print Id.print) ys';
...
let rec from_app_term nid f ts (cnt : counter) (* counter for nid *)
    (fun_env : (id * (term * ce list)) list) (var_env : var_env)
    (* scope environment *) (val_env : val_env) (* value environment *) (ce_env : ce_env)
    (* counterexamples *) (spawned : Syntax.id list) (* list of ids spawned *) (depth : int)
    (t : term) (* term to be reduced *) =
  Dbg.printf "  APP: %a@\n" Print.term t;
  let var_env_f, val_env_f, (ys, t_f) = assoc_fun nid f var_env val_env in
  Dbg.printf "    APP (ys -> t_f): %a, %d@\n" Print.term (make_funs ys t_f) (List.length ts);
  Dbg.printf "    APP var_env_f: %a@\n"
    (List.print @@ Pair.print Id.print @@ List.print Id.print)
    var_env_f;
  let arg_map = make_arg_map var_env val_env f ys ts in
  Dbg.printf "    APP arg_map: %a@\n" pr_env arg_map;
  let val_env' = List.rev arg_map @ val_env_f in
  Dbg.printf "    APP val_env': %a@\n" pr_env val_env';
  let vars_f = Id.List.assoc f var_env in
  let var_env', _ =
    List.fold_left (fun (acc, vars) x -> ((x, vars) :: acc, x :: vars)) (var_env_f, vars_f) ys
  in
  Dbg.printf "    APP var_env': %a@\n"
    (List.print @@ Pair.print Id.print @@ List.print Id.print)
    var_env';
...
and from_term (cnt : counter) (* counter for nid *) (fun_env : (id * (term * ce list)) list)
    (var_env : var_env) (* scope environment *) (val_env : val_env)
    (* value environment *) (ce_env : ce_env) (* counterexamples *) (spawned : Syntax.id list)
    (* list of ids spawned *) (depth : int) (t : term) : t =
  (* term to be reduced *)
  let nid = cnt#gen in
  Dbg.printf "TERM: %a@\n" Print.term t;
  Dbg.printf "  val_env: %a@\n" pr_env val_env;
  Dbg.printf "  Dom(var_env): %a@\n" (List.print Id.print) @@ List.map fst var_env;
  Dbg.printf "  Dom(val_env): %a@\n" (List.print Id.print) @@ List.map fst val_env;
  ...
  | Const Unit ->
      ...
      RT.Node (node, [])
  | App
      ({ desc = Const (Rand (TBase TInt, true)) }, [ { desc = Const Unit }; { desc = Fun (x, t2) } ])
    ->
      let t2' = subst_var x (rename_var nid x) t2 in
      from_term cnt fun_env var_env val_env ce_env spawned depth t2'
  | App ({ desc = Var (LId f) }, ts) when Id.List.mem_assoc f fun_env ->
      (* Top-level functions *)
      Dbg.printf "  APP2,%a@\n" Id.print f;
      let t_f, ce_env_f = Id.List.assoc f fun_env in
      let f' = Id.make (Format.asprintf "%a:%d" Id.print f nid) (Id.typ f) in
      Dbg.printf "    SPAWN: %a => %a@\n" Id.print f Id.print f';
      let tid_map = t_f |> col_id |> List.map (fun x -> (x, make_new_tid ())) in
      Dbg.printf "tid_map: %a@."
      ...
      let child =
        ...
        let val_env' =
          let t_f' = t_f |*> Trans.alpha_rename |> subst_var f f' |> rename_tid_if tid_map in
          let rec val_env_f' = [ (f', Closure ([ (f', []) ], val_env_f', t_f')) ] in
          val_env_f' @ val_env
        in
        let ce_env' =
          let ce_env_f' = Fmap.(list (list (fst (List.subst tid_map)))) ce_env_f in
          ce_env_f' @ ce_env
        in
        from_app_term cnt#gen f' ts cnt fun_env var_env' val_env' ce_env' spawned depth t'
      in
      let node = { nid; var_env; val_env; ce_env; nlabel = Spawn f } in
      RT.Node (node, [ child ])
  | App ({ desc = Fun (x, t) }, t2 :: ts) ->
      assert (
        is_simple_aexp t2
        || is_simple_bexp t2
        || is_randint_unit t2
  ...
  | App ({ desc = Var (LId f) }, ts) ->
      let f', var_env', val_env', t' =
        if true || List.exists (fst |- same_top_fun f) fun_env then
          ...
          let var_env' = (f', Id.List.assoc f var_env) :: var_env in
          let val_env' = (f', Id.List.assoc f val_env) :: val_env in
          let t' = subst_var f f' t in
          (f', var_env', val_env', t')
        else (f, var_env, val_env, t)
      in
        ...
        | Some tid ->
            ...
            let then_ce_env, else_ce_env =
              ...
              let aux (t_acc, e_acc) ce =
                let ce' = pop ce in
                if peek ce then (ce' :: t_acc, e_acc) else (t_acc, ce' :: e_acc)
              in
              List.fold_left aux ([], []) ce_env1
            in
            let child1 =
              if then_ce_env = [] then if is_fail t3 then [ aux true ce_env2' depth ] else []
              else [ aux true (then_ce_env @ ce_env2') depth ]
            in
            let child2 =
              if else_ce_env = [] then
                if then_ce_env = [] && is_fail t3 then [ aux false [] depth ] else []
              else [ aux false (else_ce_env @ ce_env2') depth ]
            in
            if child1 = [] && child2 = [] && ce_env2' <> [] && depth > 0 then (
              Dbg.printf "STUCK@.";
              [ aux true ce_env2' (depth - 1); aux false ce_env2' (depth - 1) ])
            else child1 @ child2
      in
      let node = { nid; val_env; var_env; ce_env; nlabel = Branch tid } in
      RT.Node (node, children)
  | Local (Decl_let [ (_, ({ desc = Bottom } as t1)) ], _) ->
      from_term cnt fun_env var_env val_env ce_env spawned depth t1
  | Local (Decl_let [ (_, t1) ], _) when is_fail t1 ->
      from_term cnt fun_env var_env val_env ce_env spawned depth t1
  ...
  | _ ->
      ...
      let f = match t.desc with App ({ desc = Var (LId f) }, _) -> f | _ -> assert false in
      Format.eprintf "%a is in Dom(val_env)?: %b@." Print.id f (Id.List.mem_assoc f val_env);
      unsupported "Comp_tree.from_term"

let from_term fun_env var_env val_env ce_env t =
  from_term (new counter) fun_env var_env val_env ce_env t

let rec filter_ends (RT.Node (node, ts)) =
  let check (RT.Node ({ nlabel }, ts)) = ts <> [] || nlabel = Fail (* || nlabel = End*) in
  let ts' = ts |> List.map filter_ends |> List.filter check in
  RT.Node (node, ts')

let from_program (fun_env : (id * term) list) (ce_set : ce_set) depth t =
  counter#reset;
  Dbg.printf "@.CE_SET: %a@." print_ce_set ce_set;
  Dbg.printf "FUN_ENV: %a@." (List.print @@ Pair.print Id.print Print.term) fun_env;
  Dbg.printf "from_program: %a@." Print.term t;
  let fun_env' = List.map (fun (f, t) -> (f, (t, Id.List.assoc_all f ce_set))) fun_env in
  let var_env = [] in
  let val_env = [] in
  let ce_env = [] in
  t
  |> from_term fun_env' var_env val_env ce_env [] depth
  |@> Dbg.printf "@.@.comp_tree:@.%a@.@." print
