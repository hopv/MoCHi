...
let num_of_lets =
  ...
  let col_decl decl =
    match decl with
    | Decl_let defs -> List.fold_left (fun acc (_, t) -> acc + 1 + col.term t) 0 defs
    | _ -> col.decl_rec decl
  in
  col.decl <- col_decl;
  col.term
...
let info = AInfo (String "Shrinked")
...
let shrink_let_binding =
  ...
  let tr_decl n decl =
    if !n < 0 then decl
      ...
      match decl with
      | Decl_let defs when !n < List.length defs ->
          if has_info (snd @@ List.nth defs !n) then raise AlreadyShrinked
          else
...
let get_error f t =
  let error = try Ok (f t) with e -> Error (Printexc.to_string e) in
  match error with Ok _ -> raise Error_not_found | Error e -> e

let postprocess = remove_info
...
let rec apply ?prev shrink conf =
  ...
  log "count: %d" count;
  let target_count =
    match prev with None -> 0 | Some (count', target) -> if count = count' then 0 else target
  in
  let rec loop error_reject target_count =
    ...
    if target_count >= count then { conf with error_reject }
    else
      try
        let target = shrink (ref target_count) conf.target in
        if conf.target = target then (
