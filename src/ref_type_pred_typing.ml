      ...
      | Local(Decl_type new_decls, t) ->
          let f decls (s,(_params,ty)) =
            ...
            | TVariant(VNonPoly,rows) ->
                List.L.fold_left rows ~init:decls ~f:(
                  fun decls row -> Map.add row.row_constr (row.row_args, TConstr(LId s,[])) decls
                )
            | _  -> decls
          in
  ...
  let type_pat env pat_ty (pat,ret) =
    ...
    | PConstr(false,l,ps) ->
        ...
        if not (pat_ty = ty) then begin
          Format.eprintf "Type error: constructor %a does not have type %a@."
            Print.constr l
            Print.typ pat_ty;
          assert false
        end;
        let ps', binds' =
          try
            List.fold_left
              ...
              | PVar(x) ->
                  List.snoc ps' (Term_util.make_pvar @@ Id.set_typ x ty),
                  IdMap.add x ty binds'
              | PAny ->
                  List.snoc ps' (Term_util.make_pany ty),
                  binds'
              | _ -> assert false
        ...
        make_pattern (PConstr(false,LId l,ps')) pat_ty,
        tr.term env' ret
    | _ ->
        make_pattern pat.pat_desc pat_ty,
        tr.term env ret
  in
