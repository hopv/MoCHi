...
module Id = struct
  ...
  let new_var ?name ?(attr=[]) typ =
    let name =
      match name with
      | None -> Type_util.var_name_of typ
      | Some name -> name
    in
...
module PredVar = struct
  ...
  let new_pvar ?(name=pvar_name) ?id tys =
    ...
    match id with
    | None -> Id.new_var ~name ~attr ty
    | Some id -> Id.make ~id name ~attr ty
end
...
let copy_tvar_list ?(env=[]) ts =
  List.fold_left_map (fun env t -> copy_tvar ~env t) env ts
...
let can_unify ?env ?(sub=false) ?(ignore_poly_variant=false) ty1 ty2 =
  ...
  try
    ...
    true
  with CannotUnify(Some(ty1, ty2)) when ignore_poly_variant ->
         begin
           match ty1, ty2 with
...
let merge_attrs attrs t = add_attrs (List.filter_out (function AFV _ -> true | _ -> false) attrs) t
...
let rec make_app ?env t ts =
  let check typ1 typ2 =
    ...
    | None -> ()
    | Some env ->
        if not (Flag.Debug.check_typ && not (is_mod_typ typ2) => can_unify ~env ~sub:true typ1 typ2) then
          begin
  ...
  | _, typ, t2::ts when typ = typ_unknown || is_tvar typ ->
      make_app (make (App(t,[t2])) typ ~attr) ts
  | _ when not Flag.Debug.check_typ ->
      make (App(t,ts)) typ_unknown ~attr
  | _ ->
...
let make_let_type (decls:type_decls) t2 =
...
let make_and t1 t2 =
  if t1 = false_term then
    ...
    make (BinOp(And, t1, t2)) (TBase TBool) ~attr:(make_attr[t1;t2])
...
let make_or t1 t2 =
  if t1 = true_term then
    ...
    make (BinOp(Or, t1, t2)) (TBase TBool) ~attr:(make_attr[t1;t2])
...
let make_add t1 t2 =
  if t2.desc = Const (Int 0) then
    ...
    make (BinOp(Add, t1, t2)) (TBase TInt) ~attr:(make_attr[t1;t2])

let make_sub t1 t2 =
  if t2.desc = Const (Int 0) then
    t1
  else
    make (BinOp(Sub, t1, t2)) (TBase TInt) ~attr:(make_attr[t1;t2])
...
let make_mul t1 t2 =
  if t1.desc = Const (Int 0) && has_safe_attr t2 then
    ...
    make (BinOp(Mult, t1, t2)) (TBase TInt) ~attr:(make_attr[t1;t2])

let make_div t1 t2 =
  make (BinOp(Div, t1, t2)) (TBase TInt) ~attr:(make_attr[t1;t2])
...
let make_if_ t1 t2 t3 =
  ...
  | _ ->
      let typ =
        ...
        | true, true ->
            if t2.typ <> t3.typ
            then warning " @[<hv 2>if-branches have different types@ %a and@ %a@]" Print.typ t2.typ Print.typ t3.typ;
            t2.typ
      in
...
let make_eq t1 t2 =
  ...
  | _ ->
      if t1.typ = Ty.unit && has_safe_attr t1 && has_safe_attr t2 then
        ...
        make (BinOp(Eq, t1, t2)) (TBase TBool) ~attr:(make_attr[t1;t2])

let make_neq t1 t2 =
  make_not (make_eq t1 t2)

let make_lt t1 t2 =
  make (BinOp(Lt, t1, t2)) (TBase TBool) ~attr:(make_attr[t1;t2])

let make_gt t1 t2 =
  make (BinOp(Gt, t1, t2)) (TBase TBool) ~attr:(make_attr[t1;t2])

let make_leq t1 t2 =
  make (BinOp(Leq, t1, t2)) (TBase TBool) ~attr:(make_attr[t1;t2])

let make_geq t1 t2 =
  make (BinOp(Geq, t1, t2)) (TBase TBool) ~attr:(make_attr[t1;t2])
...
let make_pwhen p t =
  if t.desc = Const True then
    ...
    let p',t' =
      ...
      | PWhen(p', t') -> p', make_and t t'
      | _ -> p, t
    in
    make_pattern (PWhen(p',t')) p.pat_typ

let make_plazy p =
  make_pattern (PLazy p) (Ty.lazy_ p.pat_typ)

let make_peval x t p =
  make_pattern (PEval(x,t,p)) (Id.typ x)

let make_psome p =
  make_pattern (PSome p) (Ty.option p.pat_typ)

let make_palias p x =
  make_pattern (PAlias(p,x)) p.pat_typ

let make_label_aux info t = set_desc t (Label(info,t))
...
let make_construct ?(poly=false) c ts typ = make (Constr(poly,c,ts)) typ
...
let is_length_var (x:lid) =
  match x with
  | LId x -> Id.is_primitive x && Id.name x = "List.length"
  | _ -> false
...
let make_length t =
  make_app (make_var @@ make_length_var t.typ) [t]
  |> add_attrs safe_attr
  |> set_afv_shallowly
...
let make_module ?typ ?env decls =
  ...
  let typ =
    match typ with
    | None ->
        let sig_of t = sig_of_mod_typ ~env:(Option.default [] env) t.typ in
        let sig_values =
          decls
          |> List.flatten_map (function
              | Decl_let defs -> List.map fst defs
              | Include t -> (sig_of t).sig_values
              | _ -> [])
        in
        let sig_types =
          decls
          |> List.flatten_map (function
              ...
              | Include t -> (sig_of t).sig_types
              | _ -> [])
        in
        let sig_ext =
          decls
          |> List.flatten_map (function
              | Type_ext (Ext_type(s, (params, row))) -> [s, (params, row)]
              ...
              | Type_ext (Ext_rebind(s1, _s2)) ->
                  ...
                  [C.exn, ([], row)] (* TODO: fix *)
              | Include t -> (sig_of t).sig_ext
              | _ -> [])
        in
        let not_unique () = failwith "Names must be unique in a structure/signature" in
        if not (List.is_unique ~eq:(Compare.eq_on Id.name) @@ List.filter is_mod_var sig_values) then
          not_unique ();
        if not (List.is_unique ~eq:(Compare.eq_on (fst |- Id.name)) @@ List.flatten sig_types) then
          not_unique ();
        TModSig {sig_values; sig_types; sig_ext}
    | Some ty -> ty
  in
  make (Module decls) typ

let make_pack t ty =
  make (Pack t) (TPackage ty)
...
let rec decomp_or t =
  match t.desc with
  | BinOp(Or, t1, t2) -> decomp_or t1 @@@ decomp_or t2
  | _ -> [t]
...
let rename,rename_pat =
  let tr = Tr2.make () in
  tr.var <- (fun map x ->
    match Id.List.assoc_opt x map with
    | None -> x
...
let get_tconstr,get_tconstr_typ =
  ...
  col.typ <- (fun ty ->
    match ty with
    | TConstr(c,tys) when not (Type_util.is_prim_constr c) -> (c,tys) :: List.rev_map_flatten col.typ tys
    | _ -> col.typ_rec ty);
  col.term, col.typ
...
let trans_decls_as_term f decls =
  let decls',eod =
    decls
    |> make_locals -$- end_of_definitions
    |> f
    |> decomp_decls
  in
...
let subst_term (b_ty,x,t,fv_t as arg) t' =
  let t' =
    if b_ty then
      make_raw ~attr:t'.attr t'.desc (subst.typ arg t'.typ)
    else
      t'
  in
  if not b_ty && not (Id.List.mem x @@ get_fv t') then
    t'
  else
    let rename map t = List.fold_left (fun t (y,y') -> subst.term_rec (b_ty, y, make_var y', [y']) t) t map in
    match t'.desc with
    ...
    | Var lid ->
        ...
        if map' = [] then
          ...
          make_var_lid ~typ:t'.typ (Lid.subst_head map' lid)
    ...
    | Match(t1,pats) ->
        ...
        make desc t'.typ
    | Module decls -> (* TODO: should be capture-avoiding *)
        let rec aux decls =
          match decls with
          | [] -> []
  ...
  tr.term <- (fun map t ->
    ...
    | map ->
        let t' =
          match t.desc with
          | Var (LId y) ->
              begin match Id.List.assoc_opt y map with
              | None -> t
              | Some (t,_) -> t
              end
          | Var lid ->
...
let subst_var_map_without_typ : (id * lid) list -> term -> term =
  let tr = Tr2.make () in
  let assoc map x =
    match Id.List.assoc x map with
  ...
  tr.desc <- (fun map desc ->
    ...
    | Fun(x, t) ->
        ...
        if map = [] then
          ...
          Fun(x, tr.term map t)
    | Local(Decl_let bindings, t2) ->
        let map = List.filter_out (fst |- Id.List.mem_assoc -$- bindings) map in
        if map = [] then
          desc
        else
    ...
    | Match(t1,pats) ->
        let pats' =
          ...
          |> List.map (fun (p,t1) ->
               ...
               let map = List.filter_out (fst |- Id.List.mem -$- bv) map in
               if map = [] then
                 p, t1
               else
                 tr.pat map p, tr.term map t1)
        in
        Match(tr.term map t1, pats')
    | Module decls -> Module (trans_decls_as_term (tr.term map) decls)
    | _ -> tr.desc_rec map desc);
  tr.attr <- (fun map ->
    List.map (function AFV xs -> AFV (List.flatten_map (assoc map |- Lid.heads) xs) | a -> a));
  tr.typ <- (fun map ->
    let map =
      map
      |> List.filter (fst |- is_mod_var)
      |> Fmap.(list (constr_of_id * path_of_lid))
    in
    subst_path_head_map map);
  tr.pat <- (fun map p -> set_afv_abv_pat @@ tr.pat_rec map p);
  tr.term
...
let make_match ?typ t1 pats =
  ...
  | [{pat_desc = PVar x}, t2] ->
      (match t1.desc with Var (LId y) -> if Id.(x = y) then [%unsupported] | _ -> ()); (* TODO *)
      make_let [x, t1] t2
  | [{pat_desc = PTuple ps}, t] when Is._Tuple t1 && List.for_all Is._PVar ps ->
      ...
      let xs',t' =
        if must_rename = [] then
          ...
          let map' = List.map (Pair.map_snd _LId) map in
...
let make_function ~partial ~loc ?ty_arg ~ty_ret pats =
  ...
  | _ ->
      let pats' =
        if partial then
          pats @ [make_pany ty, make_fail ~loc ty_ret]
        else
          pats
      in
...
let make_empty ty =
  make (Const Empty) (make_tset ty) ~attr:const_attr

let make_mem t1 t2 =
  make (MemSet(t1, t2)) Ty.bool ~attr:(make_attr [t1;t2])

let make_addset t1 t2 =
  make (AddSet(t1, t2)) t2.typ ~attr:(make_attr [t1;t2])

let make_subset t1 t2 =
  make (Subset(t1, t2)) Ty.bool ~attr:(make_attr [t1;t2])
...
and same_desc t1 t2 =
  ...
  | Constr(b1, c1, ts1), Constr(b2, c2, ts2) -> b1 = b2 && c1 = c2 && List.for_all2 (===) ts1 ts2
...
let var_name_of_term t =
  let rec name_of t =
    ...
    | Tuple([]) -> "nil"
    | Tuple(ts) ->
        if List.length ts <= 3 then
          String.join "__" (List.map name_of ts)
        else
          String.join "__" (List.map name_of (List.take 2 ts) @ ["etc"])
    | Proj(i,t) ->
        let n = tuple_num t.typ in
        let names = String.split_on_string ~by:"__" (name_of t) in
        if n = Some (List.length names) && List.nth names i <> "" && (n = Some 4 => (i <> 3))
        then List.nth names i
        else name_of t ^ "_" ^ string_of_int i
    | App({desc=Var f},_) -> "r" ^ "_" ^ Lid.name f
...
let rec var_name_of_pattern_short p =
  ...
  | PTuple [] -> "u"
  | PTuple ps ->
      String.join "" @@ List.map var_name_of_pattern_short @@ (List.take 3 ps)
  | PNil -> "e"
  | _ -> var_name_of p.pat_typ

let new_var_of_pattern p =
  let name = var_name_of_pattern p in
  let name = if String.length name < Flag.long_name_len then name else var_name_of_pattern_short p in
...
let rec merge_typ typ1 typ2 =
  ...
  | TVar(_,{contents=Some typ1},_), typ2
  | typ1, TVar(_,{contents=Some typ2},_) -> merge_typ typ1 typ2
...
and make_use_lid x t =
  ...
  if not (has_tfun ty) then
    ...
    | TFun(y,ty) ->
        ...
        make_let_s [r, make_if (make_rand_unit Ty.bool) (make_rand_unit ty) (make_app (make_var_lid x) [make_rand_unit (Id.typ y)])]
        @@ make_use_var r t
    | TFuns _ -> unsupported "%s" __FUNCTION__
    | TTuple xs ->
        let defs = List.mapi (fun i x' -> x', make_proj i (make_var_lid x)) xs in
        make_lets_s defs @@ List.fold_right make_use_var xs t
    | TVariant(_,rows) ->
        if List.exists (row_args |- List.exists Type.Is._TFun) rows then
          unsupported "%s TVariant" __FUNCTION__
        else
          t
    | TRecord _fields -> unsupported "%s TRecord" __FUNCTION__
    | TConstr(_, tys) when List.exists has_tfun tys -> [%unsupported] (* TODO: use type environment *)
    | TConstr _ -> t (* TODO: use type environment *)
...
and make_rand_unit ?(use=true) typ =
  ...
  | TAttr(_,ty) -> mk ty
  | TModSig sgn ->
      let ty_decls = List.map (fun decl -> Decl_type decl) sgn.sig_types in
      let val_decls =
        sgn.sig_values
        |> List.map (Pair.add_right (mk -| Id.typ))
        |> List.map (fun (x,t) -> Decl_let [x,t])
      in
      let typ_exts =
        sgn.sig_ext
        |> List.map (fun (s,ext) -> Type_ext(Ext_type(s,ext)))
      in
      make_module ~typ (ty_decls@typ_exts@val_decls)
...
let make_rand_unit ?(expand=true) ?(use=true) ty =
  if ty = Ty.unit then
    ...
    make_rand_unit ~use ty
  else
    make_app (make_rand ty) [unit_term]

let make_rand_cps typ =
  make
    (Const(Rand(typ,true)))
    [%ty: unit -> ('typ -> result) -> result]
    ~attr:const_attr

let make_randint_cps b =
  make_rand_cps Ty.int
  |&b&> add_attr AAbst_under
...
let is_rand_unit ?(deep=false) t =
  match t.desc with
  | App({desc=Const(Rand(_,false))}, [{desc=Const Unit} | {desc=Var _; typ=TBase TUnit}]) -> true
  | _ ->
      if deep then
        same_term t (make_rand_unit ~expand:true t.typ)
      else
        false
...
let has_no_effect =
  ...
  col.term <- (fun t ->
    ...
    | Var _ -> true
    | Fun _ -> true
    | App(t1,ts) when List.mem TAPureFun (fst (decomp_tattr t1.typ)) -> col.term t1 && List.for_all col.term ts
    | App _ -> false
...
let is_bottom_def f t =
  ...
  | _::_, App({desc=Var g},ts) ->
      Lid.(LId f = g) && List.for_all has_no_effect ts
  | _ -> false

let is_seq_desc desc =
  match desc with
  | Local(Decl_let [u,_], t2) ->
      Id.typ u = TBase TUnit &&
      not @@ Id.List.mem u @@ get_fv t2
  | _ -> false
...
let int_of_term t =
  match t.desc with
  | Const (Int n) -> n
  | _ -> invalid_arg "int_of_term (%a)" Print.term t
...
let subst_tconstr_map,subst_tconstr_map_typ =
  ...
  tr.desc <- (fun (map : (tconstr * _) list) desc ->
    match desc with
    ...
    | Module decls ->
        let _,decls =
          List.L.fold_left_map decls ~init:map
            ~f:(fun map' decl ->
              match map', decl with
              | [], _ -> map', decl
  ...
  tr.typ <- (fun map ty ->
    ...
    | TConstr(LId s,tys) when Id.List.mem_assoc s map ->
        ...
        subst_tconstr_aux tys' (Id.List.assoc s map)
    | _ -> tr.typ_rec map ty);
  tr.term, tr.typ
...
let subst_tconstr_label_map,subst_tconstr_label_map_typ =
  ...
  (fun map -> tr.term (tr_map map)),
  (fun map -> tr.typ (tr_map map))
...
let subst_constr_map =
  let tr = Tr2.make () in
  let has_common map decls = List.exists (Path._LId |- List.mem_assoc -$- map) @@ constrs_in_declaration decls in
  ...
  tr.desc <- (fun map desc ->
    ...
    | Local(Decl_let [m,t1], _t2) when is_module t1 ->
        let constrs = constrs_in_module ~add_prefix:true ~env:[] m in (* TODO: Check [] is ok *)
        if List.exists (List.mem_assoc -$- map) constrs then
          unsupported "subst_constr_map"
        else
          tr.desc_rec map desc
    | Module decls ->
        let _,decls_rev =
...
let subst_field_map =
  ...
  in
  tr.desc <- (fun (map:(label*label)list) desc ->
    match desc with
    | Field(t, s) ->
...
let subst_tconstr_map_decls (map:(tconstr*_)list) decls =
  let aux ((map:(tconstr*_)list),acc_rev) decl =
    if map = [] then
      ...
      match decl with
      | Decl_let defs ->
          let defs' = List.map (Pair.map (Id.map_typ (subst_tconstr_map_typ map)) (subst_tconstr_map map)) defs in
...
let subst_constr_map_decls map decls =
  let aux (map,acc_rev) decl =
    if map = [] then
      ...
      | Decl_type decls ->
          let constrs = constrs_in_declaration decls in
          let map' = List.filter_out (fun (c,_) -> List.exists (fun c' -> Path.(LId c' = c)) constrs) map in
          map', decl::acc_rev
      | Decl_multi _ ->
          [%unsupported]
      | Type_ext _ ->
...
let subst_tconstr_with_copy =
  let tr = Tr2.make () in
  tr.typ <- (fun (s,(params,ty1)) ty2 ->
    match ty2 with
    | TConstr(LId s',params') when s = s' -> alpha_rename_pred_share (subst_tconstr_aux params' (params,ty1))
...
let rec get_last_definition prefix env def t =
  ...
  | Local(Decl_type decls, t2) ->
      let env' = List.map (fun (c,(params,_)) -> c, (params, TConstr(prefix (Type.LId c), params))) decls @@@ env in
...
let from_fpat_idnt x =
  Id.of_string (Fpat.Idnt.string_of x) typ_unknown
...
let col_type_decl : term -> (Type.constr * (typ list * typ)) list list =
  ...
  fun t ->
    ...
    let declss =
      if not (List.exists (List.mem_assoc (Path.id_of C.exn)) declss) && List.mem_assoc C.exn exts then
        [Path.id_of C.exn, ([],TVariant(VNonPoly,[]))]::declss
      else
        declss
    in
    let declss = List.map (List.map (function (s,(ps,TOpen)) -> s, (ps, TVariant(VNonPoly, [])) | decl -> decl)) declss in
        ...
        if List.exists (Id.List.mem_assoc s) declss then
          ...
          let rows =
            if C.is_exn p then
              ...
              [{row_constr = Id.add_name_prefix "Other_" s;
                row_args = [];
                row_ret = None}]
          in
...
let find_exn_typ t =
  match
    ...
    |> col_type_decl
    |> List.flatten
    |> Id.List.assoc_opt (Path.id_of C.exn)
  with Some([],ty) -> Some ty
     | Some _ -> assert false
...
let trans_if =
  let tr = Tr2.make () in
  tr.term <- (fun trans t2 ->
    match trans t2 with
    | None -> tr.term_rec trans t2
    | Some t2' -> t2');
  tr.term

let trans_if_desc =
  let tr = Tr2.make () in
  tr.desc <- (fun trans desc ->
    match trans desc with
    | None -> tr.desc_rec trans desc
    | Some desc' -> desc');
  tr.term
...
let effect_of t =
  try
    List.find_map (function AEffect e -> Some e | _ -> None) t.attr
  with Not_found ->
...
let has_type_decl =
  ...
  it.decl <- (fun decl ->
    match decl with
    | Decl_type _ | Type_ext _ -> raise Found
    | _ -> it.decl_rec decl);
  Exception.does_raise ~exn:Found it.term
...
let get_pred_share ty =
  ...
  let aux n =
    ...
    | Some paths2 ->
        ...
        if List.Set.(paths2' <= paths1') then
...
let rec split_type_decl_and_body ?(force=false) t =
  ...
  | Local(Decl_type decl, t') ->
      ...
      if not force && has_type_decl body then
        invalid_arg "Trans.split_type_decl_and_body";
      decl::decls, body
  | _ -> [], t
...
let catch_all ty_exn t =
  ...
  let handler_body =
    if !Flag.Method.target_exn = [] then
      ...
      match ty_exn with
      | TVariant(b,rows) -> (* TODO: VPoly *)
          let make row =
            let pat_desc = PConstr(b<>VNonPoly, Type.LId row.row_constr, List.map make_pany row.row_args) in
...
let col_raise =
  ...
  let col_desc desc =
    ...
    | TryWith(t1,{desc=Fun(e,{desc=Match({desc=Var e'},pats)})})
         when Lid.(LId e = e') && (function ({pat_desc=PAny},{desc=Raise {desc=Var e''}}) -> Lid.(LId e = e'') | _ -> false) @@ List.last pats ->
...
let has_raise ?(target=[]) t =
  let check =
    if target = [] then
      ...
      Option.map_default (fun s -> List.exists ((=) (string_of_path s)) target) true
  in
  col_raise t
  |> List.exists check
...
let has_module =
  ...
  it.desc <- (fun desc ->
    match desc with
    | Module _ | Pack _ | Unpack _ -> raise Found
    | _ -> it.desc_rec desc);
  Exception.does_raise ~exn:Found it.term
...
let col_exn_constr : term -> (path * [`Used of typ list | `Rebind]) list =
  ...
  in
  let col = Col2.make [] (++) in
  col.term <- (fun (prefix,penv as env) t ->
    ...
    | Local(Decl_let [m, t1], t2), _ when is_module t1 ->
        ...
        in
        col.term (Some prefix', penv) t1 ++
          col.term (prefix, (id,prefix)::penv) t2
    | Local(Decl_let defs, _), _ when List.exists (snd |- is_module) defs -> unsupported "%s" __FUNCTION__
...
let col_poly_constr =
  ...
  col.typ <- (fun ty ->
    ...
    | TVariant(VPoly _, rows) ->
        List.rev_flatten_map (fun {row_constr;row_args} -> row_constr :: List.rev_flatten_map col.typ row_args) rows
    | _ -> col.typ_rec ty);
  col.desc <- (fun desc -> (* Needed for externally defined polymorphic types *)
    let cs = col.desc_rec desc in
    match desc with
    | Constr(true, LId c, _) -> c::cs
    | _ -> cs);
  col.pat <- (fun p ->
    let cs = col.pat_rec p in
    match p.pat_desc with
    | PConstr(true, LId c, _) -> c::cs
    | _ -> cs);
...
let col_exn_decl : term -> (Type.path * [ `Decl of typ list | `Rebind of Type.path ]) list =
  ...
  col.term <- (fun add_prefix t ->
    ...
    | Local(Decl_let defs, _) when List.exists (snd |- is_module) defs -> unsupported "%s" __FUNCTION__
...
let rec add_to_env decl env =
  match decl with
  | Decl_let bindings -> List.fold_left (fun env (f,t) -> Tenv.add_module f t.typ ~env) env bindings
  | Decl_type decl -> Tenv.add_typ_decl decl env
  | Type_ext ext -> Tenv.add_ext ext env
  | Include {typ} ->
      let sgn = sig_of_mod_typ ~env:env.menv typ in
      if sgn.sig_ext <> [] then [%unsupported];
      let defs = List.map (Pair.add_right (make_dummy -| Id.typ)) sgn.sig_values in
      let decls = Decl_let defs :: List.map _Decl_type sgn.sig_types in
...
let get_free_types =
  let col = Col2.make [] (@@@) in
  col.desc <- (fun (bv,bt as arg) desc ->
    match desc with
    | Local(decl, _) ->
        let bv' =
          get_bv_decl decl
  ...
  col.typ <- (fun (bv,bt as arg) ty ->
      ...
      | TConstr(path, tys) ->
          ...
          if is_bound path then
            ...
            (path, List.length tys) :: acc
      | TModSig sgn ->
          col.desc arg (List.fold_right make_let_type sgn.sig_types unit_term).desc
...
let rec gen_fresh_name_var ?(postfix="'") bv x =
  ...
  if List.mem ~eq:(Eq.on Id.name) x bv then
    gen_fresh_name_var bv x
  else
    x
...
let has_fail =
  ...
  it.desc <- (fun desc ->
    match desc with
    | Event("fail",_) -> raise Found
    | _ -> it.desc_rec desc);
  Exception.does_raise ~exn:Found it.term

let has_fail_or_raise =
  has_fail ||| has_raise
