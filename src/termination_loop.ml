...
let mapOption (f : 'a -> 'b option) (lst : 'a list) =
  let rec go = function
    | [] -> []
    | x :: xs -> ( match f x with Some y -> y :: go xs | None -> go xs)
  in
  go lst
...
let set_to_coeff exparam t =
  Trans.map_var (fun x -> if Id.List.mem x exparam then Id.add_attr Id.Coefficient x else x) t
...
let save_to_file t =
  ...
  let filename = Temp.S.ext ext in
...
let verify_with holed exparam_sol pred =
  ...
  let coeffs =
    List.filter (List.mem Id.Coefficient -| Id.attr) @@ Term_util.get_top_funs holed.program
  in
...
let inferCoeffs argumentVariables linear_templates constraints =
  ...
  if correspondenceVars = [] then (
    Format.eprintf "Invalid ordered.@.";
    raise Fpat.PolyConstrSolver.Unknown);
...
let makeLexicographicConstraints variables linearTemplates prevLinearTemplates failedSpc =
  ...
  *)
    ...
    Fpat.Formula.subst
      (List.map
         (fun v ->
           ( v,
             Fpat.Term.mk_var
               (Fpat.Idnt.rename_base (fun v -> v ^ "_IN_" ^ string_of_int i ^ "TH_ERRORPATH") v) ))
         variables)
  in
  let nth_constraints n =
    ...
    in
    subst_ith n
      (Fpat.Formula.band [ List.nth failedSpc n; Fpat.Formula.bnot (Fpat.Formula.band (go 0)) ])
  in
  Fpat.Formula.bor
    (Fpat.Util.List.unfold
       (fun i -> if i < lenSpcSequence then Some (nth_constraints i, i + 1) else None)
       0)
...
let rec run predicate_que holed =
  ...
  | Some predicate_info -> (
      ...
      lrf :=
        BRA_util.update_assoc (Id.to_string holed.verified.id, !cycle_counter, predicate_info) !lrf;
      ...
      try
        ...
        in
        if result then (
          Verbose.printf "%s is terminating.@." holed.verified.id.Id.name;
          result)
        else (
          Verbose.printf "%s is possibly non-terminating.@." holed.verified.id.Id.name;
          result)
      with Refine.PostCondition (env, spc, spcWithExparam) ->
        let unwrap_template = function
          | Fpat.Term.App (Fpat.Term.App (_, t), _) -> t
          | _ -> assert false
        in
        let unwrap_template t = unwrap_template (Fpat.Formula.term_of t) in
        let arg_vars =
          List.map
            (fun v -> Fpat.Idnt.make (Id.to_string (extract_id v)))
            (BRA_state.get_argvars holed.state holed.verified)
        in
        let prev_vars =
          List.map
            (fun v -> Fpat.Idnt.make (Id.to_string (extract_id v)))
            (BRA_state.get_prev_statevars holed.state holed.verified)
        in
        ...
        if !Flag.Termination.disjunctive then (
          ...
          let linear_template_prev =
            Fpat.Term.subst (List.combine arg_vars prev_var_terms) linear_template
          in
            ...
            Fpat.Formula.band
              [
                ...
                Fpat.Formula.bnot
                  (Fpat.Formula.band
                     [
                       Fpat.NumAtom.gt Fpat.Type.mk_int linear_template_prev linear_template
                       |> Fpat.Formula.of_atom;
                       Fpat.NumAtom.geq Fpat.Type.mk_int linear_template (Fpat.IntTerm.make 0)
                       |> Fpat.Formula.of_atom;
                     ]);
              ]
          in
          ...
          let newPredicateInfo _ =
            try
              ...
              {
                predicate_info with
                coefficients = coefficientInfos @ predicate_info.coefficients;
                errorPaths = spc :: predicate_info.errorPaths;
              }
          ...
          let allSpcSequencesWithExparam =
            ...
            in
            go [] predicate_info.errorPathsWithExparam
          in
          let numberOfSpcSequences = List.length allSpcSequences in
          let allVars = List.map fst env in

          let successes =
            (flip List.map) (List.combine allSpcSequences allSpcSequencesWithExparam)
              (fun (spcSequence, spcSequenceWithExparam) _ ->
                (* make templates (for arguments and previous arguments) *)
                let linearTemplates =
                  Fpat.Util.List.unfold
                    (fun i ->
                      if i < numberOfSpcSequences then
                        Some (unwrap_template (Fpat.Template.mk_atom arg_env 1), i + 1)
                      else None)
                    0
                in
                let prevLinearTemplates =
                  List.map (Fpat.Term.subst (List.combine arg_vars prev_var_terms)) linearTemplates
                in
                Fpat.Util.List.iteri
                  (fun i lt -> Verbose.printf "Linear template(%d):@.  %a@." i Fpat.Term.pr lt)
                  linearTemplates;
                try
                  (* make a constraint *)
                  let constraints =
                    makeLexicographicConstraints allVars linearTemplates prevLinearTemplates
                      spcSequence
                  in
                  Verbose.printf "Constraint:@.  %a@." Fpat.Formula.pr constraints;
                  ...
                  let newPredicateInfo =
                    {
                      predicate_info with
                      coefficients = coefficientInfos;
                      errorPaths = spcSequence;
                      errorPathsWithExparam = spcSequenceWithExparam;
                    }
                  in
                  Verbose.printf "Found ranking function: %a@." pr_ranking_function newPredicateInfo;
                  newPredicateInfo
                with Fpat.PolyConstrSolver.NoSolution | Fpat.PolyConstrSolver.Unknown -> (
                  ...
                  try
                    ...
                    (* solve constraint and obtain coefficients *)
                    let coefficientInfos, exparamInfo =
                    ...
                    let newPredicateInfo =
                      {
                        predicate_info with
                        coefficients = coefficientInfos;
                        coeffsMap = exparamInfo;
                        errorPaths = spcSequence;
                        errorPathsWithExparam = spcSequenceWithExparam;
                      }
                    in
                    Verbose.printf "Found ranking function: %a@." pr_ranking_function
                      newPredicateInfo;
                    newPredicateInfo
                  with
