...
let push_not_into (f : term) : term =
  ...
  let rec aux flip (f : term) =
    ...
    | _ -> if flip then Term.not f else f
  in
  aux false f

let rec convert2cnf (f : term) : cnf =
  let make_or (cnf1 : cnf) (cnf2 : cnf) : cnf =
    Combination.product cnf1 cnf2 |> List.map (fun (d1, d2) -> d1 @ d2)
  in
  match f.desc with
  | BinOp (And, _, _) ->
...
let chc_of_cnf (cnf : cnf) : chc list =
  let conv fs =
    let pos, neg = List.partition (function { desc = App _ } -> true | _ -> false) fs in
    let head =
      match pos with [] -> Term.false_ | [ ({ desc = App _ } as t) ] -> t | _ -> assert false
    in
...
let normalize_chc (cs : chc list) : chc list =
  if
    List.L.exists cs ~f:(function
      | { head = { desc = App (_, ts) }; _ } ->
          List.L.exists ts ~f:(function { desc = Var _ } -> false | _ -> true)
      | _ -> false)
  then assert false;
  cs
...
let seqs_of_conj ts =
  List.L.fold_right ts ~init:Term.true_ ~f:(fun t acc ->
      match t.desc with App _ -> Term.seq t acc | _ -> Term.assume t acc)
...
let unify (f, xs, body1) (g, ys, body2) =
  ...
  let body2' = if Id.List.eq xs ys then body2 else subst_var_map (List.combine ys xs) body2 in
...
let prog_of_chc cs =
  let defs, goals =
    List.L.partition_map cs ~f:(function
      | { head = { desc = App ({ desc = Var (LId f) }, ts) }; body } ->
          ...
          let t = body |> normalize_body |> seqs_of_conj in
  ...
  let defs_fv =
    ...
    Id.List.Set.(fv - bv) |> List.map (Pair.add_right make)
  in
  let goal = goals |> List.map seqs_of_conj |> List.map (add_rand []) |> Term.brs in
  Term.(lets defs_fv (let_ defs (seq (seq goal fail) unit)))

let rec remove_forall t =
  match t.desc with
  | BinOp (And, _, _) -> t |> decomp_and |> List.map remove_forall |> Term.ands
...
let normalize_var_name =
  ...
  let rename s =
    ...
    if s'.[0] = '_' then
      String.sub s' 1 (String.length s' - 1) |@> fun s'' -> assert (Char.is_letter s''.[0])
    else s
  in
