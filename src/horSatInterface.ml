...
let version_aux cmd name =
  ...
  | Some cmd -> (
      ...
      try
        ...
        if Str.string_match (Str.regexp (name ^ " \\([.0-9]+\\)")) s 0 then
          Some (String.sub s (Str.group_beginning 1) (Str.group_end 1 - Str.group_beginning 1))
        else None
...
let string_of_parseresult_apt (prerules, arity_map, tr) =
  ...
  ^ "\n"
  ^ string_of_arity_map arity_map
  ^ HS.string_of_transitions_ATA tr
...
let trans_const = function
  ...
  | c ->
      Format.eprintf "trans_const: %a@." CEGAR_print.term (Const c);
      assert false
...
let rec trans_term br = function
  ...
  | App (Const (Event s), t1) -> HS.PTapp (HS.Name ("event_" ^ s), [ trans_term br t1 ])
  | App (t1, t2) ->
...
let trans_fun_def br { fn = f; args = xs; cond = t1; body = t2 } =
  assert (t1 = Const True);
  (trans_id f, List.map trans_id xs, trans_term br t2)
...
let trans_spec_apt (q, e, qs) =
  ...
  let rec apt_transition_to_string is_top = function
    ...
    | APT_And ts ->
        let s = String.join {|/\|} (List.map (apt_transition_to_string false) ts) in
        if is_top then s else parens s
    | APT_Or ts ->
        let s = String.join {|\/|} (List.map (apt_transition_to_string false) ts) in
        if is_top then s else parens s
  in
...
let verifyFile_aux cmd filename =
  ...
  let result_file = Temp.S.ext "horsat_out" in
  ...
  lb.Lexing.lex_curr_p <-
    { Lexing.pos_fname = result_file; Lexing.pos_lnum = 1; Lexing.pos_cnum = 0; Lexing.pos_bol = 0 };
  (ic, lb)

let verifyFile cmd parser token filename =
  ...
  | HS.Unsatisfied ce ->
      let ce' =
        ...
        | Some use, Some cmd ->
            ...
            in
            let cmd =
              Format.sprintf "%s %s %d %s" cmd filename !Flag.Experiment.HORS_quickcheck.num option
            in
            let r =
              Time.measure_and_add Flag.Log.Time.hors_quickcheck (Unix.CPS.open_process_in cmd)
                IO.input_all
            in
            let ce =
              r
              ...
              |> List.hd
            in
            Flag.Experiment.HORS_quickcheck.(
              cex_length_history := List.length ce :: !cex_length_history);
            ce
      in
      Unsafe (trans_ce ce')
...
let check_nontermination target =
  check_apt_aux
    (Option.get !Flag.ModelCheck.horsat)
    HorSat_parser.output_apt HorSat_lexer.token target
...
let check_safety target =
  check_aux (Option.get !Flag.ModelCheck.horsat) HorSat_parser.output HorSat_lexer.token target
...
let make_apt_spec labels =
  let spec =
    ...
    :: (0, "br_exists", APT_Or [ APT_State (1, 0); APT_State (2, 0) ])
    :: make_label_spec labels
  in
...
let make_spec_nontermination labels = (make_arity_map labels, make_apt_spec labels)
let make_spec_safety = TrecsInterface.make_spec_safety
let is_safe_result = function Safe _ -> true | Unsafe _ -> false | UnsafeAPT _ -> false
