...
type counterexample =
  | CESafety of int list  (** Counterexample of safety problem *)
  | CENonTerm of string Rose_tree.t  (** Counterexample of non-termination problem *)
  | CEFairNonTerm of HORS_syntax.rules  (** Counterexample of fair non-termination problem *)
...
type spec = Safety | NonTermination | FairNonTermination of Fair_termination_type.fairness
...
and safety_result = SSafe of (var * Inter_type.t) list | SUnsafe of int list
and nontermination_result = NTSafe of (var * Inter_type.t) list | NTUnsafe of string Rose_tree.t
...
module type MC_fair_nontermination_interface = sig
  ...
  val make_spec_fair_nontermination :
    string list -> (string * string) list -> spec_fair_nontermination

  val check_fair_nontermination :
    CEGAR_syntax.prog * spec_fair_nontermination -> fair_nontermination_result
end
