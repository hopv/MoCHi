...
let rec print_constr fm = function
  ...
  | Sub (typ1, typ2) ->
      Format.fprintf fm "@[<hov 3>(%a@ <:@ %a)@]" print_template typ1 print_template typ2
...
and print_template fm = function
  ...
  | Base (Some (base, p)) -> Format.fprintf fm "%a[P_%d]" Print_typ.base base p
  | Const (x, t) -> Format.fprintf fm "{%a:int | %a}" Id.print x Print.term t
  | Fun (x, tmp1, tmp2) when is_fun_var x ->
      Format.fprintf fm "(@[<hov 2>%a ->@ %a@])" print_template tmp1 print_template tmp2
  | Fun (x, tmp1, tmp2) ->
      Format.fprintf fm "(@[<hov 2>%a:%a ->@ %a@])" Id.print x print_template tmp1 print_template
        tmp2
...
let _And c1 c2 = if c1 = Exp true_term then c2 else if c2 = Exp true_term then c1 else And (c1, c2)
...
and _PApp typ ts =
  let dbg = 0 = 1 in
  let typ' = inline_PApp typ @@ List.filter (Type_util.is_base_typ -| Syntax.typ) ts in
  ...
  match typ' with PApp (typ'', []) -> typ'' | _ -> typ'

let rec _Imply cs c =
  if c = Exp true_term then Exp true_term
  else
    match cs with
    | [] -> c
    | c1 :: cs' -> if c1 = Exp true_term then _Imply cs' c else Imply (c1, _Imply cs' c)

let _Inter styp tmps = match tmps with [ tmp ] -> tmp | _ -> Inter (styp, tmps)
...
let rec from_ref_type (typ : Ref_type.t) =
  match typ with
  | Var _ -> assert false
  | Base (_, x, t) -> Const (x, t)
  | Fun (x, typ1, Base (base, _, t)) ->
      assert (base = TUnit && t.desc = Const True);
      Fun (x, from_ref_type typ1, Base None)
  | Fun (x, typ1, typ2) -> Fun (x, from_ref_type typ1, from_ref_type typ2)
  | Tuple _ -> assert false
  | Inter (styp, typs) -> _Inter styp @@ List.map from_ref_type typs
  | Union _ -> assert false
  | ExtArg _ -> assert false
  | List _ -> assert false
  | Constr (_, _, _, _) -> assert false
  | Exn _ -> assert false
...
let rec constr_of_typ typ =
  ...
  | PApp (Base (Some (_, p1)), ts) ->
      let ts' = List.filter_out (Syntax.typ |- Type_util.is_fun_typ) ts in
      Debug.printf "      constr_of_typ: ts: %a@." (List.print Print.term) ts;
      Exp (make_app (make_var @@ HC.make_pred_var p1 ts') ts')
  | PApp (Const (_, t), _) -> Exp t
  | Fun _ | PApp (Fun _, _) -> Exp true_term
  | Inter (_, typs) -> _Ands (List.map constr_of_typ typs)
  | _ ->
...
let rec expand_type templates val_env typ =
  let dbg = 0 = 1 in
  let pr f =
    if dbg then Debug.printf ("        ET " ^^ f) else Format.ifprintf Format.std_formatter f
  in
  let et = expand_type templates val_env in
  let r =
    match typ with
    | Singleton _ | Base _ | Const _ -> typ
    | Var x ->
        let val_env_x =
          Id.List.assoc x val_env
          |> CT.val_env_of_value
          |> List.filter_out (fun (x, _) -> Id.List.mem_assoc x val_env)
        in
    ...
    | Arg (typ, ts) -> (
        ...
        | Fun (x, typ1, typ2) -> (
            ...
            | t :: ts' ->
                ...
                |> subst_template x t
                |> et
                |@> pr "@[<hov 2>[%a |-> %a]%a = %a@]@." Id.print x Print.term t print_template
                      (Arg (typ2, ts'))
                      print_template)
        | Inter (styp, typs) -> Inter (styp, List.map (fun typ -> et @@ Arg (typ, ts)) typs)
        | typ' ->
            Format.eprintf "@.typ: %a@." print_template typ;
            Format.eprintf "typ': %a@.@." print_template typ';
            assert false)
...
let rec _PAppSelf typ t =
  ...
  | Inter (styp, typs) -> Inter (styp, List.map (_PAppSelf -$- t) typs)
  | _ ->
...
let rec inline_sub templates val_env typ1 typ2 =
  ...
  let r =
    ...
    | Inter (_, typs1), Inter (_, typs2) ->
        _Ands @@ List.map2 (inline_sub templates val_env) typs1 typs2
    | Singleton t, _ -> constr_of_typ @@ _PAppSelf typ2 t
    | Base None, Base None -> Exp true_term
    | PApp ((Base (Some _) as typ1'), ts1), PApp ((Base (Some _) as typ2'), ts2) ->
    ...
    | Fun (x1, typ11, typ12), Fun (x2, typ21, typ22) ->
        ...
        let app typ =
          if is_fun_var x2 then typ else expand_type [] val_env @@ _PApp typ [ make_var x2 ]
        in
        let c2 = constr_of_typ @@ app typ21 in
        let c3 =
          subst_constr x1 (make_var x2) @@ inline_sub templates val_env (app typ12) (app typ22)
        in
...
let in_comp_tree ct nid = Rose_tree.exists (fun { CT.nid = nid' } -> nid' = nid) ct
let hd_of_inter typ = match typ with Inter (_, typs) -> List.hd typs | _ -> typ
...
let rec copy_template cnt typ =
  ...
  | Base None -> Base None
  | Base (Some (base, _)) -> Base (Some (base, cnt#gen))
  | Const _ -> typ
...
let new_pred base cnt = Base (Some (base, cnt#gen))
...
let base_of_typ ty =
  match Type_util.decomp_base ty with Some b -> b | None -> unsupported "%s" __FUNCTION__
...
let make_assumption templates val_env =
  ...
  let aux x =
    Debug.printf "  [MA] Dom(val_env(%a)): %a@." Id.print x (List.print Id.print)
    @@ List.map fst
    @@ CT.val_env_of_value
    @@ Id.List.assoc x val_env;
    let val_env_x =
    ...
    try
      ...
      make_var x
      |> _PAppSelf @@ Id.List.assoc x templates
...
let subst_template x t typ =
  ...
  if false then
    Debug.printf "ST: [%a |-> %a] %a = %a@." Id.print x Print.term t print_template typ
      print_template r;
  r

let new_template_of_simple cnt ty =
  ty |> Ref_type.of_simple |> from_ref_type |> init_with_pred_var cnt
...
let rec make_template cnt args (Rose_tree.Node ({ CT.var_env; val_env; nlabel }, children)) =
  let dbg = 0 = 0 in
  ...
  | CT.App (f, map) ->
      ...
      let typ =
        let head =
          ...
          let aux' (pargs, k) (x, CT.Closure (_, _, t)) =
            ...
            let tmp1 =
              if Type_util.is_base_typ t.typ then
                ...
                let typs =
                  ...
                  |> List.map snd
                in
                if typs = [] then new_template_of_simple cnt t.typ else _Inter t.typ typs
            in
      ...
      (f, typ) :: templates |@> pr "  TEMPLATE: %a@." print_tmp_env
  | CT.Spawn f ->
      let typ =
        templates
...
let rec make_arg_templates_aux typ =
  ...
  | Inter (_, typs) -> List.flatten_map aux typs
  | _ ->
      Format.eprintf "%a@." print_template typ;
      assert false

let make_arg_templates templates =
  let spawned (x, typ) = match decomp_id x with _, Some _, [] -> Some typ | _ -> None in
  templates |> List.filter_map spawned |> List.flatten_map make_arg_templates_aux

let make_template _env t =
  let is_top (x, _) = match decomp_id x with _, _, [] -> true | _ -> false in
  let cnt = new counter in
  let templates = make_template cnt [] t |> List.filter is_top in
  let templates_unused =
    (*
    env
    |> List.filter_map (fun (f,_) -> if Id.List.mem_assoc f templates then None else Some (f, new_template_of_simple cnt @@ Id.typ f))
 *)
    []
  in
  templates_unused @ templates

let rec generate_constraints templates arg_templates assumption
    (Rose_tree.Node ({ CT.nid; val_env; nlabel }, children)) =
  let dbg = 0 = 0 in
  ...
  let r =
    ...
    | CT.App (f, map) ->
        if dbg then
          Debug.printf "  map: %a@." (List.print @@ Pair.print Id.print CT.print_value) map;
        let asm =
        ...
        in
        let constr1 =
          let aux (env, acc) (_, CT.Closure (_var_env, val_env, t)) =
            let constr =
              if Type_util.is_base_typ t.typ then
                ...
                let typ2 =
                  Arg (Var f, env)
                  |*> List.fold_right
                        (fun (y, v) typ -> subst_template y (CT.term_of_value v) typ)
                        val_env
                in
            ...
            let env' = env @ [ t ] in
            let acc' = if constr = Exp true_term then acc else _Imply asm constr :: acc in
            (env', acc')
          in
          if dbg then
            Debug.printf "    map: %a@." (List.print @@ Pair.print Id.print CT.print_value) map;
        ...
        let arg_templates' =
          let arg_templates = make_arg_templates_aux @@ Id.List.assoc f templates' in
 ...
 *)
    ...
    | CT.Spawn _ -> constr arg_templates assumption
    | CT.Let _ -> constr arg_templates assumption
    | CT.Branch _ -> constr arg_templates assumption
    | CT.Assume t -> constr arg_templates @@ (t :: assumption)
...
let check_arity hcs =
  ...
  let aux env t =
    ...
    | App ({ desc = Var (LId f) }, ts) -> (
        ...
        try
          if Id.List.assoc f env <> List.length ts then (
            Format.eprintf "%a@." Id.print f;
            assert false);
          env
...
let rec pvars t =
  match t.desc with
  | App ({ desc = Var (LId p) }, _) -> [ p ]
  | App ({ desc = Var _ }, _) -> assert false
...
let solve hcs =
  ...
  in
  let sol =
    let tr t = t |> FpatInterface.of_term |> Fpat.Formula.of_term in
...
let solve_option hcs =
  try Some (solve hcs) with Fpat.HCCSSolver.Unknown | Fpat.HCCSSolver.NoSolution -> None

let any_var = Id.make ~id:(-1) "any" Ty.bool

let rec apply_sol mode sol x vars tmp =
  let dbg = 0 = 0 in
  let r =
    match tmp with
    | Base (Some _) | PApp (Base (Some _), _) ->
        if x = None then Ref_type.Base (TUnit, Id.new_var Ty.unit, true_term)
        else
          let base, p, ts =
            match tmp with
            | Base (Some (base, p)) -> (base, p, [])
            | PApp (Base (Some (base, p)), _ts) -> (base, p, [] (*ts*))
            | _ -> assert false
          in
    ...
    | Fun (y, typ1, typ2) ->
        Ref_type.Fun
          (y, apply_sol mode sol (Some y) vars typ1, apply_sol mode sol None (y :: vars) typ2)
    | Inter (styp, []) -> Ref_type.inter styp []
        ...
        let bindings, ts' =
          let aux t =
            ...
            | Fun _ ->
                ...
                ([ (x, t) ], make_var x)
            | Local (Decl_let bindings, t') -> (bindings, t')
            | _ -> ([], t)
          in
          ts |> List.map aux |> List.split
        in
...
let normalize t =
  t |> Trans.eta_normal |> lift_arg |> Trans.reconstruct |*> Trans.alpha_rename ~whole:true

let trans_CPS env funs t =
  let t, make_get_rtyp_cps =
    t
    |> List.fold_right (fun (f, t1) t -> add_attr ADoNotInline @@ make_let [ (f, t1) ] t) env
    |@> Debug.printf "trans_CPS1: %a@." Print.term_typ
    |> CPS.trans_as_direct
  in
  Debug.printf "trans_CPS2: %a@." Print.term t;
  Type_check.check t;
  let t, make_get_rtyp_pair = Curry.remove_pair_term t in
  Debug.printf "trans_CPS3: %a@." Print.term t;
  let env', t_main =
    t
  ...
  let env1, env2 = env' |> List.flatten |> List.partition (fst |- Id.List.mem_assoc -$- env) in
  ...
  if not @@ List.for_all (Id.List.mem_assoc -$- env1) funs then (
    let removed = List.filter_out (Id.List.mem_assoc -$- env1) funs in
    Format.eprintf "REMOVED: %a@." (List.print Id.print) removed;
    assert false);
  (env1, make_lets env2 t_main, make_get_rtyp)

let add_context _for_infer prog f typ =
  let dbg = 0 = 0 in
  ...
  let t' =
    ...
    let make_fail typ =
      let t = make_construct (Type.LId af) [] etyp in
      [%term raise `t] |> Problem.safety |> Encode.all |> Problem.term
    in
  ...
  let funs = env |> List.map fst |> List.takewhile (not -| Id.same f) in
  trans_CPS env funs t'

module Dependency = Set.Make (struct
  type t = int * int
...
let add_dependency deps (x, y) =
  ...
  let new_dep =
    Dependency.of_list @@ List.map Pair.of_list @@ Combination.take_each [ to_x; from_y ]
  in
...
let get_dependencies hcs =
  ...
  let aux acc { HC.body; HC.head } =
    ...
    if dbg then
      Debug.printf "  deps_cls: %a@."
        (List.print @@ Pair.print Format.pp_print_int Format.pp_print_int)
      @@ Dependency.elements acc;
...
let transitive_closure deps = List.fold_left add_dependency Dependency.empty deps
...
let save_dep deps_cls hcs filename =
  ...
  let aux acc { HC.body; HC.head } =
    ...
    let fv2 = Option.to_list @@ HC.get_pred_id_of_term head in
    let new_dep = List.flatten_map (fun x -> List.map (Pair.pair x) fv2) fv1 in
    let aux' acc (_x, _y) = new_dep @@@ acc in
    List.fold_left aux' acc new_dep
  in
  let deps = List.unique @@ List.fold_left aux [] hcs in
  let deps_cls' = transitive_closure deps in
  Format.fprintf !Flag.Print.target "Save %s@." filename;
  Graph_wrapper.(save_as_dot filename @@ from_edges deps);
  if dbg then
    Debug.printf "deps_cls: %a@." (List.print @@ Pair.print Format.pp_print_int Format.pp_print_int)
    @@ Dependency.elements deps_cls;
  if dbg then
    Debug.printf "deps_cls': %a@." (List.print @@ Pair.print Format.pp_print_int Format.pp_print_int)
    @@ Dependency.elements deps_cls';
  if dbg then
    Debug.printf "deps_cls'\\deps_cls: %a@."
      (List.print @@ Pair.print Format.pp_print_int Format.pp_print_int)
    @@ Dependency.elements
    @@ Dependency.diff deps_cls' deps_cls;
...
let subst_horn_clause x t (body, head) = (List.map (subst x t) body, subst x t head)

let replace_id p1 p2 t =
  match t.desc with
  | App ({ desc = Var (LId x) }, ts) when HC.get_pred_id x = p1 ->
...
let add_merged (p1, p2) merged =
  if List.mem_assoc p1 merged then (p2, List.assoc p1 merged) :: merged
  else if List.mem_assoc p2 merged then (p1, List.assoc p2 merged) :: merged
  else (p1, p2) :: merged

let can_merge (p1, p2) deps =
  (not @@ Dependency.mem (p1, p2) deps) && (not @@ Dependency.mem (p2, p1) deps)
...
let solve_merged _map hcs =
  ...
  let rec loop used last_sol merged deps map hcs =
    ...
    | (p1, p2) :: map' -> (
        ...
        | Some sol ->
            ...
            let map'' = sbst (p1, p2) map' in
            let deps' = add_dependency (add_dependency deps (p1, p2)) (p2, p1) in
            if 0 = 1 && !!Debug.check then
              save_dep deps' hcs' @@ Format.sprintf "tmp/test%d.dot" !cnt;
            Debug.printf "SOLVED@.";
  ...
  in
  if 0 = 1 then
    Debug.printf "init_deps: %a@." (List.print @@ Pair.print Format.pp_print_int Format.pp_print_int)
    @@ Dependency.elements dependencies;
  ...
  | Some sol ->
      ...
      Debug.printf "merged': %a@."
        ...
        merged';
      Debug.printf "Dom(sol'): %a@." (List.print Format.pp_print_int) @@ List.map fst sol';
      let aux (x, y) = try Some (x, List.assoc y sol') with Not_found -> None in
      Some (List.filter_map aux merged' @ sol')

let assoc_pred_var p hcs =
  if 0 = 1 then Format.fprintf !Flag.Print.target "APV: P_%d@." p;
  let rec find hcs =
    match hcs with
    | [] -> HC.make_pred_var p []
    | (body, head) :: hcs' -> (
        match
          List.find_opt (fun x -> HC.is_pred_var x && Id.id x = p)
          @@ get_fv
          @@ make_ands (head :: body)
        with
        | None -> find hcs'
...
let rec get_merge_candidates_aux typ1 typ2 =
  ...
  | Fun (_, typ11, typ12), Fun (_, typ21, typ22) ->
      get_merge_candidates_aux typ11 typ21 @ get_merge_candidates_aux typ12 typ22
  ...
  let rec aux map t =
    ...
    | Proj (_, { desc = Var _ }) ->
        if List.exists (snd |- same_term t) map then
          (map, Term.var @@ fst @@ List.find (snd |- same_term t) map)
        else
...
let widen t =
  if get_fv t = [] then t
    ...
    in
    map_with_replacing_tuples aux t |@> Debug.printf "widen OUTPUT: %a@." Print.term

let rec instantiate_any mode pos (typ : Ref_type.t) : Ref_type.t =
  match typ with
  | Var _ -> [%unsupported]
  | Base (base, y, p) ->
      let p' =
      ...
      in
      Base (base, y, p')
  | Constr (_, _, _, _) -> unsupported "instantiate_any App"
  | Fun (y, typ1, typ2) ->
      Fun (y, instantiate_any mode (not pos) typ1, instantiate_any mode pos typ2)
  ...
  | ExtArg _ -> unsupported "instantiate_any ExtArg"
  | List _ -> unsupported "instantiate_any List"
  | Exn (typ1, typ2) ->
      let mode' = if mode = ToNatural then ToStronger else mode in
      Exn (instantiate_any mode pos typ1, instantiate_any mode' pos typ2)
  | Variant _ -> unsupported "instantiate_any Variant"
  | Record _ -> unsupported "instantiate_any Record"
...
let infer prog f typ (ce_set : ce_set) depth merge =
  let ce_set =
    if 0 = 1 then
      List.filter
        (fun (x, ce) ->
          Format.fprintf !Flag.Print.target "%a, %a@.?: @?" Id.print x print_ce ce;
          read_int () <> 0)
        ce_set
    else ce_set
  in
 ...
 *)
  ...
  let need =
    List.flatten_map
      (fun (f, tmp) -> if Id.List.mem_assoc f fun_env' then get_pred_ids tmp else [])
      templates
  in
  ...
  let merge_candidates =
    if merge then templates |> List.filter_out (Id.is_external -| fst) |> get_merge_candidates
    else []
  in
  match solve_merged merge_candidates hcs with
  | None -> fun _ -> None
  | Some sol ->
      Debug.printf "TEMPLATES of TOP_FUNS: @[%a@.@." print_tmp_env
      @@ List.filter (fun (f, _) -> Id.List.mem_assoc f fun_env') templates;
      Debug.printf "  Dom(sol): %a@." (List.print Format.pp_print_int) @@ List.map fst sol;
      let top_funs =
        prog.fun_def_env
        |> List.map fst
        |> List.filter (Id.List.mem_assoc -$- fun_env')
      ...
      fun mode ->
        let env' =
          let aux (g, tmp) =
            if Id.List.mem g top_funs then Some (g, apply_sol mode sol tmp) else None
          in
        ...
        let env'' =
          ...
          |> List.flatten_map (fun (x, typ) ->
                 List.map (fun typ -> (x, typ)) @@ Ref_type.decomp_inter typ)
        ...
        let env'' =
          if true then env''
            ...
            let env'' =
              List.flatten_map
                (fun (x, typ) -> List.map (fun typ' -> (x, typ')) @@ Ref_type.split_inter typ)
                env''
            in
        ...
        let env_unused =
          let aux f =
            if Id.List.mem_assoc f env' then None
            else
              match mode with
              | ToTrue | ToNatural -> Some (f, Ref_type.of_simple @@ Id.typ f)
