...
let rec make_template env ty =
  let ty', is_attr =
    ...
    | TApp(s,tys) -> TApp(s, List.map (make_template env) tys), false
...
let rec flatten_sub ?(ignore_top=false) env ty1 ty2 =
  ...
  | TTuple _, _ ->
      ...
      Format.printf "ty2: %a@." Print.typ ty2;
      assert false
  | TData _, _ -> unsupported __LOC__
  | TVariant _, _ -> unsupported __LOC__
  | TRecord _, _ -> unsupported __LOC__
  | TApp("set", [ty1]), TApp("set", [ty2]) ->
      flatten_sub env ty1 ty2
  | TApp _, _ -> unsupported __LOC__
  | TAttr _, _ -> assert false
  | TModule _, _ -> unsupported __LOC__
...
let rec gen_constr env tenv t =
  ...
  | Const (Rand(_, false)) ->
      ...
      t'
  | Const (Rand(_, true)) -> unsupported __LOC__
  ...
  | Event(_,true) -> unsupported __LOC__
  | Event(_,false) ->
      let t' = add_evar env t in
  ...
  | MemSet(t1, t2) ->
      ...
      add_constr (effect_of t1') es env;
      add_constr (effect_of t2') es env;
      set_effect es {desc=MemSet(t1',t2'); typ; attr=t.attr}
  | AddSet(t1, t2) ->
      let t1' = gen_constr env tenv t1 in
      let t2' = gen_constr env tenv t2 in
      let typ = make_template env t.typ in
      ...
      add_constr (effect_of t1') es env;
      add_constr (effect_of t2') es env;
      set_effect es {desc=AddSet(t1',t2'); typ; attr=t.attr}
  | Subset(t1, t2) ->
      let t1' = gen_constr env tenv t1 in
      let t2' = gen_constr env tenv t2 in
      let typ = make_template env t.typ in
      ...
      add_constr (effect_of t1') es env;
      add_constr (effect_of t2') es env;
      set_effect es {desc=Subset(t1',t2'); typ; attr=t.attr}
  | _ ->
