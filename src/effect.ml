...
type env = { mutable constraints : (effect * effect) list; mutable counter : int; for_cps : bool }
...
let add_effect_typ es ty = _TAttr [ TAEffect es ] ty
...
let rec make_template env ty =
  ...
  in
  match ty with TAttr _ -> ty' | _ -> add_effect_typ [ new_evar env ] ty'
...
let rec force_cont ty =
  let attrs, ty1 =
    match ty with
    | TAttr (attrs, ty1) ->
        (List.map (function TAEffect _ -> TAEffect [ EEvent ] | a -> a) attrs, ty1)
    | _ -> assert false
  in
  let ty1' =
    match ty1 with
...
let rec flatten_sub ?(ignore_top = false) env ty1 ty2 =
  if not ignore_top then add_constr (effect_of_typ ty1) (effect_of_typ ty2) env;
  ...
  | TTuple xs1, TTuple xs2 -> List.iter2 (flatten_sub_var env) xs1 xs2
  ...
  | ty1, _ -> unsupported "%s (%a)" __FUNCTION__ Print.typ ty1

and flatten_sub_var ?(ignore_top = false) env x y =
  flatten_sub ~ignore_top env (Id.typ x) (Id.typ y)

let get_tfun_effect ty =
  match elim_tattr ty with TFun (_, ty2) -> effect_of_typ ty2 | _ -> assert false
...
let rec gen_constr env tenv t =
  ...
  let es, t' =
    ...
    | Var (LId x) ->
        let typ =
          try Id.List.assoc x tenv with
          | Not_found when Fpat.RefTypInfer.is_parameter (Id.name x) -> add_effect_typ [] Ty.int
          | Not_found ->
        ...
        let bindings' =
          ...
          |> List.map (fun (f, t1) ->
                 ...
                 let rec aux_div t =
                   match t.desc with
                   | Fun (_, t') -> aux_div t'
                   | _ when Id.List.mem f' (get_fv ~force:true t) ->
                       add_constr [ EDiv ] (effect_of t) env
                   | _ -> ()
                 in
                 aux_div t1';
                 flatten_sub' t1 env t1'.typ f_typ;
                 add_constr (effect_of t1') [ e ] env;
                 (f', t1'))
        in
        let t1' = gen_constr env tenv' t1 in
    ...
    | BinOp (op, t1, t2) -> gen2 t1 t2 (_BinOp op)
    | Not t1 ->
        let t1' = gen_constr env tenv t1 in
        let typ = make_template env t.typ in
        let es = effect_of_typ typ in
    ...
    | _ ->
        ...
        assert false
  in
  t' |> add_attrs t.attr |> Option.map_default set_effect Fun.id es
...
let solve env =
  ...
  let init (nondets, events, divs, exceps) (x, y) =
    ...
    | _ -> unsupported "Effect.infer: external functions with raise-throwable function arguments?"
  in
...
let apply_sol =
  ...
  let tr_effects sol es =
    es |> List.flatten_map (function EVar x -> sol x | e -> [ e ]) |> List.unique
  in
  let tr_attr sol attrs =
    let aux = function AEffect es -> AEffect (tr_effects sol es) | a -> a in
    List.map aux attrs
  in
  let tr_typ sol ty =
    match ty with
    | TAttr (attrs, ty1) ->
        let attrs' =
          List.map (function TAEffect es -> TAEffect (tr_effects sol es) | a -> a) attrs
        in
...
let infer ?(for_cps = false) t =
  ...
  let tenv =
    let make x = x |> Id.typ |> make_template env |> for_cps ^> force_cont in
  ...
  List.iter
    (fun (e1, e2) -> Dbg.printf "  %a <: %a@." Print_typ.effect e1 Print_typ.effect e2)
    env.constraints;
  Dbg.printf "@.";
...
let mark =
  ...
  let mark_id x =
    let ty = Id.typ x in
    let ty' =
      if is_base_typ (Id.typ x) then _TAttr [ TARefPred (Id.new_var_id x, Term.true_) ] ty else ty
    in
    Id.set_typ x ty'
  in
  tr.term <-
    (fun t ->
      ...
      | Const (Dummy _) ->
          ...
          (tr.term_rec (set_desc t desc) [@alert "-unsafe"])
      | _ -> tr.term_rec t);
  tr.typ <-
    (fun ty ->
      match ty with
      ...
      | TTuple xs ->
          ...
          let xs'' =
            let mark x acc =
              let x' = if can_have_pred (TTuple acc) then mark_id x else x in
              x' :: acc
            in
      ...
      | TFun (x, ty2) ->
          ...
          let x' = tr.var x in
          let x'' = if [] = effect_of_typ ty2 && can_have_pred ty2' then mark_id x' else x' in
          TFun (x'', ty2')
      | _ -> tr.typ_rec ty);
  tr.attr <- Fun.id;
  tr.term
