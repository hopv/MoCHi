...
let subst_state states s q b =
  let aux i q' = if q = q' then make_bool b else make_proj i @@ make_var s in
  make_tuple @@ List.mapi aux states

let index_of states q = List.assoc_opt q @@ List.mapi Pair.pair_rev states

let make_is_fair states fairness s =
  let aux p =
    if p = "Always" then true_term
    else match index_of states p with None -> false_term | Some i -> make_proj i @@ make_var s
  in
...
let join x y = match (x, y) with None, _ -> y | _, None -> x | Some _, Some _ -> assert false
...
let trans_typ env typ =
  ...
  | TFuns (xs, typ1) ->
      ...
      ( None,
        TFuns (xs'' @ [ env.s; env.set_flag ] @ env.ps @ [ x ], make_tpair (Id.typ env.s) typ1') )
...
let trans_term env t =
  ...
  | App (t1, [ _ ]) when t1.desc = fail_term.desc ->
      (None, make_fail @@ make_tpair (Id.typ env.s) t.typ)
  | App (v1, vs) ->
      ...
      let rec aux head typ vs =
        ...
        | TFuns (xs, typ') ->
            ...
            let head' =
              make_app head (vs1' @ (List.map make_var @@ (env.s :: env.set_flag :: env.ps)) @ [ v ])
            in
            if vs2 = [] then head' else aux (make_snd head') typ' vs2
        | _ -> assert false
      in
  ...
  | Local (Decl_let [ (x, t1) ], t2) when (not @@ Id.(x = env.target)) && [] = fst @@ decomp_funs t1
    ->
      ...
      ( join vs1 vs2,
        make_lets [ (sx, t1'); (s', make_fst (make_var sx)); (x', make_snd (make_var sx)) ] t2' )
      ...
      let aux (g, t1) =
        ...
        let vs, t1' =
          if Id.(g = env.target) then
            ...
            let t_check =
              make_if (make_is_fair env.states env.fairness s') (make_assert t_rank) unit_term
            in
            let t_b =
              make_if (make_var set_flag') (make_seq t_check randbool_unit_term) true_term
            in
            let t_sp =
            ...
            let t1'' =
              ...
              Debug.printf "t1: %a@." Print.term t1;
              let vs, t1''' =
                trans.term { env with s = s''; ps = ps''; set_flag = set_flag'' } t1
              in
              assert (vs = None);
            ...
            let t1'''' =
              if !Flag.FairTermination.expand_set_flag then
                make_if (make_var set_flag') (subst set_flag' true_term t1''')
                  (Trans.alpha_rename @@ subst set_flag' false_term t1''')
              else t1'''
            in
            (Some (rank_var, ps', xs'), t1'''')
          else trans.term { env with s = s'; set_flag = set_flag' } t1
        in
        ...
        let aux2 (s, set_flag, ps) xs (first, t) =
          ...
          (false, make_funs (xs' @ (s :: set_flag :: ps) @ [ x ]) t')
        in
        if false && Id.(g = env.target) then
          ( vs,
            [
              (g', Trans.alpha_rename @@ snd @@ List.fold_right2 aux1 args xss' (true, t1'));
...
let verify_with rank_var rank_funs prev_vars arg_vars exparam_sol t =
  if !Flag.Termination.add_closure_exparam then
    Verbose.printf "EXPARAM: %a@."
      (List.print @@ Pair.print Print.id Format.pp_print_int)
      exparam_sol;
  ...
  List.for_all
    (fun t ->
      incr Flag.FairTermination.loop_count;
      Main_loop.run ~exparam_sol (Problem.safety t))
    ts

let rec main_loop rank_var rank_funs prev_vars arg_vars exparam_sol spcs spcWithExparams preds_info
    (*need?*) t =
  try
    let result =
      if !Flag.Termination.separate_pred then unsupported "separte_pred"
      else if !Flag.Termination.split_callsite then unsupported "split_callsite"
      else verify_with rank_var rank_funs prev_vars arg_vars exparam_sol t
    in
    result
  with Refine.PostCondition (env, spc, spcWithExparam) ->
    ...
    let solution =
      ...
      Fpat.RankFunInfer.lrf
        !Flag.Termination.add_closure_exparam
        spcs' spcWithExparams' (*all_vars*) arg_vars' prev_vars'
    in
    ...
    let exparam_sol'' =
      List.map (fun (x, n) -> (x, List.assoc_default n x exparam_sol')) exparam_sol
    in
    if !Flag.Termination.add_closure_exparam then
      Debug.printf "SOLUTION: %a@."
        (List.print @@ Pair.print Print.id Format.pp_print_int)
        exparam_sol'';
    ...
    Verbose.printf "DONE!@.@.";
    main_loop rank_var rank_funs'' prev_vars arg_vars exparam_sol'' spcs' spcWithExparams'
      preds_info' t

let pr ?(check_typ = Some Ty.unit) s t =
  if !!Debug.check then (
    Format.fprintf !Flag.Print.target "##[%a] %a:@.%a@.@." Color.s_yellow "Fair_termination"
      Color.s_red s Print.term_typ t;
    Option.iter (fun ty -> Type_check.check t ~ty) check_typ)

let rec run spec t =
  let { Spec_syntax.fairness } = spec in
  ...
  let verify f =
    ...
    if !!Debug.check then
      if result then Format.fprintf !Flag.Print.target "%a is fair terminating.@.@." Id.print f
      else Format.fprintf !Flag.Print.target "%a is possibly non fair terminating.@.@." Id.print f;
    result
  in
  try List.for_all verify top_funs' with
  | Fpat.RankFunInfer.LRFNotFound when !Flag.Termination.add_closure_exparam -> false
  | Fpat.RankFunInfer.LRFNotFound ->
      Flag.Termination.add_closure_exparam := true;
      run spec t
