...
let is_basetype_variable = function { Syntax.typ = Type.TFun (_, _) } -> false | _ -> true

(* remove variables of functional type *)
let remove_functional_vars =
  state_transducer
    (List.filter is_basetype_variable)
    (List.filter is_basetype_variable)
    (List.filter is_basetype_variable)

let filter_non_integer =
  let is_integer = function { Syntax.typ = Type.TBase Type.TInt } -> true | _ -> false in
...
let make_prev_statevar_id function_name baseId =
  Id.new_var ~name:(make_prev_statevar_name function_name baseId) baseId.Id.typ

let make_prev_statevar function_name baseId =
  build_var (make_prev_statevar_name function_name baseId) baseId.Id.typ

let make_statevar_name function_name baseId = "s_" ^ function_name ^ "_" ^ baseId.Id.name

let make_statevar_id function_name baseId =
  Id.new_var ~name:(make_statevar_name function_name baseId) baseId.Id.typ

let make_statevar function_name baseId =
  build_var (make_statevar_name function_name baseId) baseId.Id.typ
...
let build_record { id = { Id.name = f_name }; args = f_args } =
  ...
  let record =
    ref
      {
        update_flag = build_var "update_flag" Ty.bool;
...
let build_state function_infos target =
  {
    initial_state =
      Term_util.false_term
      :: List.map (fun { Syntax.typ = t } -> default_val t) (build_record target).argvars;
    statetable =
      List.fold_left
        (fun map function_info -> InnerState.add function_info.id (build_record function_info) map)
        InnerState.empty function_infos;
  }
...
let passed_statevars { BRA_types.verified; BRA_types.state } f =
  let table = InnerState.find verified.id state.statetable in
  if f = verified.id then table.prev_set_flag :: table.prev_statevars
  else table.set_flag :: table.statevars

let propagated_statevars { BRA_types.verified; BRA_types.state } =
  let table = InnerState.find verified.id state.statetable in
  table.set_flag :: table.statevars

let find_state { BRA_types.state } f = InnerState.find f.id state.statetable

let type_of_state { BRA_types.state = { BRA_types.initial_state = inits } } =
  List.map (fun { Syntax.typ = t } -> t) inits
