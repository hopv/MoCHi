...
let trans_const = function
  ...
  | c ->
      Format.eprintf "print_const: %a@." CEGAR_print.const c;
      assert false
...
let rec trans_term = function
  ...
  | App (Const (Event s), t1) -> TS.PTapp (TS.Name ("event_" ^ s), [ trans_term t1 ])
  | App (t1, t2) ->
...
let rec verifyFile filename =
  ...
  | Some trecs -> (
      ...
      let result_file = Temp.S.ext "trecs_out" in
      let oc = open_out result_file in
      let out_descr = Unix.descr_of_out_channel oc in
      let args =
        String.split_on_string ~by:" " (Format.sprintf "%s -p %d %d %s" trecs p1 p2 filename)
      in
      let pid =
        Unix.create_process (List.hd args) (Array.of_list args) Unix.stdin out_descr Unix.stderr
      in
      let _, _st =
        try Unix.waitpid [ Unix.WUNTRACED ] pid
        with e ->
          Unix.kill pid 14;
          raise e
      in
      ...
      close_in ic;
      match r with
      | TS.Safe env -> Model_check_common.SSafe env
      | TS.Unsafe trace -> SUnsafe (trans_ce trace)
      | TS.TimeOut ->
  ...
  | Some trecs ->
      ...
      let v =
        try
          ...
          if Str.string_match (Str.regexp "TRecS \\([.0-9]+\\)") s 0 then
...
let make_label_spec n q =
  let rec aux i spec =
    if i < 0 then spec else aux (i - 1) ((q, "l" ^ string_of_int i, [ q ]) :: spec)
  in
  aux n []
...
let make_spec_safety n : spec_safety =
  let spec =
    ...
    | Flag.Reachability | Flag.Termination | Flag.FairTermination ->
        (0, "unit", []) :: make_base_spec n 0
    | Flag.FileAccess ->
