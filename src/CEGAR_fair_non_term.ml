...
let rec subst x ex1 ex2 =
  match ex2 with
  | Var s -> if s = x then ex1 else Var s
  | Apply (e1, e2) -> Apply (subst x ex1 e1, subst x ex1 e2)
  | Abst (s, e1) -> Abst (s, subst x ex1 e1)
...
let apply s children = List.fold_left (fun e1 e2 -> Apply (e1, e2)) (Var s) children

(**
   Expands expr n-times and generates a counterexample tree
*)
let rec expand_tree rules n expr =
  let is_term f = not (List.mem_assoc f rules) in

  let get_fun f = List.assoc f rules in
  ...
  if n < -max 30 !Flag.FairNonTermination.expand_ce_iter_init then (
    ...
    match expr with
    | Var s when is_term s -> Var s
    | Var s ->
        let e = get_fun s in
        expand_tree rules (n - 1) e
    | Abst (x, e) -> Abst (x, e)
    | Apply (e1, e2) -> (
        match expand_tree rules (n - 1) e1 with
        | Var s when n < 0 && String.starts_with s "randint_" ->
            (* Stop expansion if it already expands n-times and right before random int generation *)
            Var "end"
        | Abst (x, e) -> expand_tree rules n (subst x e2 e)
        | e1' -> (
...
let not_dummy = function Var "_" -> false | _ -> true
...
let rec value2tree v =
  ...
  | Var s, children -> Rose_tree.Node (s, List.map value2tree children)
  | _ -> assert false
...
let cegar prog0 labeled is_cp ce_rules prog =
  expansion_iter_count_ref :=
    max !Flag.FairNonTermination.expand_ce_iter_init !expansion_iter_count_ref;
