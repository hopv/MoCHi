...
let pp_literal fm lit =
  match lit with
  | FPred(p,ts) -> Ast.pp_term fm (Ast.App(p,ts))
  | Exp t -> Ast.pp_term fm t

let of_ty (ty:Ast.ty) =
  match ty with
  | Ast.Ty_bool -> Ty.bool
  | Ast.Ty_real -> unsupported "%s(%a)" __FUNCTION__ Ast.pp_ty ty
  | Ast.Ty_app ("Int", []) -> Ty.int
  | Ast.Ty_app (_, _) -> unsupported "%s(%a)" __FUNCTION__ Ast.pp_ty ty
  | Ast.Ty_arrow (_, _) -> unsupported "%s(%a)" __FUNCTION__ Ast.pp_ty ty

let of_var x ty =
  Id.make x (of_ty ty)

let of_env env =
  List.map (Fun.uncurry of_var) env
...
let add_binds binds env =
  List.map (Pair.map_snd of_ty) binds @@@ env
...
let rec of_term env (t:Ast.term) : Syntax.term =
  ...
  | Const x ->
      (try
         Term.var (Id.make x (List.assoc x env))
       with Not_found ->
         try
           Term.int (int_of_string x)
  ...
  | App(f, ts) ->
      ...
      [%term f @ `ts]
  | HO_app (_, _) -> unsupported "%s(%a)" __FUNCTION__ Ast.pp_term t
  | Match (_, _) -> unsupported "%s(%a)" __FUNCTION__ Ast.pp_term t
  | If (_, _, _) -> unsupported "%s(%a)" __FUNCTION__ Ast.pp_term t
  | Let (_, _) -> unsupported "%s(%a)" __FUNCTION__ Ast.pp_term t
  ...
  | Attr (_, _) -> unsupported "%s(%a)" __FUNCTION__ Ast.pp_term t
