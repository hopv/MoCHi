...
let rec is_base_term env t =
  match t with
  | Const (Unit | True | False | Int _ | Rand _ | Char _ | String _ | Float _ | Int32 _ | Int64 _ | Nativeint _ | Empty) -> true
  | Const _ -> false
  | Var x ->
      (try
...
let add_ext_funs prog =
  ...
  let defs = List.map (fun (f,typ) -> {fn=f; args=[]; cond=Const True; body=make_ext_fun typ}) env in
