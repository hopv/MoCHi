...
val trans_def :
  Syntax.typ Id.t * (Syntax.typ Id.t list * Syntax.term) -> (string * typ * var list * t * t) list
...
val rename_prog : prog -> prog * (var * var) list
val id_prog : prog -> prog * (var * var) list * (var * Syntax.id) list
val trans_ref_type : CEGAR_ref_type.t -> Ref_type.t
val trans_term : Syntax.term -> (string * typ * var list * t * t) list * t
...
val step_eval_abst_cbn : int list -> var list -> 'a -> fun_def list -> t -> int list * t
val eval_abst_cbn : prog -> var list -> prog -> int list -> unit
val trans_ce : var list -> prog -> int list -> int option -> int list
val simplify_if : prog -> prog
val add_fail_to_end : prog -> prog
val add_env : (Syntax.id * Syntax.typ) list -> prog -> prog
val expand_non_rec : prog -> prog
val beta_reduce_term : t -> t
val bool_to_int : prog -> prog
val lift_for_hfl : prog -> prog
