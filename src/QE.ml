  ...
  | App (App (Var (("exists" | "forall") as q), args), t1) ->
      let args' =
        try
          ...
          | Var "args", ts ->
              List.map (fun t -> (F.Idnt.make @@ Option.get @@ decomp_var t, F.Type.mk_int)) ts
          | _ -> assert false
        with _ ->
      ...
      let f =
        match q with
        | "exists" -> F.Formula.exists
        | "forall" -> F.Formula.forall
        | _ -> assert false
      in
      f args' @@ conv_formula' t1
  | Const c -> F.Formula.of_term @@ F.Term.mk_const (conv_const c)
  | Var x -> F.Formula.of_term @@ F.Term.mk_var @@ conv_var x
  | App (t1, t2) ->
      F.Formula.of_term
      @@ F.Term.mk_app
           (F.Formula.term_of @@ conv_formula' t1)
           [ F.Formula.term_of @@ conv_formula' t2 ]
  | Fun _ -> assert false
  | Let _ -> assert false

let rec used_as_bool x t =
  match decomp_app t with
  | (Const (Or | And), [ Var y; _ ] | Const (Or | And), [ _; Var y ]) when x = y -> true
  | Const Not, [ Var y ] -> x = y
...
let rec eliminate_bool t =
  ...
  | App (App (Var (("exists" | "forall") as q), args), t1) ->
      ...
      if xs2 = [] then t1'' else Term.(var q @ [ var "args" @ vars xs; t1' ])
  | Const c -> Const c
  | Var x -> Var x
...
let eliminate t =
  let of_formula phi =
    let open Fpat in
    let phi = phi |> CunFormula.elim_unit |> Formula.map_atom CunAtom.elim_beq_bneq in
    let tenv, phi = SimTypInfer.infer_formula [] phi in
    F.Z3Interface.of_formula phi tenv []
  in
  let fv = get_fv t in
  let map =
    List.map
      (Pair.add_right @@ String.replace_chars (function '!' -> "_bang_" | c -> String.of_char c))
      fv
  in
