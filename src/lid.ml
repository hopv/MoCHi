...
let id_of lid = match lid with LId x -> x | _ -> invalid_arg "%s" __FUNCTION__
...
let rec head lid =
  match lid with LId x -> x | LDot (lid', _) -> head lid' | LApp _ -> invalid_arg "Longid.head"
...
let rec head lid = match lid with LId id -> id | LDot (p, _) -> head p | LApp _ -> [%invalid_arg]
...
let typ lid =
  ...
  | LDot _ -> (
      try
        let m = head lid in
        let env = Tenv.add_module m (Id.typ m) ~env:!!Tenv.init in
...
let rec subst_head map lid =
  ...
  match lid with
  | LId id -> IdMap.find_default lid id map
