...
let is_ground_typ typ = match typ with TBase TInt -> true | _ -> false
...
let make_check_rank ps xs rank_funs = make_ors @@ List.map (apply_rank_fun ps xs) rank_funs
...
let remove_and_replace_event_desc desc =
  ...
  | Local (Decl_let [ (f, _) ], t') when is_event_fun_var f ->
      (remove_and_replace_event.term t').desc
...
let insert_extra_param_desc vars desc =
  ...
  | Local (Decl_let bindings, t2) ->
      ...
      let vars' =
        List.filter_map (fun (x, _) -> if Id.typ x = Ty.int then Some x else None) bindings @ vars
      in
  ...
  | App (t1, ts) ->
      ...
      let ts'' =
        List.flatten_map
          (fun t -> if is_fun_typ t.typ then [ make_extra_param vars; t ] else [ t ])
          ts'
      in
      App (t1', ts'')
...
let get_states_desc desc =
  ...
  else match desc with Event (q, _) -> [ q ] | _ -> get_states.desc_rec desc
