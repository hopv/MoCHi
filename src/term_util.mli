...
val make_function : partial:bool -> loc:Warnings.loc -> ?ty_arg:typ -> ty_ret:typ -> (pattern * term) list -> term
...
val make_label : ?label:string -> info -> term -> term
...
val split_type_decl_and_body : ?force:bool -> term -> (Type.constr * (typ list * typ)) list list * term
...
val has_fail_or_raise : term -> bool
...
val subst_var_without_typ : id -> lid -> term -> term (* does not change types except when the targets are modules *)
val subst_var_map_without_typ : (id * lid) list -> term -> term (* does not change types except when the targets are modules *)
val subst_constr : Type.path -> Type.path -> term -> term (* does not substitute constructors in types *)
...
val subst_tconstr_map_decls : (Type.tconstr * (typ list * typ)) list -> declaration list -> declaration list
...
val copy_tvar_list : ?env:((typ option ref * typ) list) -> term list -> ((typ option ref * typ) list) * term list
