...
val make_type : (string * typ) list -> term -> term
...
val decomp_module : term -> declaration list
...
val split_type_decl_and_body : term -> (string * typ) list list * term
...
val col_decl_type : term -> (string * typ) list
val col_free_tdata : term -> string list
val col_raise : term -> string option list
...
val subst_tdata_map_typ : (string * typ) list -> typ -> typ
...
val subst_constr : string -> string -> term -> term
val subst_constr_map : (string * string) list -> term -> term
val subst_field : string -> string -> term -> term
val subst_field_map : (string * string) list -> term -> term
val subst_decls : id -> term -> declaration list -> declaration list
val subst_fdecls : id -> term -> (rec_flag * declaration) list -> (rec_flag * declaration) list
...
val subst_tdata_map_decls : (string * typ) list -> declaration list -> declaration list
val subst_field_map_decls : (string * string) list -> declaration list -> declaration list
val subst_constr_map_decls : (string * string) list -> declaration list -> declaration list
...
val can_unify : ?tenv:term Type.declaration -> typ -> typ -> bool
val can_unify_multi : ?tenv:term Type.declaration -> typ list -> bool
val copy_tvar : ?env:((typ option ref * typ) list) -> term -> ((typ option ref * typ) list) * term
val copy_tvar_typ : ?env:((typ option ref * typ) list) -> typ -> ((typ option ref * typ) list) * typ
val catch_all : typ -> term -> term
val make_catch_all : term -> term -> term
val trans_decls_by_term : (term -> term) -> declaration list -> declaration list
...
module Term : sig
  ...
  val (@@) : term -> term list -> term
  val type_ : (string * typ) list -> term -> term
  ...
  val none : typ -> term
  val some : term -> term
  val field : ?ty:typ -> term -> string -> term
  val ref : term -> term
  val (!) : term -> term
  val (:=) : term -> term -> term
  val constr : string -> term list -> typ -> term
