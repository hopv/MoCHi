...
let encode_list_term post t =
  ...
  | App({desc=Const(Rand(TBase TInt,false)); attr}, t2) when List.mem AAbst_under attr -> (* for disproving termination  *)
      assert (t2 = [unit_term]);
      t
  | App({desc=Const(Rand(_,false))}, t2) ->
      assert (t2 = [unit_term]);
...
let rec gen_eq env t1 t2 =
  match elim_tattr t1.typ with
  | TBase _ -> env, Term.(t1 = t2)
  | TTuple _ -> unsupported "Encode_list TTuple"
  | TApp("list", [ty]) ->
      begin
        match List.assoc_opt ty env with
        | Some(f,_) -> env, Term.(var f @ [t1; t2])
        | None ->
            ...
            let xs' = Id.new_var ~name:"xs'" (Ty.list ty) in
            let ys' = Id.new_var ~name:"ys'" (Ty.list ty) in
            let env = (ty,(f,Term.unit))::env in
            let env,t_eq_xy = gen_eq env (Term.var x) (Term.var y) in
            let env,t_eq_xs'ys' = gen_eq env (Term.var xs') (Term.var ys') in
            let t =
              Term.(match_ (pair (var xs) (var ys))
                      [Pat.(pair (nil ty) (nil ty)),
                         true_;
                       Pat.(pair (cons (var x) (var xs')) (cons (var y) (var ys'))),
                         t_eq_xy && t_eq_xs'ys';
                       Pat.__ (Ty.list ty),
                         false_])
            in
            let env = List.modify ty (Fun.const (f, Term.(funs [xs;ys] t))) env in
            env, Term.(var f @ [t1; t2])
      end
  | _ -> assert false
...
let inst_list_eq =
  ...
  let fld_desc env desc =
    ...
    | BinOp(Eq, ({typ=TApp("list",_)} as t1), t2) ->
        let env,t1' = fld.fld_term env t1 in
        let env,t2' = fld.fld_term env t2 in
        let env,t = gen_eq env t1' t2' in
        env, t.desc
    | _ -> fld.fld_desc_rec env desc
  in
  fld.fld_desc <- fld_desc;
  fun t ->
    let env,t' = fld.fld_term [] t in
    let defs =
...
let get_rtyp_list_opt _rtyp _typ = raise (Fatal "not implemented get_rtyp_list_opt")
