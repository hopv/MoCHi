...
let rec is_filled_pattern p =
  try
    ...
    | PAlias (p1, x) -> Some (Option.default (make_var x) @@ is_filled_pattern p1)
    | PConst c -> Some c
    | PConstr (poly, c, ps) ->
    ...
    | PRecord fields ->
        ...
        Some
          (make_record
             (List.map (Pair.map_snd @@ (Option.get -| is_filled_pattern)) fields)
             p.pat_typ)
          ...
          let sbst =
            ...
            | Some t' -> (
                fun t0 ->
                  match t0 with
                  | { desc = Const True } -> t0
                  | _ when !Flag.Method.tupling -> add_attrs [ AInfo (Id x); AInfo (Term t') ] t0
                  | t0 -> subst x t' t0)
          in
          (p, sbst t1')
        in
...
let rec get_rtyp_list rtyp typ =
  ...
  | RT.Inter (_, rtyps), _ -> RT.Inter (typ, List.map (get_rtyp_list -$- typ) rtyps)
  | RT.Union (_, rtyps), _ -> RT.Union (typ, List.map (get_rtyp_list -$- typ) rtyps)
  (* TODO RT.Data *)
  | ( RT.Tuple [ (x, RT.Base (TInt, x', p_len)); (_, RT.Fun (y, RT.Base (TInt, y', p_i), typ2)) ],
      TConstr (c, [ typ ]) )
    when Path.(c = C.list) ->
      let p_len' = subst_var x' x p_len in
      let p_i' = subst_var y' y p_i in
      RT.List (x, p_len', y, p_i', get_rtyp_list typ2 typ)
  | RT.Tuple [ (x, RT.Base (TInt, x', p_len)); (_, RT.Inter (_, [])) ], TConstr (c, [ typ ])
    when Path.(c = C.list) ->
      let p_len' = subst_var x' x p_len in
      RT.List (x, p_len', Id.new_var typ_unknown, true_term, RT.Inter (typ, []))
  | RT.Tuple [ (x, RT.Base (TInt, x', p_len)); (_, RT.Inter (_, typs)) ], TConstr (c, [ typ ])
    when Path.(c = C.list) ->
      let typs' =
        List.map
          (fun typ -> RT.Tuple [ (x, RT.Base (TInt, x', p_len)); (Id.new_var typ_unknown, typ) ])
          typs
      in
  ...
  | RT.ExtArg (x, rtyp1, rtyp2), _ -> RT.ExtArg (x, rtyp1, get_rtyp_list rtyp2 typ)
  | RT.Exn (rtyp1, rtyp2), _ -> RT.Exn (get_rtyp_list rtyp1 typ, rtyp2)
  | _ ->
...
let make_get_rtyp_list_of typed get_rtyp f =
  ...
  if !!Flag.Debug.print_ref_typ then
    Format.fprintf !Flag.Print.target "LIST: %a: @[@[%a@]@ ==>@ @[%a@]@]@." Id.print f RT.print rtyp
      RT.print rtyp';
  rtyp'
...
let add_bind bind t =
  let aux (x, t) t' =
    let x' = if occur_in x t then Id.new_var_id x else x in
...
let remove_palias =
  let tr = Tr.make () in
  let tr_pat p =
    match p.pat_desc with PAlias _ -> unsupported "Encode_list.remove_palias" | _ -> tr.pat_rec p
  in
  let tr_desc desc =
    match desc with
    | Match (t, pats) ->
...
let remove_match =
  ...
  let rec get_subst t p =
    ...
    | PTuple ps ->
        ...
        List.fold_right
          Option.(fun sbst acc -> sbst >>= fun x -> lift (( -| ) x) acc)
          sbsts (Some Fun.id)
    | _ -> None
  in
  let tr_desc desc =
    ...
    | Match (t, pats) -> (
        match pats with
        | [ ({ pat_desc = PVar x }, t1) ] | [ ({ pat_desc = PAlias ({ pat_desc = PAny }, x) }, t1) ]
          ->
            let t = tr.term t in
            let t1 = tr.term t1 in
        ...
        | ({ pat_desc = PWhen (p, cond) }, t1) :: pats when Option.is_some @@ get_subst t p ->
            let t', bind =
              if has_no_effect t then (t, Fun.id)
              else
            ...
            let t2' = tr.term @@ Term.match_ ~typ:t1.typ t' pats in
...
let make_rand typ =
  ...
  | TConstr (c, [ typ' ]) when c = C.list ->
      ...
      Term.(
        let_
          [ (l, randi) ]
          (assume (int 0 <= var l) (pair (var l) (fun_ (Id.new_var Ty.int) (rand typ')))))
  | _ -> make_rand_unit typ

let encode_list_term post t =
  match t.desc with
  | App ({ desc = Const (Rand (TBase TInt, false)); attr }, t2) when List.mem AAbst_under attr ->
  ...
  | Cons _ ->
      ...
      let x_t2 = new_var_of_term t2 in
      let bindings = (x_t2, t2) :: (List.rev @@ List.combine xs ts) in
      (* TODO: Use Flag.EvalStrategy *)
      let x = Id.new_var ~name:"i" Ty.int in
      let n = List.length ts in
      let _, t =
        let t0 = Term.(snd (var x_t2) @ [ var x - int n ]) in
        let aux y (i, t) = (i - 1, Term.(if_ (var x = int i) (var y) t)) in
        List.fold_right aux xs (n - 1, t0)
      in
      Term.(lets bindings (pair (int n + fst (var x_t2)) (pfun x t)))
  | Match (t1, pats) ->
      let t1' = encode_list.term post t1 in
      let aux (p, t) =
        let p', bind = encode_list_pat p in
      ...
      let pats' = List.map aux pats in
      Term.(match_ t1' pats')
  | _ -> t |> encode_list.term_rec post |> set_afv_shallowly
...
let make_list_eq typ =
  let ty_list = make_tlist typ in
  let f =
    Id.new_var ~name:"Primitive.list_eq"
    @@ pureTFun (Id.new_var ~name:"xs" ty_list, pureTFun (Id.new_var ~name:"ys" ty_list, Ty.bool))
  in
  let xs = Id.new_var ~name:"xs'" ty_list in
  let ys = Id.new_var ~name:"ys'" ty_list in
  let t_eq =
    let pat_nil = (Pat.(nil ~elem_typ:typ * nil ~elem_typ:typ), Term.true_) in
    let pat_cons =
      let x = Id.new_var ~name:"x" typ in
      let xs' = Id.new_var ~name:"xs'" ty_list in
      let y = Id.new_var ~name:"y" typ in
      let ys' = Id.new_var ~name:"ys'" ty_list in
      (Pat.(cons (var x) (var xs') * cons (var y) (var ys')), [%term x = y && f xs' ys'])
    in
    let pat_any = (Pat.(__ ty_list * __ ty_list), Term.false_) in
    Term.(match_ (var xs * var ys) [ pat_nil; pat_cons; pat_any ])
  in
  (f, Term.funs [ xs; ys ] t_eq)
...
let inst_list_eq =
  ...
  let fld_desc env desc =
    match desc with
    | BinOp (Eq, t1, { desc = Nil }) | BinOp (Eq, { desc = Nil }, t1) ->
  ...
  fun t ->
    ...
    let defs =
      let make =
        if !Flag.Abstract.list_eq then (fun (ty, (eq, _)) ->
          Flag.Abstract.over "List equality";
...
let encode_list_opt_typ typ =
  ...
  | TVar (_, { contents = None }, _) ->
      failwith "Polymorphic types occur! (Encode_list.encode_list_opt_typ)"
  | TConstr (c, [ typ ]) when c = C.list ->
      TFun (Id.new_var ~name:"i" Ty.int, opt_typ_enc @@ encode_list_opt.typ typ)
...
let rec get_match_bind_cond_opt t p =
  ...
  | PCons _ ->
      ...
      let rec aux bind cond i = function
        ...
        | p :: ps ->
            let bind', cond' =
              get_match_bind_cond_opt (make_get_val (make_app t [ make_int i ])) p
            in
  ...
  | PTuple ps ->
      let binds, conds =
        List.split @@ List.mapi (fun i p -> get_match_bind_cond_opt (make_proj i t) p) ps
      in
      (List.rev_flatten binds, List.fold_left make_and true_term conds)
  | _ ->
      Format.eprintf "get_match_bind_cond_opt: %a@." Print.pattern p;
      assert false
...
let encode_list_opt_term t =
  ...
  | Cons (t1, t2) ->
      ...
      if true then Term.(lets [ (x, t1'); (xs, t2') ] (fun_ i (if_ t11 t12 t13)))
      else
...
let trans_term t =
  let tr = if !Flag.Encode.encode_list_opt then trans_opt else trans in
  t
...
let trans_var x =
  if !Flag.Encode.encode_list_opt then encode_list_opt.var x else encode_list.var "" x

let trans_typ typ =
  if !Flag.Encode.encode_list_opt then encode_list_opt.typ typ else encode_list.typ "" typ
...
let rec trans_rty ty =
  ...
  | List (x, p_len, y, p_i, ty2) ->
      if !Flag.Encode.encode_list_opt then unsupported "encode_list_opt"
      else
