...
let rec decomp_bool t =
  ...
  | App (App (Const EqInt, t1), t2) when !Flag.PredAbst.decomp_eq_pred ->
      [ make_leq t1 t2; make_geq t1 t2 ]
  | _ -> [ t ]
...
let merge_preds ps1 ps2 =
  ...
  let ps2'' = if !Flag.PredAbst.decomp_pred then List.flatten_map decomp_bool ps2' else ps2' in
  let add p ps =
    let equiv t1 t2 =
      let t1' = FpatInterface.conv_formula t1 in
      let t2' = FpatInterface.conv_formula t2 in
      FpatInterface.implies [ t1' ] [ t2' ] && FpatInterface.implies [ t2' ] [ t1' ]
    in
    if List.exists (equiv p) ps then ps else normalize_bool_term p :: ps
  in
...
let rec merge_typ env typ typ' =
  ...
  | TConstr (TFixPred p, [ TBase (b1, _) ], _), TBase (b2, ps)
    when b1 = b2 ->
      if ps (Var "x") <> [] then (
  ...
  | TBase (b1, ps1), TBase (b2, ps2) when b1 = b2 -> TBase (b1, merge_preds ps1 ps2)
  ...
  | TBase _, _ | TFun _, _ | TConstr _, _ ->
...
let merge_typ typ1 typ2 =
  ...
  with _ when !!Debug.check ->
    Format.eprintf "Cannot merge@.  TYPE 1: %a@.  TYPE 2: %a@." CEGAR_print.typ typ1 CEGAR_print.typ
      typ2;
    assert false
...
let rec negate_typ = function
  ...
  | TFun (typ1, typ2) ->
      ...
      TFun (typ1, typ2)
  | TConstr _ as typ ->
      Format.eprintf "negate_typ: %a." CEGAR_print.typ typ;
      assert false
...
let trans_var x =
  let s = Id.to_string x |> String.filter_out (fun c -> List.mem c [ '('; ')'; ' ' ]) in
  if String.is_sign s.[0] || String.starts_with s "op" then "op" ^ s else s
...
let rec trans_inv_term t =
  ...
  | App (Const Not, t) -> Term_util.make_not (trans_inv_term t)
...
let rec preds_of typ =
  ...
  | Type.TAttr (attr, _) -> (
      let ypss =
        List.filter_map (function Type.TAPred (y, ps) -> Some (y, ps) | _ -> None) attr
      in
      match ypss with
      | [] -> fun _ -> []
      | [ (y, ps) ] ->
...
and trans_typ (ty : S.typ) =
  ...
  | TAttr (TAPred (x, ps) :: attrs, typ) -> (
      ...
      in
      match trans_typ @@ TAttr (attrs, typ) with
      | TBase (b, preds) ->
          let preds' y = List.map (subst x' y) ps' @ preds y in
          TBase (b, preds')
      | typ ->
          Format.eprintf "trans_typ[TPred]: %a@." CEGAR_print.typ typ;
          assert false)
  | TAttr (_ :: attrs, typ) -> trans_typ @@ Type.TAttr (attrs, typ)
...
and trans_term post xs env t =
  ...
  | App ({ desc = Const (Rand (TBase TInt, false)); attr }, [ { desc = Const Unit } ])
    when List.mem S.AAbst_under attr ->
  ...
  | App ({ desc = Const (Rand (Type.TConstr _, true)) }, [ _t1 ]) -> assert false
  (*
  ...
  | App ({ desc = Const (Rand (typ, true)) }, [ t1; t2 ]) ->
      assert (t1.desc = Const Unit);
      let defs2, t2' = trans_term post xs env t2 in
      let base = match trans_typ typ with TBase (base, _) -> base | _ -> assert false in
  ...
  | BinOp (Eq, t1, t2) ->
      ...
      let op =
        ...
        | typ ->
            Format.eprintf "trans_term: %a@." Print.typ typ;
            assert false
      in
...
let rec formula_of t =
  ...
  | BinOp (Eq, t1, t2) ->
      ...
      let op =
        ...
        | TConstr (ty, _) -> CmpPoly (TU.string_of_path ty, "=")
        | _ ->
            Format.eprintf "%a@." Print.typ t1.S.typ;
            assert false
      in
  ...
  | _ ->
      Format.eprintf "formula_of: %a@." Print.desc_constr t.desc;
      assert false
...
let trans_def (f, (xs, t)) =
  ...
  try
    ...
    | S.If (t1, t2, t3) ->
        ...
        ((f', typ', xs', t1', t2') :: defs2) @ ((f', typ', xs', make_not t1', t3') :: defs3)
    | _ -> raise Not_found
  with Not_found ->
    let defs, t' = trans_term post xs' env t in
    let typ' = trans_typ @@ Id.typ f in
...
let rec is_CPS_value env = function
  ...
  | App
      ( App
          ( Const
              ( And | EqUnit | EqInt | EqBool | CmpPoly _ | Or | Lt | Gt | Leq | Geq | Add | Sub
              | Mul | Div ),
            t1 ),
        t2 ) ->
      is_CPS_value env t1 && is_CPS_value env t2
...
let is_CPS { env; defs } = List.for_all (is_CPS_def env) defs

(* TODO: Refine *)
let event_of_temp { env; defs; main; info } =
  if List.mem ACPS info.attr then
      ...
      | App (Const (Temp s), t2') ->
          ...
          ( [
              ( g,
                TFun
                  ( typ_unit,
                    fun _ -> TFun (TFun (typ_unit, fun _ -> typ_result), fun _ -> typ_result) ) );
            ],
    ...
    let evt_defs =
      List.map
        (fun (e, f) -> { fn = f; args = [ "u" ]; cond = Const True; body = Term.(event e unit) })
        map
    in
    let rec aux = function
      | Const c -> Const c
...
let rec uniq_env = function
  | [] -> []
  | (f, typ) :: env ->
      if List.exists (fun (g, _) -> f = g) env then uniq_env env else (f, typ) :: uniq_env env
...
let rename_prog prog =
  ...
  let rename_id' x var_names =
    ...
    if
      List.length (List.filter (( = ) x_name) var_names) <= 1
      && x_name <> "l0"
      && x_name <> "l1" (* for labels in model-checking *)
    then x_name
    else rename_id x
  in
  let make_map_fun (f, _) = (f, rename_id' f var_names) in
  ...
  let defs =
    let rename_def { fn = f; args = xs; cond = t1; body = t2 } =
      Id.save_counter ();
      Id.clear_counter ();
      let var_names' = fun_map |> List.map snd |> ( @@@ ) (List.rev_map id_name xs) in
      let arg_map =
        xs
...
let rec trans_ref_type : CRT.t -> RT.t = function
  ...
  | Fun (x, typ1, typ2) -> Fun (trans_inv_var x, trans_ref_type typ1, trans_ref_type typ2)
  | Inter (typ, typs) ->
...
let shorten_arg_name prog =
  let rec fresh ?(n = 0) x used =
    let x' = x ^ if n = 0 then "" else string_of_int n in
    if List.mem x' used then fresh ~n:(n + 1) x used else x'
  in
  let shorten x = match String.split x ~by:"_" with s1, _ -> s1 | exception Not_found -> x in
  let used = List.map (fun { fn } -> fn) prog.defs in
  let defs =
    prog.defs
    |> List.map (fun def ->
           let _, args =
             List.L.fold_left_map def.args ~init:used ~f:(fun used x ->
                 let x' = fresh (shorten x) used in
                 (x' :: used, x'))
           in
           let sbst = subst_map @@ List.map2 (fun x x' -> (x, Var x')) def.args args in
           let cond = sbst def.cond in
           let body = sbst def.body in
           { def with args; cond; body })
  in
  { prog with defs }
...
let trans_prog { Problem.term = t; attr } =
  ...
  let (defs, t_main), _ =
    t |> Trans.trans_let |> Trans.remove_obstacle_type_attribute_for_pred_share |> Lift.lift
  in
  ...
  let env, defs'' =
    List.split_map
      (fun (f, typ, xs, t1, t2) -> ((f, typ), { fn = f; args = xs; cond = t1; body = t2 }))
      defs'
  in
  ...
  in
  let max_id =
    List.fold_left (fun m x -> max m @@ snd @@ decomp_id x) 0
    @@ List.flatten_map (fun def -> def.fn :: def.args) defs''
  in
  ...
  let make_get_rtyp get_rtyp f =
    Debug.printf "rrmap: %a@." Print.(list (id * string)) rrmap;
...
let add_env spec prog =
  let env =
    ...
    let aux (f, typ) =
      let ty = try merge_typ typ @@ List.assoc f spec' with Not_found -> typ in
      (f, ty)
    in
...
let assoc_def labeled t ce defs =
  if List.exists (fun f -> Var f = t) labeled then (List.tl ce, assoc_def_aux defs (List.hd ce) t)
...
let rec step_eval_abst_cbn ce labeled env_abst defs = function
  ...
  | Const (Rand (TBool, _)) ->
      let t = if read_bool () then Const True else Const False in
      (ce, t)
  | Const Unit -> (ce, Const Unit)
  | Var x ->
      let ce', { cond; body } = assoc_def labeled (Var x) ce defs in
  ...
  | App (Const (Label _), t) -> step_eval_abst_cbn ce labeled env_abst defs t
  | App _ as t ->
      let t1, ts = decomp_app t in
      if t1 = Const If then
        match ts with
...
let assoc_def labeled defs ce acc rand_num t =
  if ce = [] && Flag.(!mode = FairNonTermination && !FairNonTermination.break_expansion_ref) then
    ...
    let aux { body = t } =
      (* In fair nonterm mode, trans_ce ends just before 'read_int' *)
      rand_num = Some 0 && is_app_read_int t
    in
    if List.mem f labeled then
      ...
      | c :: ce' ->
          ...
          if aux def then None else Some (ce', acc', def)
    else
      let acc' = 0 :: acc in
...
let rec trans_ce_aux labeled ce acc defs rand_num t k =
  ...
  | App _ when is_app_randint t ->
      ...
      let r = new_id "r" in
      let rand_num' = if is_app_read_int t then Option.map pred rand_num else rand_num in
      if rand_num = Some 0 then (* fair non-termination *)
        init_cont (ce, acc, rand_num', t)
      else trans_ce_aux labeled ce acc defs rand_num' (App (t', Var r)) k
  ...
  | App (t1, t2) -> (
      ...
      if List.length xs > List.length ts then k (ce, acc, rand_num, App (t1, t2))
      else
...
let trans_ce labeled { defs; main } ce rand_num =
  if !Flag.Debug.input_cex then
    Format.fprintf !Flag.Print.target "Obtained counterexample: %a@." Print.(list int) ce;
...
let rec simplify_if_term env fv t =
  ...
  | Const c -> Const c
  | Var x -> Var x
  | App (App (App (Const If, Const True), t2), _) -> simplify_if_term env fv t2
  | App (App (App (Const If, Const False), _), t3) -> simplify_if_term env fv t3
  | App (App (App (Const If, t1), t2), t3) ->
      let add_env t env = if has_rand t then env else t :: env in
      let t1' = t1 |> simplify_if_term env fv |> normalize_bool_term in
      let fv_t1 = get_fv t1' in
      let fv' = fv_t1 @ fv in
      let b = List.Set.(fv && fv_t1) <> [] in
      if b && implies env t1' then simplify_if_term env fv t2
      else if b && implies env (make_not t1') then simplify_if_term env fv t3
      else
        let t2' = simplify_if_term (add_env t1' env) fv' t2 in
        let t3' = simplify_if_term (add_env (make_not t1') env) fv' t3 in
        let t1'', t2'', t3'' =
          match t1' with App (Const Not, t1'') -> (t1'', t3', t2') | _ -> (t1', t2', t3')
        in
        make_if t1'' t2'' t3''
...
let simplify_if { env; defs; main; info } =
  let defs' =
    List.map
      (fun def -> { def with cond = simplify_if_term def.cond; body = simplify_if_term def.body })
      defs
  in
...
let add_fail_to_end prog =
  let aux def =
    let body = if def.body = Const CPS_result then Term.(event "fail" def.body) else def.body in
    { def with body }
  in
...
let rec beta_subst_aux x (y, t1) t2 =
  ...
  | App (t21, t22) -> (
      ...
      match t21' with Fun (y', _, t211) -> beta_subst y' t22' t211 | _ -> App (t21', t22'))
  | Let (y, t21, t22) ->
      let t22' = if x = y then t22 else beta_subst_aux x (y, t1) t22 in
      Let (y, beta_subst_aux x (y, t1) t21, t22')
  | Fun (y, typ, t21) ->
      let t21' = if x = y then t1 else beta_subst_aux x (y, t1) t21 in
      Fun (y, typ, t21')

and beta_subst x t1 t2 =
  match t1 with Fun (y, _, t11) -> beta_subst_aux x (y, t11) t2 | _ -> subst x t1 t2

let rec beta_reduce_term t =
  match t with
  | Const c -> Const c
  | Var x -> Var x
  ...
  | App (t1, t2) -> (
      ...
      match t1' with Fun (x, _, t1') -> subst_reduce x t2' (get_fv t2') t1' | _ -> App (t1', t2'))
  | Fun (x, typ, t) -> Fun (x, typ, beta_reduce_term t)
  | Let _ -> assert false

and subst_reduce x t fv t' =
  let sbst = subst_reduce x t fv in
  match t' with
  | Const c -> Const c
  | Var y when x = y -> t
  | Var y -> Var y
  | App (t1, t2) -> (
      ...
      let t2' = sbst t2 in
      let t' = App (t1', t2') in
      match t1' with Fun _ -> beta_reduce_term t' | _ -> t')
...
let bool_to_int prog =
  let int_of_bool t =
    match t with
    | App (Const Not, t') -> Term.(if_ t' (int 0) (int 1))
    | _ -> Term.(if_ t (int 1) (int 0))
  in
  let bool_of_int t =
    match t with
    | App (App (App (Const If, t1), t2), t3) when t2 = Term.int 1 && t3 = Term.int 0 -> t1
    | App (App (App (Const If, t1), t2), t3) when t2 = Term.int 0 && t3 = Term.int 1 -> Term.not' t1
    | _ -> Term.(not' (t = int 0))
  in
  let rec b2i t =
    match t with
    ...
    | App (Const Not, t) ->
        let t' = b2i_bool t in
        int_of_bool (Term.not' t')
    | App (App (Const ((And | Or) as op), t1), t2) ->
        let op' = match op with And -> Term.( && ) | Or -> Term.( || ) | _ -> assert false in
        let t1' = b2i_bool t1 in
        let t2' = b2i_bool t2 in
        int_of_bool @@ op' t1' t2'
    | App (App (Const EqBool, t1), Const False) -> b2i @@ App (Const Not, t1)
    | App (App (Const EqBool, t1), Const True) -> b2i t1
    | App (App (Const EqBool, t1), t2) ->
        let t1' = b2i_bool t1 in
        let t2' = b2i_bool t2 in
        int_of_bool @@ Term.(((not t1') || t2') && (t1' || not t2'))
    | App (App ((Const (EqInt | Lt | Gt | Leq | Geq) as op), t1), t2) ->
        let t1' = b2i t1 in
        let t2' = b2i t2 in
        int_of_bool (make_app op [ t1'; t2' ])
    | App (App (App (Const If, Const (Rand _)), t2), t3) ->
        let x = new_id "n" in
        let typ = !!Ty.bool in
        let rand = Const (Rand (TInt, None)) in
        let t2' = b2i t2 in
        let t3' = b2i t3 in
        Term.(rand @ [ fun_ x ~typ (if_ (var x = int 0) t2' t3') ])
    | App (App (App (Const If, t1), t2), t3) ->
        let t1' = b2i_bool t1 in
        let t2' = b2i t2 in
        let t3' = b2i t3 in
        Term.(if_ t1' t2' t3')
    | App (t1, t2) -> App (b2i t1, b2i t2)
    | Fun (x, ty, t1) -> Fun (x, ty, b2i t1)
    | _ ->
        Format.eprintf "t: %a@." CEGAR_print.term t;
        [%unsupported]
  and b2i_bool t = b2i t |> bool_of_int in
  let defs =
    prog.defs
    |> List.map (fun def ->
           let cond = b2i_bool def.cond in
           let body = b2i def.body in
           { def with cond; body })
  in
  { prog with defs }

let lift_for_hfl prog =
  let rec lift t =
    let xs = ref [] in
    let newvar t t2 t3 =
      let x = new_id "x" in
    ...
    let rec col arg t =
      ...
      | Const _ -> t
      | Var _ -> t
      | App (App (Const (And | Or | EqBool), _), _) -> Var (newvar t Term.tt Term.ff)
      | App (App (App (Const If, t1), t2), t3) ->
          if arg then Var (newvar t1 t2 t3) else Term.if_ t1 (col false t2) (col false t3)
      | App ((Const (Label _ | Rand _) as t1), t2) -> App (t1, col arg t2)
      | App (t1, t2) -> App (col arg t1, col true t2)
      | Let _ -> [%invalid_arg]
      | Fun (x, ty, t1) -> Fun (x, ty, lift t1)
    in
    let t' = col false t in
    List.fold_left
      (fun t (x, t', t2, t3) ->
        let f = new_id "f" in
        App (Fun (f, None, Term.if_ t' (App (Var f, t2)) (App (Var f, t3))), Fun (x, None, t)))
      t' !xs
  in
  map_body_prog lift prog
