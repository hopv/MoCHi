...
let rec print_of_type mk_ident (ty:core_type) =
  ...
  | Ptyp_constr(c, tys) ->
      ...
      if tys = [] then
        ...
        Ast_helper.Exp.apply ~loc id args
  | Ptyp_tuple [ty1; ty2] ->
      let t1 = pot ty1 in
      let t2 = pot ty2 in
      [%expr Print.pair [%e t1] [%e t2]]
  | Ptyp_tuple [ty1; ty2; ty3] -> (* TODO: merge & generalize with the above *)
      let t1 = pot ty1 in
      let t2 = pot ty2 in
      let t3 = pot ty3 in
  ...
  | Ptyp_poly(_, _)
  | Ptyp_package _
  | Ptyp_extension _ -> Location.raise_errorf ~loc "Unsupported syntax of %%ty: %a" Pprintast.core_type ty
