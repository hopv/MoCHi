...
let constrs =
  List.map (fun (s1,s2) -> Lident s1, s2)
    ["()", "unit";
     "true", "true_";
     "false", "false_";
     "[]", "nil"; (* unused *)
...
let rec term_of_pattern p =
  ...
  | Ppat_constraint({ppat_desc = Ppat_any; _}, ty) ->
  ...
  | Ppat_tuple ps ->
      ...
      [%expr Pat.tuple [%e ps]]
  | Ppat_constant (Pconst_integer _ as c) ->
      [%expr Pat.const (Term.int [%e Ast_helper.Exp.constant c])]
  | Ppat_alias(p1, x) ->
  ...
  | Ppat_construct ({txt=Lident "::"; _}, Some (_, {ppat_desc = Ppat_tuple [p1; p2]; _})) ->
      let p1 = term_of_pattern p1 in
      let p2 = term_of_pattern p2 in
  ...
  | Ppat_exception _
  | Ppat_extension _
  | Ppat_open (_, _) -> Location.raise_errorf ~loc "Unsupported syntax of %%term (pattern)"
...
class my_mapper = object (self)
  ...
  method! expression e =
    ...
    | Pexp_construct(constr, arg) when constr.txt = Lident "::" ->
        begin
          match arg with
          | Some {pexp_desc = Pexp_tuple [e1; {pexp_desc = Pexp_construct(constr, None); _}]; _} when constr.txt = Lident "[]" ->
    ...
    | Pexp_assert {pexp_desc = Pexp_construct(constr, None); _} when constr.txt = Lident "false" -> [%expr Term.fail]
    | _ when is_var e "bot" ->
        let ty = find_typ_in_attr ~loc e.pexp_attributes in
        [%expr Term.bot [%e ty]]
    ...
    | Pexp_sequence({pexp_desc = Pexp_apply({pexp_desc = Pexp_ident id; _}, [Nolabel, e1]); _}, e2) when id.txt = Lident "assume" ->
        let e1 = self#expression e1 in
        let e2 = self#expression e2 in
    ...
    | Pexp_variant(label, None) ->
        Ast_helper.Exp.(ident ~loc {txt=Lident label; loc})
    | _ ->
